...
# -*- coding: utf-8 -*-
"""
Cu@Ag Core-Shell Nanostructure Quantitative Descriptor Graph v6.2 (CS Edition)
================================================================================
Multi-level reasoning concept graph for numerical/quantitative description
of Cu@Ag core-shell nanostructures.
Focus: Synthesis, Structural, Optical, Catalytic, and Stability Descriptors.
...
pattern from the working AgNPs code. The domain ontology and extraction patterns have
been replaced with those for Cu@Ag core-shell quantitative descriptors.
...
DOMAIN: Cu@Ag Core-Shell Nanostructures
- Materials: core-shell nanoparticle (Cu@Ag), Cu core, Ag shell, core-shell interface
- Synthesis: galvanic replacement, seed-mediated growth, wet-chemical synthesis, co-precipitation
- Properties: plasmon resonance (LSPR), catalytic activity, stability, morphology, cytotoxicity
- Phenomena: sintering, oxidation, coalescence
- Parameters: shell thickness, core size, temperature, reaction time, precursor concentration, pH
- Methods: UV-Vis spectroscopy, TEM, SEM, XRD, EDS, CV, EIS
...
Run:
    streamlit run cs_concept_graph_v6.2_llm.py
...
# ============================================================================
st.set_page_config(
    page_title="Cu@Ag Core-Shell Nanostructure Quantitative Descriptor Graph v6.2",
    page_icon="🧪",
    layout="wide",
...
class DomainOntology:
    """Comprehensive ontology for Cu@Ag Core-Shell Nanostructure Quantitative Descriptors."""
...
class AdvancedConceptResolver:
    ...
    def _context_disambiguation(
        ...
        optical_indicators = ['plasmon', 'lspr', 'uv-vis', 'absorption']
        catalytic_indicators = ['catalytic', 'activity', 'co2', 'reduction']
        structural_indicators = ['shell', 'core', 'thickness', 'morphology']
        if any(ind in context_lower for ind in optical_indicators):
            if 'plasmon' in text or 'lspr' in text:
                return "plasmonic_property"
        if any(ind in context_lower for ind in catalytic_indicators):
            if 'catalytic' in text or 'activity' in text:
                return "catalytic_activity"
        if any(ind in context_lower for ind in structural_indicators):
            if 'shell' in text or 'thickness' in text:
...
class EnhancedConceptExtractor:
    ...
    def _build_extraction_patterns(self) -> None:
        # Cu@Ag core-shell specific patterns
        self.material_patterns = [
            r'\bcu@ag\b', r'\bcu@ag\s+core-shell\b', r'\bcore-shell\s+nanoparticle[s]?\b',
            r'\bbimetallic\s+nanoparticle[s]?\b', r'\bcu\s+core\b', r'\bag\s+shell\b',
        ]
        self.process_patterns = [
            r'\bgalvanic\s+replacement\b', r'\bgalvanic\s+exchange\b',
            r'\bseed[- ]mediated\s+growth\b', r'\bco[- ]precipitation\b',
            r'\bwet[- ]chemical\s+synthesis\b', r'\bencapsulation\b', r'\bcoating\b',
        ]
        self.property_patterns = [
            r'\bplasmon\s+resonance\b', r'\blspr\b',
            r'\bcatalytic\s+activity\b', r'\bcatalytic\s+performance\b',
            r'\bstability\b', r'\bcolloidal\s+stability\b', r'\boxidation\s+resistance\b',
            r'\bcytotoxicity\b', r'\bbiocompatibility\b',
            r'\bmorphology\b', r'\bshape\b', r'\bsurface\s+roughness\b',
        ]
        self.phenomena_patterns = [
            r'\bsintering\b', r'\bcoalescence\b', r'\boxidation\b', r'\bcu\s+oxidation\b',
        ]
        self.param_patterns = [
            r'\bshell\s+thickness\b', r'\bcore\s+diameter\b', r'\bcore\s+size\b',
            r'\bag\s+shell\s+thickness\b',
            r'\btemperature\b', r'\breaction\s+time\b', r'\bprecursor\s+concentration\b', r'\bph\b',
        ]
        self.method_patterns = [
            r'\buv[- ]vis\s+spectroscopy\b', r'\btransmission\s+electron\s+microscopy\b', r'\btem\b',
            r'\bscanning\s+electron\s+microscopy\b', r'\bsem\b', r'\bxrd\b', r'\bx[- ]ray\s+diffraction\b',
            r'\benergy[- ]dispersive\s+x[- ]ray\s+spectroscopy\b', r'\beds\b',
            r'\bcyclic\s+voltammetry\b', r'\bcv\b',
            r'\belectrochemical\s+impedance\s+spectroscopy\b', r'\beis\b',
        ]
        self.all_patterns = (
            self.material_patterns + self.process_patterns +
            self.property_patterns + self.phenomena_patterns +
            self.param_patterns + self.method_patterns
        )
...
# ============================================================================
MATERIAL_KEYWORDS = [
    "cu@ag", "cu@ag core-shell", "core-shell nanoparticle", "bimetallic nanoparticle",
    "cu core", "ag shell", "copper core", "silver shell"
]
PROPERTY_KEYWORDS = [
    "plasmon resonance", "lspr", "localized surface plasmon resonance",
    "catalytic activity", "catalytic performance", "catalytic efficiency",
    "stability", "colloidal stability", "oxidation resistance",
    "cytotoxicity", "biocompatibility", "morphology", "shape"
]
PHENOMENON_KEYWORDS = [
    "sintering", "coalescence", "oxidation", "cu oxidation", "tarnishing"
]
PROCESS_KEYWORDS = [
    "galvanic replacement", "galvanic exchange", "seed-mediated growth",
    "co-precipitation", "wet-chemical synthesis", "encapsulation", "coating"
]
PARAM_KEYWORDS = [
    "shell thickness", "core diameter", "core size", "ag shell thickness",
    "temperature", "reaction time", "precursor concentration", "ph"
]
METHOD_KEYWORDS = [
    "uv-vis spectroscopy", "transmission electron microscopy", "tem",
    "scanning electron microscopy", "sem", "xrd", "x-ray diffraction",
    "energy-dispersive x-ray spectroscopy", "eds", "cyclic voltammetry", "cv"
]
ALL_DOMAIN_KEYWORDS = (
...
# Regex patterns and descriptor mapping (renamed to CS_PATTERNS / CS_DESCRIPTOR_MAPPING)
CS_PATTERNS = [
    r'\bcu@ag\b', r'\bcu@ag\s+core-shell\b', r'\bcore-shell\s+nanoparticle[s]?\b',
    r'\bbimetallic\s+nanoparticle[s]?\b', r'\bcu\s+core\b', r'\bag\s+shell\b',
    r'\bgalvanic\s+replacement\b', r'\bgalvanic\s+exchange\b',
    r'\bseed[-\s]mediated\s+growth\b', r'\bco[-\s]precipitation\b',
    r'\bwet[-\s]chemical\s+synthesis\b', r'\bencapsulation\b', r'\bcoating\b',
    r'\bplasmon\s+resonance\b', r'\blspr\b',
    r'\bcatalytic\s+activity\b', r'\bcatalytic\s+performance\b',
    r'\bstability\b', r'\bcolloidal\s+stability\b', r'\boxidation\s+resistance\b',
    r'\bcytotoxicity\b', r'\bbiocompatibility\b',
    ...
    r'\benergy[-\s]dispersive\s+x[-\s]ray\s+spectroscopy\b', r'\beds\b',
    r'\bcyclic\s+voltammetry\b', r'\bcv\b',
    r'\belectrochemical\s+impedance\s+spectroscopy\b', r'\beis\b'
]

CS_DESCRIPTOR_MAPPING = {
    r'cu@ag|core-shell nanoparticle|bimetallic nanoparticle|cu core|ag shell': 'material',
    ...
    r'uv-vis spectroscopy|transmission electron microscopy|tem|sem|xrd|x-ray diffraction|eds|cyclic voltammetry|cv|eis': 'method',
    r'general': 'general'
}
...
def is_valid_cs_concept(concept: str) -> bool:
    ...
    generic = {
        ...
        'propose', 'develop', 'investigate', 'discuss', 'conclude', 'nanoparticle',
        'particle', 'material', 'system', 'sample', 'specimen'
    }
...
def extract_concepts_from_text(text: str) -> List[str]:
    ...
    noun_pattern = (
        r'\b(?:[a-z]+(?:[-\s]?[a-z]+){0,2}[-\s]?)?'
        r'(?:plasmon|lspr|activity|stability|morphology|shell|core|sintering|oxidation|thickness|size|temperature|time|concentration|ph)\b'
    )
...
def extract_concepts_from_abstracts(
    ...
    for idx, row in df.iterrows():
        ...
        # Extract numeric metrics typical for core-shell literature
    ...
    for concept in concepts:
        ...
        for pattern, category in CS_DESCRIPTOR_MAPPING.items():
            if re.search(pattern, concept, re.I):
                ...
                break
        if not matched:
            if any(re.search(p, concept, re.I) for p in [r'core-shell', r'cu@ag', r'cu core', r'ag shell']):
                concept_to_abstract[concept] = 'material'
            elif any(re.search(p, concept, re.I) for p in [r'galvanic', r'seed-mediated', r'wet-chemical', r'co-precipitation', r'encapsulation', r'coating']):
                concept_to_abstract[concept] = 'process'
            elif any(re.search(p, concept, re.I) for p in [r'plasmon', r'lspr', r'catalytic', r'stability', r'morphology', r'cytotoxicity']):
                concept_to_abstract[concept] = 'property'
            elif any(re.search(p, concept, re.I) for p in [r'sintering', r'oxidation', r'coalescence']):
                concept_to_abstract[concept] = 'phenomenon'
            elif any(re.search(p, concept, re.I) for p in [r'shell thickness', r'core size', r'temperature', r'time', r'concentration', r'ph']):
                concept_to_abstract[concept] = 'parameter'
            elif any(re.search(p, concept, re.I) for p in [r'uv-vis', r'tem', r'sem', r'xrd', r'eds', r'cv', r'eis']):
...
def export_publication_figure(
    ...
    try:
        ...
        plt.title(
            "Cu@Ag Core-Shell Nanostructure Quantitative Descriptor Graph",
...
def generate_analysis_report(
    ...
    report: List[str] = []
    report.append("# Cu@Ag Core-Shell Nanostructure Quantitative Descriptor Graph Analysis Report")
    report.append(
    ...
    report.append("")
    report.append("---")
    report.append("*Report generated by Cu@Ag Core-Shell Nanostructure Quantitative Descriptor Graph v6.2*")
    return "\n".join(report)
...
def render_pyvis_graph(
    ...
    for i, node in enumerate(nx_graph.nodes()):
        ...
        # FIX 1: Plain text with newlines prevents Vis.js HTML parsing truncation
        tooltip_content = (
            f"{node}\n"
            f"Type: {concept_type}\n"
            f"Degree: {degree}\n"
    ...
    # FIX 3: Robust CSS injection (SIB app pattern)
    custom_css = f"""
    <style>
    ...
    div.vis-tooltip {{
        max-width: 540px !important; width: auto !important;
        max-height: 280px !important; height: auto !important;
        overflow-y: auto !important; overflow-x: hidden !important;
        z-index: 10000 !important;
        white-space: pre-wrap !important; /* Preserves newlines */
        word-wrap: break-word !important; overflow-wrap: break-word !important;
        line-height: 1.45 !important; padding: 10px 14px !important;
        border-radius: 8px !important; box-shadow: 0 4px 16px rgba(0,0,0,0.25) !important;
    }}
            ...
            var checkExist = setInterval(function() {
                if (typeof network !== 'undefined' && network !== null && network.body && network.body.data) {
                    ...
                    function showEdgeInfoPanel(nodeId, connectedEdges) {
                        ...
                        // Use nodeId directly as the primary name to avoid tooltip parsing truncation
                        var nodeName = nodeId;
    ...
    # FIX 5: Downloadable HTML for the Pyvis Concept Graph
    try:
        html_bytes = html_content.encode('utf-8')
        st.download_button(
            "📥 Download Interactive Graph (HTML)",
            data=html_bytes,
            file_name="cs_concept_graph.html",
            mime="text/html"
        )
        del html_bytes
        gc.collect()
...
_SUNBURST_CATEGORY_COLORS = {
    ...
    "Synthesis Methods":         "#E67E22",
    "Optical Properties":        "#2ECC71",
    "Functional Properties":     "#F1C40F",
    "Phenomena":                 "#3498DB",
    "Characterization Methods":  "#9B59B6",
    "Structural Parameters":     "#1ABC9C",
    "Reaction Conditions":       "#2980B9",
}
    ...
    if export_format == "GraphML":
        try:
            if include_metadata:
                ...
                nx_graph.graph['tool'] = 'CS-ConceptGraph'
            try:
                nx.write_graphml_lxml(nx_graph, "cs_graph.graphml")
            except Exception:
                nx.write_graphml(nx_graph, "cs_graph.graphml")
            with open("cs_graph.graphml", "rb") as f:
                return f.read(), "application/graphml+xml", "cs_graph.graphml"
        except Exception as e:
        ...
        return json_str.encode('utf-8'), "application/json", "cs_graph_full.json"
    elif export_format == "JSON (Compact)":
        data = nx.node_link_data(nx_graph)
        json_str = json.dumps(data, indent=2, default=str)
        return json_str.encode('utf-8'), "application/json", "cs_graph.json"
        ...
        return csv_df.to_csv(index=False).encode('utf-8'), "text/csv", "cs_edges_enhanced.csv"
        ...
        return csv_df.to_csv(index=False).encode('utf-8'), "text/csv", "cs_nodes_enhanced.csv"
        ...
        try:
            ...
            buf.seek(0)
            plt.close()
            return buf.read(), "image/png", "cs_graph.png"
        except Exception as e:
        ...
        try:
            ...
            buf.seek(0)
            plt.close()
            return buf.read(), "image/svg+xml", "cs_graph.svg"
        except Exception as e:
        ...
        try:
            if include_metadata:
                nx_graph.graph['created'] = datetime.now().isoformat()
                nx_graph.graph['version'] = '6.2'
            nx.write_gexf(nx_graph, "cs_graph.gexf")
            with open("cs_graph.gexf", "rb") as f:
                return f.read(), "application/xml", "cs_graph.gexf"
        except Exception as e:
...
def render_reasoning_dashboard(
    ...
    synonym_examples = [
        ("core-shell nanoparticle", "core_shell_nanoparticle"),
        ("cu@ag", "core_shell_nanoparticle"),
        ("plasmon resonance", "plasmonic_property"),
...
def render_sidebar() -> None:
    with st.sidebar:
        ...
        st.markdown("- **Synthesis:** Galvanic replacement, seed-mediated growth, wet-chemical synthesis, co-precipitation")
        st.markdown("- **Properties:** Plasmon resonance (LSPR), catalytic activity, stability, morphology, cytotoxicity")
        st.markdown("- **Phenomena:** Sintering, oxidation, coalescence")
        st.markdown("- **Parameters:** Shell thickness, core size, temperature, reaction time, precursor concentration")
        st.markdown("- **Methods:** UV-Vis, TEM, SEM, XRD, EDS, CV, EIS")
        st.subheader("🧠 NLP Reasoning Options")
        st.session_state['use_ontology'] = st.checkbox(
            "Use ontology-based resolution", value=True,
            help="Maps synonyms like 'core-shell', 'Cu@Ag' to canonical concepts",
        )
...
CS_PROBLEM_DEFINITIONS: Dict[CSCoreProblem, CSProblemDefinition] = {
    ...
    CSCoreProblem.PLASMON_TUNABILITY: CSProblemDefinition(
        ...
        relevant_materials=["core_shell_nanoparticle", "cu_core", "ag_shell"],
        relevant_phenomena=[],
        relevant_properties=["plasmonic_property", "morphology"],
        example_queries=[
            "How does Mie theory explain the LSPR tuning in Cu@Ag core-shell nanoparticles?",
            "What is the optimal core size for plasmonic applications?"
        ],
    ...
    CSCoreProblem.CATALYTIC_ACTIVITY_ENHANCEMENT: CSProblemDefinition(
        ...
        relevant_materials=["core_shell_nanoparticle", "cu_core", "ag_shell"],
        relevant_phenomena=[],
        relevant_properties=["catalytic_activity", "morphology", "stability"],
    ...
    CSCoreProblem.OXIDATION_RESISTANCE: CSProblemDefinition(
        ...
        relevant_phenomena=["oxidation"],
        relevant_properties=["stability", "catalytic_activity"],
    ...
    CSCoreProblem.SYNTHESIS_REPRODUCIBILITY: CSProblemDefinition(
        ...
        relevant_materials=["core_shell_nanoparticle"],
        relevant_phenomena=[],
...
# ============================================================================
# 2. LLM QUERY ANALYZERS (Abstract + Implementations) — unchanged from SIB version
# ============================================================================
...
class FallbackAnalyzer(LLMQueryAnalyzer):
    PROBLEM_KEYWORDS = {
        CSCoreProblem.SHELL_UNIFORMITY: {"shell uniformity", "uniform shell", "galvanic replacement", "shell thickness", "morphology"},
        CSCoreProblem.PLASMON_TUNABILITY: {"plasmon", "lspr", "tunability", "core size", "shell thickness", "mie theory", "mie", "optical"},
        CSCoreProblem.CATALYTIC_ACTIVITY_ENHANCEMENT: {"catalytic activity", "co2 reduction", "catalysis", "interface"},
        CSCoreProblem.OXIDATION_RESISTANCE: {"oxidation", "stability", "corrosion", "cu oxidation", "tarnishing"},
        CSCoreProblem.SYNTHESIS_REPRODUCIBILITY: {"reproducibility", "batch", "synthesis", "parameter", "reaction conditions"},
    }
...
class OpenAIQueryAnalyzer(LLMQueryAnalyzer):
    # Same as SIB version, but using CSCoreProblem and CS_PROBLEM_DEFINITIONS
    ...
    def analyze_query(self, query: str, ontology: Any) -> QueryAnalysisResult:
        ...
        system_prompt = """You are an expert in Cu@Ag core-shell nanostructures. Analyze the user's query and return ONLY valid JSON with:
...
class LocalLLMQueryAnalyzer(LLMQueryAnalyzer):
    # Same as SIB version, but using the CS problem definitions
    ...
    def analyze_query(self, query: str, ontology: Any) -> QueryAnalysisResult:
        if not self.is_available():
            return FallbackAnalyzer().analyze_query(query, ontology)
        prompt = (
            f"[INST] You are an expert on Cu@Ag core-shell nanostructures. Analyze: '{query}'. "
...
# ============================================================================
# 3. DYNAMIC ONTOLOGY EXPANDER (unchanged architecture, uses CS problem defs)
# ============================================================================
...
# ============================================================================
# 4. PRIORITY-GUIDED SUBGRAPH EXTRACTOR & VISUALIZER (unchanged architecture)
# ============================================================================
...
class GraphRAGAnswerGenerator:
    ...
    def generate_ground_response(self, query: str, analysis: QueryAnalysisResult, subgraph: nx.Graph, concept_abstract_map: Dict[str, List[int]], all_texts: Union[List[str], Dict[int, str]], max_docs_per_concept: int = 2) -> str:
        ...
        nl = chr(10)
        prompt = "You are an expert on Cu@Ag core-shell nanostructures. Answer the user's query based *strictly* on the provided graph context and evidence snippets." + nl
        ...
        if evidence_snippets:
            ...
            prompt += "No direct text snippets found. Rely on your general knowledge of Cu@Ag core-shell systems but note the lack of specific retrieved context." + nl
...
# ============================================================================
# 5. STREAMLIT UI INTEGRATORS (unchanged from SIB version, but using CS-specific definitions)
# ============================================================================
def render_llm_query_panel(ontology: Any, expander: DynamicOntologyExpander, full_graph: nx.Graph) -> Optional[QueryAnalysisResult]:
    ...
    query = st.sidebar.text_area("Your Cu@Ag question:", value=selected_example, height=100, key="llm_query_input", placeholder="e.g., How does shell thickness affect the LSPR of Cu@Ag?")
...
def render_llm_qa_tab(analysis_data: Dict, ontology: Any):
    ...
    with col1: query = st.text_input("Enter your Cu@Ag research question:", placeholder="e.g., How does shell thickness affect the LSPR of Cu@Ag?")
...
# ============================================================================
# 6. MAIN APPLICATION FUNCTION (unchanged, but with CS domain)
# ============================================================================
def main() -> None:
    st.title(
        "🧪 Cu@Ag Core-Shell Nanostructure Quantitative Descriptor Graph v6.2"
    )
    st.caption(
        "Multi-level reasoning concept graph for numerical/quantitative description of Cu@Ag core-shell nanostructures | "
        "Focus: Synthesis, Structural, Optical, Catalytic, and Stability Descriptors | "
        ...
        with tabs[tab_idx]:
            ...
            if top_scores.empty:
                ...
                st.download_button(
                    ...
                    file_name="cs_research_directions.csv", mime="text/csv",
                )
        ...
        with tabs[tab_idx]:
            ...
            if st.button("Generate Publication Figure"):
                ...
                if pub_bytes:
                    st.download_button(
                        ...
                        data=pub_bytes,
                        file_name="cs_graph_publication.png",
                        mime="image/png",
                    )
            st.markdown("---")
            ...
            if st.button("Generate Markdown Report"):
                ...
                st.download_button(
                    ...
                    file_name="cs_analysis_report.md",
                    mime="text/markdown",
                )
            ...
            st.download_button(
                ...
                data=csv_concepts,
                file_name="cs_concepts_enhanced.csv", mime="text/csv",
            )
