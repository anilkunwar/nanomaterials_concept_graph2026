...
Focus: Materials, Synthesis, Properties, Phenomena, Parameters, Methods.
...
pattern. The domain ontology and extraction patterns have been replaced with those
for Cu@Ag core-shell nanoparticles.

NEW in v6.2 — LLM-Guided Q&A with dynamic ontology expansion.
NEW in v6.0 — BATCH PROCESSING MODE (Streamlit Cloud ≤ 1 GB RAM).
v6.1 memory-hotfixes are retained.

DEPLOYMENT:
pip install streamlit torch transformers sentence-transformers networkx scikit-learn
pip install pyvis plotly pandas numpy kaleido matplotlib scipy seaborn bibtexparser openai
...
class RelationshipType(Enum):
    ...
    PREVENTS = "prevents"   # added for oxidation protection
...
class DomainOntology:
    ...
    def _build_causal_chains(self) -> None:
        causal_chains = [
            ...
            # Protection
            ("ag_shell", RelationshipType.PREVENTS, "oxidation", 0.88),
        ]
...
class EnhancedConceptExtractor:
    ...
    def _build_extraction_patterns(self) -> None:
        ...
        self.material_patterns = [
            ...
            r'\bcu\s+core\b', r'\bag\s+shell\b', r'\bcopper\s+core\b', r'\bsilver\s+shell\b',
        ]
        ...
        self.property_patterns = [
            r'\bplasmon\s+resonance\b', r'\blspr\b', r'\bcatalytic\s+activity\b', r'\bcatalytic\s+performance\b',
            r'\bstability\b', r'\bcolloidal\s+stability\b', r'\boxidation\s+resistance\b', r'\bcytotoxicity\b',
        ...
        self.param_patterns = [
            r'\bshell\s+thickness\b', r'\bcore\s+diameter\b', r'\bcore\s+size\b', r'\btemperature\b',
            r'\breaction\s+time\b', r'\bprecursor\s+concentration\b', r'\bph\b',
        ]
...
def extract_concepts_from_text(text: str) -> List[str]:
    ...
    noun_pattern = (
        r'\b(?:[a-z]+(?:[-\s]?[a-z]+){0,2}[-\s]?)?'
        r'(?:plasmon|lspr|catalytic|activity|stability|morphology|shell|thickness|core|size|sintering|oxidation|coating|encapsulation)\b'
    )
...
def extract_concepts_from_abstracts(
    ...
    for idx, row in df.iterrows():
        ...
        metrics: Dict[str, Any] = extract_doc_metrics(combined_text)
    ...
    for concept in concepts:
        ...
        for pattern, category in CS_DESCRIPTOR_MAPPING.items():
            if re.search(pattern, concept, re.I):
                ...
                break
        if not matched:
            if any(re.search(p, concept, re.I) for p in [r'cu@ag', r'core-shell', r'cu core', r'ag shell']):
                concept_to_abstract[concept] = 'material'
            elif any(re.search(p, concept, re.I) for p in [r'galvanic', r'seed', r'co-precipitation', r'coating']):
                concept_to_abstract[concept] = 'process'
            elif any(re.search(p, concept, re.I) for p in [r'plasmon', r'catalytic', r'stability', r'cytotoxicity']):
            ...
            elif any(re.search(p, concept, re.I) for p in [r'thickness', r'size', r'temperature', r'time', r'concentration']):
                concept_to_abstract[concept] = 'parameter'
            elif any(re.search(p, concept, re.I) for p in [r'uv-vis', r'tem', r'sem', r'xrd', r'eds', r'cv']):
...
def render_reasoning_dashboard(
    ...
    synonym_examples = [
        ...
        ("copper core", "cu_core"),
        ("silver shell", "ag_shell"),
        ("plasmon resonance", "plasmonic_property"),
        ("catalytic performance", "catalytic_activity"),
    ]
...
def render_sidebar() -> None:
    with st.sidebar:
        ...
        st.session_state['use_ontology'] = st.checkbox(
            ...
            help="Maps synonyms like 'Cu@Ag', 'core-shell' to canonical concepts",
        )
        ...
        st.session_state['use_inference'] = st.checkbox(
            ...
            help="Infers causal chains even when not co-occurring",
        )
...
class OpenAIQueryAnalyzer(LLMQueryAnalyzer):
    ...
    def analyze_query(self, query: str, ontology: Any) -> QueryAnalysisResult:
        ...
        system_prompt = """You are an expert on Cu@Ag core-shell nanoparticles. Analyze the user's query and return ONLY valid JSON with:
...
class LocalLLMQueryAnalyzer(LLMQueryAnalyzer):
    ...
    def analyze_query(self, query: str, ontology: Any) -> QueryAnalysisResult:
        if not self.is_available():
            return FallbackAnalyzer().analyze_query(query, ontology)
        prompt = (
            f"[INST] You are an expert on Cu@Ag core-shell nanoparticles. Analyze: '{query}'. "
...
class GraphRAGAnswerGenerator:
    ...
    def generate_ground_response(self, query: str, analysis: QueryAnalysisResult, subgraph: nx.Graph, concept_abstract_map: Dict[str, List[int]], all_texts: Union[List[str], Dict[int, str]], max_docs_per_concept: int = 2) -> str:
        ...
        nl = chr(10)
        prompt = "You are an expert on Cu@Ag core-shell nanoparticles. Answer the user's query based *strictly* on the provided graph context and evidence snippets." + nl
        ...
        if evidence_snippets:
            ...
            prompt += "No direct text snippets found. Rely on your general Cu@Ag knowledge but note the lack of specific retrieved context." + nl
...
def render_llm_qa_tab(analysis_data: Dict, ontology: Any):
    ...
    with col1: query = st.text_input("Enter your research question:", placeholder="e.g., How does shell thickness affect the catalytic activity of Cu@Ag nanoparticles?")
