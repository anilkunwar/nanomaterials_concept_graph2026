...
=================================================
Multi-level reasoning concept graph for Cu@Ag core-shell nanostructures.
Focus: Synthesis, Properties, and Performance Descriptors.
...
replaced with those for Cu@Ag core-shell nanostructure quantitative descriptors.
...
- Synthesis: Galvanic replacement, seed-mediated growth, wet chemical synthesis
- Properties: Plasmonic (LSPR), Catalytic activity, Stability, Morphology, Cytotoxicity
- Phenomena: Sintering, Oxidation
- Parameters: Shell thickness, Core size, Temperature, Reaction time, Precursor concentration
- Methods: UV-Vis, TEM, XRD, EDS
...
Run:
    streamlit run cuag_concept_graph_v6.2_llm.py
...
# ============================================================================
st.set_page_config(
    page_title="Cu@Ag Core-Shell Nanoparticle Concept Graph v6.2-Ollama",
    page_icon="🧪",
    layout="wide",
...
# ============================================================================
# ENHANCED ONTOLOGY & NLP REASONING SYSTEM (Cu@Ag Core-Shell)
# ============================================================================
...
_SHORT_NAME_MAP: Dict[str, str] = {
    "core_shell_nanoparticle": "Cu@Ag",
    "cu_core": "Cu",
    "ag_shell": "Ag",
    "core_shell_interface": "Int",
    "galvanic_replacement": "GR",
    "seed_mediated_growth": "SMG",
    "wet_chemical_synthesis": "Wet",
    "plasmonic_property": "LSPR",
    "catalytic_activity": "Cat",
    "stability": "Stab",
    "morphology": "Morph",
    "cytotoxicity": "Cyto",
    "sintering": "Sint",
    ...
    "reaction_time": "Time",
    "precursor_concentration": "Conc",
    "uv_vis_spectroscopy": "UV-Vis",
...
class RelationshipType(Enum):
    ...
    PREVENTS = "prevents"    # Added for core-shell
...
EDGE_COLOR_REGISTRY: Dict[RelationshipType, str] = {
    ...
    RelationshipType.PREVENTS:          "#2E8B57",   # sea green
...
class DomainOntology:
    """Comprehensive ontology for Cu@Ag Core-Shell Nanoparticles."""
    ...
    def _build_ontology(self) -> None:
        ...
        self._add_concept("core_shell_nanoparticle", ConceptType.MATERIAL,
            synonyms={"cu@ag", "cu@ag core-shell", "core-shell nanoparticles", "bimetallic nanoparticles", "cuag"},
            definition="Nanoparticles with a copper core and a silver shell (Cu@Ag)")
        self._add_concept("cu_core", ConceptType.MATERIAL,
            synonyms={"copper core", "cu core", "cu nanoparticle"},
            definition="Copper core in core-shell nanostructures")
        self._add_concept("ag_shell", ConceptType.MATERIAL,
            synonyms={"silver shell", "ag shell", "silver coating"},
            definition="Silver shell encapsulating the copper core")
        self._add_concept("core_shell_interface", ConceptType.MATERIAL,
            synonyms={"interface", "core-shell boundary", "cu-ag interface"},
            definition="The interface region between the Cu core and Ag shell, crucial for charge transfer and stability")

        # --- Synthesis Processes ---
        self._add_concept("galvanic_replacement", ConceptType.PROCESS,
            synonyms={"galvanic exchange", "galvanic reaction", "galvanic displacement"},
            definition="Redox process where Ag+ is reduced by Cu, forming a silver shell")
        self._add_concept("seed_mediated_growth", ConceptType.PROCESS,
            synonyms={"seed growth", "layer-by-layer growth", "seed-mediated synthesis"},
            definition="Stepwise growth of a shell onto pre-formed seed nanoparticles")
        self._add_concept("wet_chemical_synthesis", ConceptType.PROCESS,
            synonyms={"wet chemistry", "solution synthesis", "colloidal synthesis"},
            definition="Synthesis in solution phase (e.g., co-precipitation, reduction)")

        # --- Properties ---
        self._add_concept("plasmonic_property", ConceptType.PROPERTY,
            synonyms={"lspr", "localized surface plasmon resonance", "plasmon resonance", "surface plasmon"},
            definition="Optical property arising from collective electron oscillation in the nanoparticle")
        self._add_concept("catalytic_activity", ConceptType.PROPERTY,
            synonyms={"catalytic performance", "catalytic efficiency", "turnover frequency"},
            definition="Activity of the catalyst for chemical reactions (e.g., CO2 reduction, organic transformations)")
        self._add_concept("stability", ConceptType.PROPERTY,
            synonyms={"colloidal stability", "oxidation resistance", "shelf life", "chemical stability"},
            definition="Resistance to aggregation, oxidation, or degradation over time")
        self._add_concept("morphology", ConceptType.PROPERTY,
            synonyms={"shape", "structure", "surface roughness", "surface topology"},
            definition="Physical shape and surface features of the nanoparticle")
        self._add_concept("cytotoxicity", ConceptType.PROPERTY,
            synonyms={"biocompatibility", "cell toxicity", "cell viability"},
            definition="Toxic effect on living cells, relevant for biomedical applications")

        # --- Phenomena ---
        self._add_concept("sintering", ConceptType.PHENOMENON,
            synonyms={"coalescence", "necking", "thermal fusion"},
            definition="Thermal fusion of nanoparticles leading to morphological changes")
        self._add_concept("oxidation", ConceptType.PHENOMENON,
            synonyms={"cu oxidation", "tarnishing", "corrosion"},
            definition="Oxidation of the copper core, often degrading properties")

        # --- Parameters ---
        self._add_concept("shell_thickness", ConceptType.PARAMETER,
            synonyms={"ag shell thickness", "shell thickness", "coating thickness"},
            definition="Thickness of the silver shell in nanometers")
        self._add_concept("core_size", ConceptType.PARAMETER,
            synonyms={"cu core diameter", "core diameter", "particle size"},
            definition="Diameter of the copper core in nanometers")
        self._add_concept("reaction_temperature", ConceptType.PARAMETER,
            synonyms={"temperature", "reaction temp", "synthesis temperature"},
            definition="Temperature during synthesis, affecting kinetics and morphology")
        self._add_concept("reaction_time", ConceptType.PARAMETER,
            synonyms={"time", "aging time", "growth time"},
            definition="Duration of the reaction, influencing shell thickness and uniformity")
        self._add_concept("precursor_concentration", ConceptType.PARAMETER,
            synonyms={"concentration", "ag+ concentration", "metal ion concentration"},
            definition="Concentration of Ag+ precursor in the reaction mixture")

        # --- Methods ---
        self._add_concept("uv_vis_spectroscopy", ConceptType.METHOD,
            synonyms={"uv-vis", "uv-vis absorption", "optical spectroscopy"},
            definition="UV-Vis spectroscopy for monitoring plasmon resonance and particle size")
        self._add_concept("transmission_electron_microscopy", ConceptType.METHOD,
            synonyms={"tem", "transmission electron microscopy", "hr-tem"},
            definition="Transmission electron microscopy for imaging morphology and core-shell structure")
        ...
        self._add_concept("eds", ConceptType.METHOD,
            ...
            definition="Energy-dispersive X-ray spectroscopy for elemental composition analysis")
    ...
    def _build_causal_chains(self) -> None:
        ...
        causal_chains = [
            # Synthesis → Structure
            ("galvanic_replacement", RelationshipType.CAUSES, "ag_shell", 0.90),
            ("seed_mediated_growth", RelationshipType.INFLUENCES, "shell_thickness", 0.80),
            ("wet_chemical_synthesis", RelationshipType.INFLUENCES, "core_size", 0.75),
            ("reaction_temperature", RelationshipType.INFLUENCES, "shell_thickness", 0.70),
            ("reaction_time", RelationshipType.INFLUENCES, "shell_thickness", 0.65),
            ("precursor_concentration", RelationshipType.INFLUENCES, "shell_thickness", 0.60),
            # Structure → Properties
            ("shell_thickness", RelationshipType.INFLUENCES, "plasmonic_property", 0.85),
            ("core_size", RelationshipType.INFLUENCES, "plasmonic_property", 0.80),
            ("morphology", RelationshipType.INFLUENCES, "plasmonic_property", 0.75),
            ("shell_thickness", RelationshipType.INFLUENCES, "catalytic_activity", 0.70),
            ("core_size", RelationshipType.INFLUENCES, "catalytic_activity", 0.65),
            ("morphology", RelationshipType.INFLUENCES, "catalytic_activity", 0.80),
            ("ag_shell", RelationshipType.STABILIZES, "cu_core", 0.90),
            ("shell_thickness", RelationshipType.INFLUENCES, "stability", 0.75),
            # Phenomena → Properties
            ("oxidation", RelationshipType.CAUSES, "stability", -0.85),
            ("sintering", RelationshipType.CAUSES, "morphology", -0.70),
            ("sintering", RelationshipType.CAUSES, "catalytic_activity", -0.60),
            # Interface effects
            ("core_shell_interface", RelationshipType.INFLUENCES, "catalytic_activity", 0.80),
            ("core_shell_interface", RelationshipType.INFLUENCES, "plasmonic_property", 0.70),
            # Methods ↔ Properties
            ("uv_vis_spectroscopy", RelationshipType.MEASURES, "plasmonic_property", 0.90),
            ("transmission_electron_microscopy", RelationshipType.MEASURES, "morphology", 0.95),
            ("xrd", RelationshipType.MEASURES, "core_size", 0.85),
            ("eds", RelationshipType.MEASURES, "shell_thickness", 0.80),
            # Process constraints
            ("galvanic_replacement", RelationshipType.DEPENDS_ON, "reaction_temperature", 0.70),
            ("galvanic_replacement", RelationshipType.DEPENDS_ON, "precursor_concentration", 0.75),
        ]
...
_HIERARCHY_PARENTS = {
    # --- Root domain ---
    "core_shell_nanoparticle": (None, 0),
    # --- Tier 1: Materials ---
    "cu_core":                ("Core-Shell Components", 1),
    "ag_shell":               ("Core-Shell Components", 1),
    "core_shell_interface":   ("Core-Shell Components", 1),
    # --- Tier 1: Synthesis ---
    "galvanic_replacement":   ("Synthesis Methods", 1),
    "seed_mediated_growth":   ("Synthesis Methods", 1),
    "wet_chemical_synthesis": ("Synthesis Methods", 1),
    # --- Tier 1: Properties ---
    "plasmonic_property":     ("Optical Properties", 1),
    "catalytic_activity":     ("Functional Properties", 1),
    "stability":              ("Functional Properties", 1),
    "morphology":             ("Structural Features", 1),
    "cytotoxicity":           ("Functional Properties", 1),
    # --- Tier 1: Phenomena ---
    "sintering":              ("Phenomena", 1),
    "oxidation":              ("Phenomena", 1),
    # --- Tier 1: Parameters ---
    "shell_thickness":        ("Structural Parameters", 1),
    "core_size":              ("Structural Parameters", 1),
    "reaction_temperature":   ("Reaction Conditions", 1),
    "reaction_time":          ("Reaction Conditions", 1),
    "precursor_concentration":("Reaction Conditions", 1),
    # --- Tier 1: Methods ---
    "uv_vis_spectroscopy":            ("Characterization Methods", 1),
    "transmission_electron_microscopy": ("Characterization Methods", 1),
...
def get_hierarchy_path(concept_key: str) -> List[str]:
    ...
    ["Cu@Ag Core-Shell", "Optical Properties", "Plasmonic Property"].
    """
    leaf = concept_key.replace("_", " ").title()
    entry = _HIERARCHY_PARENTS.get(concept_key)

    if entry is None or entry[0] is None:
        return ["Cu@Ag Core-Shell", leaf]

    parent_label = entry[0]
    return ["Cu@Ag Core-Shell", parent_label, leaf]
...
def build_sunburst_data(
    ...
    # --- Root node ---
    root_id = "Cu@Ag Core-Shell"
    ids.append(root_id)
    labels.append("Cu@Ag Core-Shell")
    values.append(0)
    parents.append("")
...
class AdvancedConceptResolver:
    ...
    def __init__(
        ...
        # v6.1 (Patch 3): both caches are now BOUNDED.
    ...
    def _context_disambiguation(self, text: str, context: str) -> Optional[str]:
        # Block 8: context disambiguation for Cu@Ag
        context_lower = context.lower()
        cs_indicators = [
            'plasmon', 'lspr', 'uv-vis', 'absorption', 'optical',
            'catalytic', 'activity', 'co2', 'reduction',
            'shell', 'thickness', 'core', 'size', 'diameter',
            'stability', 'oxidation', 'sintering', 'aggregation'
        ]
        if any(ind in context_lower for ind in cs_indicators):
            if 'plasmon' in text or 'lspr' in text or 'optical' in text:
                return "plasmonic_property"
            if 'catalytic' in text or 'activity' in text:
                return "catalytic_activity"
            if 'shell' in text and 'thickness' in text:
                return "shell_thickness"
            if 'core' in text and ('size' in text or 'diameter' in text):
                return "core_size"
            if 'stability' in text or 'oxidation' in text:
                return "stability"
        return None
...
# ============================================================================
# ENHANCED CONCEPT EXTRACTOR (Cu@Ag-focused)
# ============================================================================
class EnhancedConceptExtractor:
    ...
    def _build_extraction_patterns(self) -> None:
        # Cu@Ag specific patterns
        self.material_patterns = [
            r'\bcu@ag\b', r'\bcu@ag\s+core-shell\b', r'\bcore-shell\s+nanoparticle[s]?\b', r'\bbimetallic\s+nanoparticle[s]?\b',
            r'\bcu\s+core\b', r'\bag\s+shell\b', r'\bcore-shell\s+interface\b'
        ]
        self.process_patterns = [
            r'\bgalvanic\s+replacement\b', r'\bgalvanic\s+exchange\b', r'\bseed[-\s]mediated\s+growth\b',
            r'\bco[-\s]precipitation\b', r'\bwet[-\s]chemical\s+synthesis\b', r'\bencapsulation\b', r'\bcoating\b'
        ]
        self.property_patterns = [
            r'\bplasmon\s+resonance\b', r'\blspr\b', r'\blocalized\s+surface\s+plasmon\s+resonance\b',
            r'\bcatalytic\s+activity\b', r'\bcatalytic\s+performance\b', r'\bstability\b',
            r'\bcolloidal\s+stability\b', r'\boxidation\s+resistance\b', r'\bcytotoxicity\b',
            r'\bbiocompatibility\b', r'\bmorphology\b', r'\bshape\b', r'\bsurface\s+roughness\b'
        ]
        self.phenomena_patterns = [
            r'\bsintering\b', r'\bcoalescence\b', r'\boxidation\b', r'\bcu\s+oxidation\b', r'\btarnishing\b'
        ]
        self.param_patterns = [
            r'\bshell\s+thickness\b', r'\bcore\s+diameter\b', r'\bcore\s+size\b', r'\bag\s+shell\s+thickness\b',
            r'\btemperature\b', r'\breaction\s+time\b', r'\bprecursor\s+concentration\b', r'\bph\b'
        ]
        self.method_patterns = [
            r'\buv[-\s]vis\s+spectroscopy\b', r'\btransmission\s+electron\s+microscopy\b', r'\btem\b',
            r'\bscanning\s+electron\s+microscopy\b', r'\bsem\b', r'\bxrd\b', r'\bx[-\s]ray\s+diffraction\b',
            r'\benergy[-\s]dispersive\s+x[-\s]ray\s+spectroscopy\b', r'\beds\b', r'\bcyclic\s+voltammetry\b', r'\bcv\b'
        ]
    ...
    def extract_from_text(self, text: str, doc_id: int = 0, allowed_concepts: Optional[Set[str]] = None) -> List[str]:
        ...
        # 2. Localized Context Window Extraction
        ...
        # 3. Batch resolve remaining raw concepts (limit to 50)
        raw_concepts = set()
        for c in concepts:
...
# ============================================================================
# Cu@Ag CORE-SHELL KEYWORDS
# ============================================================================
MATERIAL_KEYWORDS = ["cu@ag", "cu@ag core-shell", "core-shell nanoparticle", "bimetallic nanoparticle", "cu core", "ag shell", "copper core", "silver shell"]
PROPERTY_KEYWORDS = ["plasmon resonance", "lspr", "localized surface plasmon resonance", "catalytic activity", "catalytic performance", "stability", "colloidal stability", "oxidation resistance", "cytotoxicity", "biocompatibility", "morphology", "shape"]
PHENOMENON_KEYWORDS = ["sintering", "coalescence", "oxidation", "cu oxidation", "tarnishing"]
PROCESS_KEYWORDS = ["galvanic replacement", "galvanic exchange", "seed-mediated growth", "co-precipitation", "wet-chemical synthesis", "encapsulation", "coating"]
PARAM_KEYWORDS = ["shell thickness", "core diameter", "core size", "ag shell thickness", "temperature", "reaction time", "precursor concentration", "ph"]
METHOD_KEYWORDS = ["uv-vis spectroscopy", "transmission electron microscopy", "tem", "scanning electron microscopy", "sem", "xrd", "x-ray diffraction", "energy-dispersive x-ray spectroscopy", "eds", "cyclic voltammetry", "cv"]

ALL_DOMAIN_KEYWORDS = (MATERIAL_KEYWORDS + PROPERTY_KEYWORDS + PHENOMENON_KEYWORDS + PROCESS_KEYWORDS + PARAM_KEYWORDS + METHOD_KEYWORDS)

CS_PATTERNS = [
    r'\bcu@ag\b', r'\bcu@ag\s+core-shell\b', r'\bcore-shell\s+nanoparticle[s]?\b', r'\bbimetallic\s+nanoparticle[s]?\b', 
    r'\bcu\s+core\b', r'\bag\s+shell\b', r'\bgalvanic\s+replacement\b', r'\bgalvanic\s+exchange\b',
    r'\bseed[-\s]mediated\s+growth\b', r'\bco[-\s]precipitation\b', r'\bwet[-\s]chemical\s+synthesis\b', 
    r'\bencapsulation\b', r'\bcoating\b', r'\bplasmon\s+resonance\b', r'\blspr\b',
    r'\bcatalytic\s+activity\b', r'\bcatalytic\s+performance\b', r'\bstability\b', r'\bcolloidal\s+stability\b', 
    r'\boxidation\s+resistance\b', r'\bcytotoxicity\b', r'\bbiocompatibility\b', r'\bmorphology\b', r'\bshape\b', 
    r'\bsurface\s+roughness\b', r'\bsintering\b', r'\bcoalescence\b', r'\boxidation\b', r'\bcu\s+oxidation\b',
    r'\bshell\s+thickness\b', r'\bcore\s+diameter\b', r'\bcore\s+size\b', r'\bag\s+shell\s+thickness\b',
    r'\btemperature\b', r'\breaction\s+time\b', r'\bprecursor\s+concentration\b', r'\bph\b',
    r'\buv[-\s]vis\s+spectroscopy\b', r'\btransmission\s+electron\s+microscopy\b', r'\btem\b',
    r'\bscanning\s+electron\s+microscopy\b', r'\bsem\b', r'\bxrd\b', r'\bx[-\s]ray\s+diffraction\b',
...
CS_DESCRIPTOR_MAPPING = {
    r'cu@ag|core-shell nanoparticle|bimetallic nanoparticle|cu core|ag shell|core-shell interface': 'material',
    r'galvanic replacement|seed-mediated growth|co-precipitation|wet-chemical synthesis|encapsulation|coating': 'process',
    r'plasmon resonance|lspr|catalytic activity|stability|cytotoxicity|morphology|shape|surface roughness': 'property',
    r'sintering|coalescence|oxidation|cu oxidation|tarnishing': 'phenomenon',
    r'shell thickness|core diameter|core size|temperature|reaction time|precursor concentration|ph': 'parameter',
    r'uv-vis spectroscopy|transmission electron microscopy|tem|sem|xrd|x-ray diffraction|eds|cyclic voltammetry|cv': 'method',
    r'general': 'general'
}

def normalize_cs_concept(concept: str) -> str:
    concept = concept.lower().strip()
    concept = re.sub(r'\bcu@ag\b|\bcu@ag\s+core-shell\b', 'core_shell_nanoparticle', concept)
    concept = re.sub(r'\bcopper\s+core\b|\bcu\s+core\b', 'cu_core', concept)
    concept = re.sub(r'\bsilver\s+shell\b|\bag\s+shell\b', 'ag_shell', concept)
    concept = re.sub(r'\bgalvanic\s+replacement\b|\bgalvanic\s+exchange\b', 'galvanic_replacement', concept)
    concept = re.sub(r'\bseed[-\s]mediated\s+growth\b', 'seed_mediated_growth', concept)
    concept = re.sub(r'\bwet[-\s]chemical\s+synthesis\b', 'wet_chemical_synthesis', concept)
    concept = re.sub(r'\bplasmon\s+resonance\b|\blspr\b', 'plasmonic_property', concept)
    concept = re.sub(r'\bcatalytic\s+activity\b', 'catalytic_activity', concept)
    concept = re.sub(r'\bstability\b', 'stability', concept)
    concept = re.sub(r'\bmorphology\b', 'morphology', concept)
    concept = re.sub(r'\bcytotoxicity\b', 'cytotoxicity', concept)
    concept = re.sub(r'\bsintering\b', 'sintering', concept)
    concept = re.sub(r'\boxidation\b', 'oxidation', concept)
    concept = re.sub(r'\bshell\s+thickness\b', 'shell_thickness', concept)
    concept = re.sub(r'\bcore\s+size\b|\bcore\s+diameter\b', 'core_size', concept)
    concept = re.sub(r'\btemperature\b', 'reaction_temperature', concept)
    concept = re.sub(r'\breaction\s+time\b', 'reaction_time', concept)
    concept = re.sub(r'\bprecursor\s+concentration\b', 'precursor_concentration', concept)
    concept = re.sub(r'\buv[-\s]vis\s+spectroscopy\b', 'uv_vis_spectroscopy', concept)
    concept = re.sub(r'\btransmission\s+electron\s+microscopy\b', 'transmission_electron_microscopy', concept)
    concept = re.sub(r'\bxrd\b', 'xrd', concept)
    concept = re.sub(r'\benergy[-\s]dispersive\s+x[-\s]ray\s+spectroscopy\b', 'eds', concept)
    concept = re.sub(r'\bcore[-\s]shell\s+interface\b', 'core_shell_interface', concept)
    return concept

def is_valid_cs_concept(concept: str) -> bool:
    concept_lower = concept.lower()
    has_domain = any(kw.lower() in concept_lower for kw in ALL_DOMAIN_KEYWORDS)
    has_pattern = any(re.search(p, concept, re.I) for p in CS_PATTERNS)
    generic = {
        'study', 'analysis', 'effect', 'role', 'investigation', 'research',
        'method', 'approach', 'paper', 'work', 'using', 'based', 'novel',
        'nanoparticle', 'particle', 'nanomaterial', 'material', 'system',
        'sample', 'specimen', 'structure', 'surface'
    }
...
def extract_concepts_from_text(text: str) -> List[str]:
    ...
    noun_pattern = (
        r'\b(?:[a-z]+(?:[-\s]?[a-z]+){0,2}[-\s]?)?'
        r'(?:core|shell|plasmon|catalytic|stability|morphology|oxidation|sintering|thickness|size|temperature|time|concentration)\b'
    )
    matches = re.findall(noun_pattern, text, re.I)
    for m in matches:
        concept = m.lower().strip()
        if is_valid_cs_concept(concept):
            concepts.add(concept)
...
def extract_concepts_from_abstracts(
    ...
    for idx, row in df.iterrows():
        ...
        # Extract metrics for Cu@Ag literature
        thickness_matches = re.findall(r'(\d+(?:\.\d+)?)\s*(?:nm)', combined_text, re.I)
        if thickness_matches:
            metrics['shell_thickness_nm'] = [float(m) for m in thickness_matches]
        size_matches = re.findall(r'(\d+(?:\.\d+)?)\s*(?:nm|µm)', combined_text, re.I)
        if size_matches:
            metrics['core_size_nm'] = [float(m) for m in size_matches]
        temp_matches = re.findall(r'(\d+(?:\.\d+)?)\s*(?:°c|celsius|k)', combined_text, re.I)
        if temp_matches:
            metrics['temperature'] = [float(m) for m in temp_matches]
        time_matches = re.findall(r'(\d+(?:\.\d+)?)\s*(?:min|h|s)', combined_text, re.I)
        if time_matches:
            metrics['reaction_time'] = [float(m) for m in time_matches]
        concentration_matches = re.findall(r'(\d+(?:\.\d+)?)\s*(?:mM|M|mol/L)', combined_text, re.I)
        if concentration_matches:
            metrics['concentration'] = [float(m) for m in concentration_matches]
        all_metrics.append(metrics)
        concepts = extract_concepts_from_text(combined_text)
        normalized = [normalize_cs_concept(c) for c in concepts]
...
def normalize_and_filter_concepts(
    ...
    for doc_idx, concepts in enumerate(all_concepts):
        ...
        for c in concepts:
            if c not in seen_in_doc and is_valid_cs_concept(c):
                concept_counts[c] += 1
...
def abstract_concepts_to_categories(concepts: List[str]) -> Dict[str, str]:
    ...
    for concept in concepts:
        matched = False
        for pattern, category in CS_DESCRIPTOR_MAPPING.items():
            if re.search(pattern, concept, re.I):
                concept_to_abstract[concept] = category
                matched = True
                break
        if not matched:
            if any(re.search(p, concept, re.I) for p in [r'core', r'shell', r'cu@ag', r'ag']):
                concept_to_abstract[concept] = 'material'
            elif any(re.search(p, concept, re.I) for p in [r'galvanic', r'seed', r'synthesis', r'coating']):
                concept_to_abstract[concept] = 'process'
            elif any(re.search(p, concept, re.I) for p in [r'plasmon', r'catalytic', r'stability', r'morphology']):
                concept_to_abstract[concept] = 'property'
            elif any(re.search(p, concept, re.I) for p in [r'sintering', r'oxidation']):
                concept_to_abstract[concept] = 'phenomenon'
            elif any(re.search(p, concept, re.I) for p in [r'thickness', r'size', r'temperature']):
                concept_to_abstract[concept] = 'parameter'
            elif any(re.search(p, concept, re.I) for p in [r'uv-vis', r'tem', r'xrd', r'eds']):
...
def sample_edges_for_training(
    ...
    # Calculate the maximum possible non-edges to prevent infinite loops
    ...
    # Cap the target to what is mathematically possible
    ...
    # Added max_attempts to the fallback loop to guarantee it exits
    attempts = 0
    while len(neg_pairs) < target_negs and attempts < max_attempts:
        u_idx, v_idx = np.random.choice(n_nodes, 2, replace=False)
...
def train_gnn(
    ...
    # Force CPU device to match Streamlit Cloud
    ...
    # Move node features to CPU to be safe
    ...
    # Safely handle empty neg_pairs so it doesn't crash the decoder
    ...
    for epoch in range(epochs):
        ...
        # Only calculate neg_loss if we actually have negative pairs
...
def export_publication_figure(
    ...
    filename="cuag_graph_pub.png",
) -> bytes:
    try:
        pos = nx.spring_layout(nx_graph, seed=42, k=2.5, iterations=200)
        plt.figure(figsize=figsize, dpi=dpi)
        node_colors = [get_cs_category_color(n) for n in nx_graph.nodes()]
        node_sizes = [
        ...
        plt.title(
            "Cu@Ag Core-Shell Nanoparticle Concept Graph",
...
def generate_analysis_report(
    ...
    report: List[str] = []
    report.append("# Cu@Ag Core-Shell Nanoparticle Concept Graph Analysis Report")
    report.append(
    ...
    report.append("")
    report.append("---")
    report.append("*Report generated by Cu@Ag Core-Shell Nanoparticle Concept Graph v6.2*")
    return "\n".join(report)
...
# ============================================================================
def get_cs_category_color(concept: str, cmap_colors: Optional[List[str]] = None) -> str:
    if cmap_colors:
        return cmap_colors[hash(concept) % len(cmap_colors)]
    concept_lower = concept.lower()
    category = 'general'
    for pattern, cat in CS_DESCRIPTOR_MAPPING.items():
...
get_mpea_category_color = get_cs_category_color
...
def render_pyvis_graph(
    ...
    for i, node in enumerate(nx_graph.nodes()):
        ...
        color = get_cs_category_color(node, cmap_colors)
        degree = int(nx_graph.degree(node))
        
        original_label = node
        
        # 1) LABEL TEXT — single source of truth
        if label_mode == NodeLabelMode.SHORT_NAME:
            ...
            label = external_label_text or (
                get_hierarchy_label(node, "arrow") if node in _HIERARCHY_PARENTS
                else node.replace("_", " ").title())
        else:
            label = (get_hierarchy_label(node, "arrow") if node in _HIERARCHY_PARENTS
                     else node.replace("_", " ").title())

        # 2) Abbreviation ONLY for inside modes (never resurrects text in blank modes)
        ...
        # 3) FONT — honour every sidebar control
        if label_mode == NodeLabelMode.NO_NAME:
            font_dict = {'size': 0, 'color': 'rgba(0,0,0,0)', 'face': node_font_face}
        elif label_mode == NodeLabelMode.EXTERNAL_LABEL:
            font_dict = {'color': external_font_color,
                         'size': max(8, min(int(external_font_size), 60)),
                         'face': node_font_face,
                         'align': external_label_align,   # vis.js draws it OUTSIDE the circle
                         'vadjust': 0}
        else:  # inside modes now respect size + position + family
            font_dict = {'color': '#ffffff',
                         'size': max(8, min(int(node_label_size), 60)),
                         'face': node_font_face, 'bold': True,
                         'align': node_label_position}    # was previously ignored
    ...
    # Generate HTML directly in memory to avoid local file permission issues
    try:
    ...
    # Safely inject CSS (replace first occurrence only)
    ...
    if enable_node_highlight:
        ...
        (function() {
            ...
            // Safety timeout: stop checking after 15 seconds if CDN is blocked
    ...
    try:
        ...
        st.download_button(
            ...
            data=html_bytes,
            file_name="cuag_concept_graph.html",
            mime="text/html"
        )
...
_SUNBURST_CATEGORY_COLORS = {
    "Core-Shell Components":     "#E74C3C",
    "Synthesis Methods":         "#3498DB",
    "Optical Properties":        "#F1C40F",
    "Functional Properties":     "#2ECC71",
    "Structural Features":       "#E67E22",
    "Phenomena":                 "#9B59B6",
    "Structural Parameters":     "#1ABC9C",
    "Reaction Conditions":       "#F39C12",
    "Characterization Methods":  "#2980B9",
}
    ...
    if export_format == "GraphML":
        try:
            if include_metadata:
                ...
                nx_graph.graph['tool'] = 'CuAg-ConceptGraph'
            try:
                nx.write_graphml_lxml(nx_graph, "cuag_graph.graphml")
            except Exception:
                nx.write_graphml(nx_graph, "cuag_graph.graphml")
            with open("cuag_graph.graphml", "rb") as f:
                return f.read(), "application/graphml+xml", "cuag_graph.graphml"
        except Exception as e:
        ...
        return json_str.encode('utf-8'), "application/json", "cuag_graph_full.json"
    elif export_format == "JSON (Compact)":
        data = nx.node_link_data(nx_graph)
        json_str = json.dumps(data, indent=2, default=str)
        return json_str.encode('utf-8'), "application/json", "cuag_graph.json"
        ...
        return csv_df.to_csv(index=False).encode('utf-8'), "text/csv", "cuag_edges_enhanced.csv"
        ...
        return csv_df.to_csv(index=False).encode('utf-8'), "text/csv", "cuag_nodes_enhanced.csv"
    elif export_format == "PNG":
        try:
            pos = nx.spring_layout(nx_graph, seed=42)
            plt.figure(figsize=(14, 12), dpi=300)
            node_colors = [
                get_cs_category_color(n) for n in nx_graph.nodes()
            ]
            nx.draw(
            ...
            buf.seek(0)
            plt.close()
            return buf.read(), "image/png", "cuag_graph.png"
        except Exception as e:
        ...
        try:
            ...
            buf.seek(0)
            plt.close()
            return buf.read(), "image/svg+xml", "cuag_graph.svg"
        except Exception as e:
        ...
        try:
            if include_metadata:
                nx_graph.graph['created'] = datetime.now().isoformat()
                nx_graph.graph['version'] = '6.2'
            nx.write_gexf(nx_graph, "cuag_graph.gexf")
            with open("cuag_graph.gexf", "rb") as f:
                return f.read(), "application/xml", "cuag_graph.gexf"
        except Exception as e:
...
def render_reasoning_dashboard(
    ...
    synonym_examples = [
        ("cu@ag", "core_shell_nanoparticle"),
        ("silver shell thickness", "shell_thickness"),
        ("core size", "core_size"),
        ("galvanic replacement", "galvanic_replacement"),
    ]
...
def extract_doc_metrics(text: str) -> Dict[str, Any]:
    """Regex metric extraction for Cu@Ag core-shell literature."""
    metrics: Dict[str, Any] = {}
    
    thickness_matches = re.findall(r'(\d+(?:\.\d+)?)\s*(?:nm)', text, re.I)
    if thickness_matches:
        metrics['shell_thickness_nm'] = [float(m) for m in thickness_matches]
    
    size_matches = re.findall(r'(\d+(?:\.\d+)?)\s*(?:nm|µm)', text, re.I)
    if size_matches:
        metrics['core_size_nm'] = [float(m) for m in size_matches]
    ...
    time_matches = re.findall(r'(\d+(?:\.\d+)?)\s*(?:min|h|s)', text, re.I)
    if time_matches:
        metrics['reaction_time'] = [float(m) for m in time_matches]
    
    concentration_matches = re.findall(r'(\d+(?:\.\d+)?)\s*(?:mM|M|mol/L)', text, re.I)
    if concentration_matches:
        metrics['concentration'] = [float(m) for m in concentration_matches]
...
def run_batch_analysis(
    ...
    try:
        # Cap PyTorch threads so the Ollama server isn't starved
    ...
    if _is_query_focused:
        ...
        config["USE_SEMANTIC_CLUSTERING"] = False  # avoid merging whitelist concepts
        st.info(
    ...
    def _process_one_batch(batch_num: int) -> None:
        ...
        # ★ QUERY-FOCUSED: report whitelist hit rate per batch ★
    ...
    def _finalize() -> None:
        ...
        # ★ QUERY-FOCUSED: force-include whitelist concepts ★
        ...
        valid_concepts.sort(
            key=lambda c: (
                1 if c in _wl and _is_qf else 0,   # whitelist first
        ...
        # Guarantee every whitelist concept appears even if freq < min_freq
        if _is_qf and _wl:
            for c in _wl:
...
def render_sidebar() -> None:
    with st.sidebar:
        ...
        st.subheader("🔬 Cu@Ag Core-Shell Focus Areas")
        st.markdown("- **Materials:** Cu@Ag core-shell, Cu core, Ag shell, core-shell interface")
        st.markdown("- **Synthesis:** Galvanic replacement, Seed-mediated growth, Wet chemical synthesis")
        st.markdown("- **Properties:** Plasmonic (LSPR), Catalytic activity, Stability, Morphology, Cytotoxicity")
        st.markdown("- **Phenomena:** Sintering, Oxidation")
        st.markdown("- **Parameters:** Shell thickness, Core size, Temperature, Reaction time, Precursor concentration")
        st.markdown("- **Methods:** UV-Vis, TEM, XRD, EDS")
        st.subheader("🧠 NLP Reasoning Options")
        st.session_state['use_ontology'] = st.checkbox(
            "Use ontology-based resolution", value=True,
            help="Maps synonyms like 'Cu@Ag' to canonical concepts",
        )
        ...
        st.session_state['use_inference'] = st.checkbox(
            ...
            help="Infers synthesis→property chains even when not co-occurring",
        )
        ...
        with st.expander("Node & Label Settings"):
            st.markdown("##### 🏷️ Node Label Display")
            label_mode_choice = st.selectbox(
                "Label mode", options=list(LABEL_MODE_OPTIONS.keys()), index=0,
                key="pyvis_label_mode",
                help="Blank = empty interior (tooltip still works); External = text outside the node.",
            )
            ...
            if st.session_state['label_mode'] == NodeLabelMode.EXTERNAL_LABEL:
                ...
                st.session_state['external_label_text'] = st.text_input(
                    "Custom label text (blank = per-node name)", "", key="ext_label_text_input")
...
# ============================================================================
# 1. QUERY ANALYSIS DATA STRUCTURES (Cu@Ag Core-Shell)
# ============================================================================
class CSCoreProblem(Enum):
    SHELL_UNIFORMITY = "shell_uniformity"
    PLASMON_TUNABILITY = "plasmon_tunability"
    CATALYTIC_ACTIVITY_ENHANCEMENT = "catalytic_activity_enhancement"
    OXIDATION_RESISTANCE = "oxidation_resistance"
    SYNTHESIS_REPRODUCIBILITY = "synthesis_reproducibility"
    GENERAL = "general"
    MULTI_PROBLEM = "multi_problem"

@dataclass
class CSProblemDefinition:
    problem_id: CSCoreProblem
    title: str
...
# Pre-defined CS Problem Definitions
CS_PROBLEM_DEFINITIONS: Dict[CSCoreProblem, CSProblemDefinition] = {
    CSCoreProblem.SHELL_UNIFORMITY: CSProblemDefinition(
        problem_id=CSCoreProblem.SHELL_UNIFORMITY, title="Achieving Uniform Shell Thickness",
        scientific_description="Incomplete or non-uniform Ag shells lead to exposed Cu cores and oxidation, degrading properties.",
        root_cause="Kinetics of galvanic replacement and diffusion limitations cause uneven deposition.",
        key_concepts=["galvanic_replacement", "shell_thickness", "morphology", "oxidation"],
        key_relationships=[("galvanic_replacement", "CAUSES", "shell_thickness"), ("shell_thickness", "INFLUENCES", "oxidation")],
        solution_directions=["Optimize reaction temperature and precursor concentration", "Use slow injection of Ag+", "Apply surfactant-assisted growth"],
        relevant_materials=["core_shell_nanoparticle", "cu_core", "ag_shell"],
        relevant_phenomena=["galvanic_replacement", "oxidation"],
        relevant_properties=["morphology", "stability"],
        example_queries=["How can we achieve a uniform Ag shell on Cu nanoparticles?", "What factors affect the uniformity of the Ag shell?"],
        visualization_focus=["shell_thickness_distribution", "tem_images"]
    ),
    CSCoreProblem.PLASMON_TUNABILITY: CSProblemDefinition(
        problem_id=CSCoreProblem.PLASMON_TUNABILITY, title="Tuning LSPR Peak Position",
        scientific_description="The plasmon resonance of Cu@Ag is sensitive to core and shell dimensions, often modeled via Mie theory.",
        root_cause="Core size and shell thickness determine the dielectric environment and electron density.",
        key_concepts=["core_size", "shell_thickness", "plasmonic_property", "mie_theory"],
        key_relationships=[("core_size", "INFLUENCES", "plasmonic_property"), ("shell_thickness", "INFLUENCES", "plasmonic_property")],
        solution_directions=["Synthesize monodisperse cores", "Control shell thickness via reaction time", "Apply Mie theory modeling for optical prediction"],
        relevant_materials=["core_shell_nanoparticle", "cu_core", "ag_shell"],
        relevant_phenomena=[], relevant_properties=["plasmonic_property", "morphology"],
        example_queries=["How does Mie theory explain the LSPR tuning in Cu@Ag core-shell nanoparticles?", "What is the optimal core size for plasmonic applications?"],
        visualization_focus=["uv_vis_spectra", "size_series"]
    ),
    CSCoreProblem.CATALYTIC_ACTIVITY_ENHANCEMENT: CSProblemDefinition(
        problem_id=CSCoreProblem.CATALYTIC_ACTIVITY_ENHANCEMENT, title="Maximizing Catalytic Activity",
        scientific_description="Cu@Ag shows synergy for catalysis (e.g., CO2 reduction, organic transformations).",
        root_cause="Electronic and geometric effects at the core-shell interface enhance reactivity.",
        key_concepts=["catalytic_activity", "morphology", "core_shell_interface"],
        key_relationships=[("core_shell_interface", "INFLUENCES", "catalytic_activity"), ("morphology", "INFLUENCES", "catalytic_activity")],
        solution_directions=["Optimize shell thickness for maximum active sites", "Design high-surface-area morphologies", "Dope with additional metals"],
        relevant_materials=["core_shell_nanoparticle", "cu_core", "ag_shell"],
        relevant_phenomena=[], relevant_properties=["catalytic_activity", "morphology", "stability"],
        example_queries=["What is the optimal shell thickness for CO2 reduction?", "How does the core-shell interface affect catalytic performance?"],
        visualization_focus=["activity_heatmap", "structure_activity_plot"]
    ),
    CSCoreProblem.OXIDATION_RESISTANCE: CSProblemDefinition(
        problem_id=CSCoreProblem.OXIDATION_RESISTANCE, title="Preventing Cu Core Oxidation",
        scientific_description="Cu oxidizes easily, degrading optical and catalytic properties.",
        root_cause="Ag shell provides a barrier, but defects allow oxygen ingress.",
        key_concepts=["oxidation", "stability", "shell_thickness", "coating"],
        key_relationships=[("shell_thickness", "STABILIZES", "oxidation"), ("coating", "PREVENTS", "oxidation")],
        solution_directions=["Increase shell thickness", "Create a dense defect-free shell", "Apply a secondary protective layer (e.g., SiO2)"],
        relevant_materials=["core_shell_nanoparticle", "ag_shell"],
        relevant_phenomena=["oxidation"], relevant_properties=["stability", "catalytic_activity"],
        example_queries=["How does the Ag shell protect the Cu core from oxidation?", "What is the minimum shell thickness required to prevent oxidation?"],
        visualization_focus=["oxidation_rate_plot", "tem_after_exposure"]
    ),
    CSCoreProblem.SYNTHESIS_REPRODUCIBILITY: CSProblemDefinition(
        problem_id=CSCoreProblem.SYNTHESIS_REPRODUCIBILITY, title="Reproducible Synthesis",
        scientific_description="Batch-to-batch variations in size and shell thickness hinder applications.",
        root_cause="Sensitivity to reaction parameters (temperature, concentration, pH, mixing rate).",
        key_concepts=["synthesis_method", "core_size", "shell_thickness", "reaction_conditions"],
        key_relationships=[("reaction_temperature", "INFLUENCES", "core_size"), ("reaction_time", "INFLUENCES", "shell_thickness")],
        solution_directions=["Standardize synthesis protocols", "Use automated injection systems", "Perform statistical design of experiments (DoE)"],
        relevant_materials=["core_shell_nanoparticle"], relevant_phenomena=[],
        relevant_properties=["morphology", "shell_thickness", "core_size"],
        example_queries=["What reaction parameters most influence shell thickness?", "How can we improve batch-to-batch reproducibility?"],
        visualization_focus=["parameter_screening", "size_distribution_histogram"]
    ),
    CSCoreProblem.GENERAL: CSProblemDefinition(
        problem_id=CSCoreProblem.GENERAL, title="General Cu@Ag Inquiry", scientific_description="General inquiry about Cu@Ag core-shell nanoparticles.",
        root_cause="N/A", key_concepts=["core_shell_nanoparticle"], key_relationships=[], solution_directions=[],
        relevant_materials=[], relevant_phenomena=[], relevant_properties=[], example_queries=["What are Cu@Ag core-shell nanoparticles?"], visualization_focus=["general_overview"]
    ),
    CSCoreProblem.MULTI_PROBLEM: CSProblemDefinition(
        problem_id=CSCoreProblem.MULTI_PROBLEM, title="Multi-Problem Cu@Ag Inquiry", scientific_description="Inquiry spanning multiple core problems.",
...
class QueryAnalysisResult:
    ...
    primary_problem: CSCoreProblem
    secondary_problems: List[CSCoreProblem]
...
class FallbackAnalyzer(LLMQueryAnalyzer):
    PROBLEM_KEYWORDS = {
        CSCoreProblem.SHELL_UNIFORMITY: {"shell", "uniformity", "thickness", "coverage", "galvanic", "deposition", "incomplete"},
        CSCoreProblem.PLASMON_TUNABILITY: {"plasmon", "lspr", "optical", "tuning", "mie", "resonance", "peak"},
        CSCoreProblem.CATALYTIC_ACTIVITY_ENHANCEMENT: {"catalytic", "activity", "co2 reduction", "synergy", "interface"},
        CSCoreProblem.OXIDATION_RESISTANCE: {"oxidation", "oxidize", "tarnish", "stability", "protect", "barrier"},
        CSCoreProblem.SYNTHESIS_REPRODUCIBILITY: {"reproducib", "batch", "variation", "scale", "protocol", "standardize"},
    }
    ...
    def analyze_query(self, query: str, ontology: Any) -> QueryAnalysisResult:
        ...
        primary = max(problem_scores, key=problem_scores.get) if sum(problem_scores.values()) > 0 else CSCoreProblem.GENERAL
        ...
        inferred = []
        if primary != CSCoreProblem.GENERAL:
            pdef = CS_PROBLEM_DEFINITIONS[primary]
        ...
        priorities = {}
        pdef = CS_PROBLEM_DEFINITIONS.get(primary, CS_PROBLEM_DEFINITIONS[CSCoreProblem.GENERAL])
...
class OpenAIQueryAnalyzer(LLMQueryAnalyzer):
    ...
    def analyze_query(self, query: str, ontology: Any) -> QueryAnalysisResult:
        ...
        system_prompt = """You are an expert in Cu@Ag core-shell nanoparticle synthesis and characterization. Analyze the user's query and return ONLY valid JSON with:
        1. "primary_problem": One of: shell_uniformity, plasmon_tunability, catalytic_activity_enhancement, oxidation_resistance, synthesis_reproducibility, general, multi_problem
        ...
        try:
            ...
            problem_map = {p.value: p for p in CSCoreProblem}
            primary = problem_map.get(parsed.get("primary_problem", "general"), CSCoreProblem.GENERAL)
            ...
            return QueryAnalysisResult(
                ...
                visualization_focus=CS_PROBLEM_DEFINITIONS[primary].visualization_focus, reasoning_chain=parsed.get("reasoning_chain", ["LLM analysis completed"]), confidence=0.85
            )
        except Exception as e:
...
class LocalLLMQueryAnalyzer(LLMQueryAnalyzer):
    ...
    def analyze_query(self, query: str, ontology: Any) -> QueryAnalysisResult:
        ...
        prompt = (
            f"[INST] You are an expert in Cu@Ag core-shell nanoparticles. Analyze: '{query}'. "
        ...
        try:
            if self._is_ollama:
                # Extract actual model name (e.g., "qwen2.5:0.5b")
    ...
    def unload_model(self) -> None:
        ...
        if self._is_ollama:
            # Tell the Ollama server to release the model immediately
            try:
                ollama_model_name = self.model_name.split(":", 1)[1]
                payload = {
                    "model": ollama_model_name,
                    "prompt": "",           # minimal valid payload
                    "keep_alive": 0         # 0 = unload instantly
                }
                requests.post(
...
class DynamicOntologyExpander:
    ...
    def _create_bridge_concept(self, missing_concept: str, source_query: str, problem: CSCoreProblem) -> Optional[Dict]:
        bridge_name = f"query_bridge_{missing_concept.replace(' ', '_').lower()}"
        if bridge_name in self.ontology.concepts: return None
        pdef = CS_PROBLEM_DEFINITIONS.get(problem, CS_PROBLEM_DEFINITIONS[CSCoreProblem.GENERAL])
...
class GraphRAGAnswerGenerator:
    ...
    def generate_ground_response(self, query: str, analysis: QueryAnalysisResult, subgraph: nx.Graph, concept_abstract_map: Dict[str, List[int]], all_texts: Union[List[str], Dict[int, str]], max_docs_per_concept: int = 2) -> str:
        ...
        nl = chr(10)
        prompt = "You are an expert in Cu@Ag core-shell nanoparticles. Answer the user's query based *strictly* on the provided graph context and evidence snippets." + nl
        ...
        if evidence_snippets:
            ...
            prompt += "No direct text snippets found. Rely on your general knowledge of Cu@Ag but note the lack of specific retrieved context." + nl
        prompt += "Instructions:" + nl
        prompt += "1. Provide a direct, scientifically accurate answer (2-3 paragraphs)." + nl
        prompt += "2. Explicitly mention how the key concepts interact (e.g., causal chains like 'shell thickness influences plasmon resonance')." + nl
...
class QuerySessionManager:
    SESSION_KEY = "cuag_query_session"
    @classmethod
...
def render_llm_query_panel(ontology: Any, expander: DynamicOntologyExpander, full_graph: nx.Graph) -> Optional[QueryAnalysisResult]:
    ...
    example_queries = [q for pdef in CS_PROBLEM_DEFINITIONS.values() for q in pdef.example_queries[:1]]
    selected_example = st.sidebar.selectbox("Or select an example:", [""] + example_queries, key="example_query_select")
    query = st.sidebar.text_area("Your Cu@Ag question:", value=selected_example, height=100, key="llm_query_input", placeholder="e.g., How can we achieve a uniform Ag shell on Cu nanoparticles?")
...
def render_llm_qa_tab(analysis_data: Dict, ontology: Any):
    ...
    st.markdown("Ask a specific scientific question about Cu@Ag core-shell nanoparticles. The system will dynamically expand the ontology, extract a relevant subgraph, and generate a grounded answer using retrieved literature snippets.")
...
def main() -> None:
    st.title(
        "🧪 Cu@Ag Core-Shell Nanoparticle Concept Graph v6.2"
    )
    st.caption(
        "Multi-level reasoning concept graph for Cu@Ag core-shell nanostructures | "
        "Focus: Synthesis, Properties, and Performance Descriptors | "
        ...
        try:
            with status:
                ...
                def _process_single_row(idx, row, allowed_concepts=None):
                    ...
                    metrics: Dict[str, Any] = {}
                    # Cu@Ag metric extraction
                    thickness_matches = re.findall(
                        r'(\d+(?:\.\d+)?)\s*(?:nm)', text, re.I
                    )
                    if thickness_matches:
                        metrics['shell_thickness_nm'] = [float(m) for m in thickness_matches]
                    size_matches = re.findall(
                        r'(\d+(?:\.\d+)?)\s*(?:nm|µm)', text, re.I
                    )
                    if size_matches:
                        metrics['core_size_nm'] = [float(m) for m in size_matches]
                    ...
                    time_matches = re.findall(
                        r'(\d+(?:\.\d+)?)\s*(?:min|h|s)', text, re.I
                    )
                    if time_matches:
                        metrics['reaction_time'] = [float(m) for m in time_matches]
                    concentration_matches = re.findall(
                        r'(\d+(?:\.\d+)?)\s*(?:mM|M|mol/L)', text, re.I
                    )
                    if concentration_matches:
                        metrics['concentration'] = [float(m) for m in concentration_matches]
        ...
        with tabs[tab_idx]:
            ...
            if top_scores.empty:
                ...
                st.download_button(
                    ...
                    file_name="cuag_research_directions.csv", mime="text/csv",
                )
        ...
        with tabs[tab_idx]:
            ...
            if st.button("Generate Publication Figure"):
                ...
                if pub_bytes:
                    st.download_button(
                        ...
                        data=pub_bytes,
                        file_name="cuag_graph_publication.png",
                        mime="image/png",
                    )
            st.markdown("---")
            ...
            if st.button("Generate Markdown Report"):
                ...
                st.download_button(
                    ...
                    file_name="cuag_analysis_report.md",
                    mime="text/markdown",
                )
            ...
            st.download_button(
                ...
                data=csv_concepts,
                file_name="cuag_concepts_enhanced.csv", mime="text/csv",
            )
