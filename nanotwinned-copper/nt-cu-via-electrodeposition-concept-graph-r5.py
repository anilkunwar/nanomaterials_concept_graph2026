...
# -*- coding: utf-8 -*-
"""
Nanomaterials-ConceptGraph: Concept Graph Builder for Electrodeposited Nanotwinned Cu
==================================================================================
...
- Nanomaterials-focused domain filtering (electrodeposited nanotwinned Cu, twin boundaries, etc.)
...
Run: streamlit run nanomaterials_concept_graph_enhanced.py
...
# ==========================================
st.set_page_config(
    page_title="Nanomaterials-ConceptGraph: Electrodeposited Nanotwinned Cu Explorer",
    page_icon="🔬",
    layout="wide",
...
# ==========================================
# NANOTWINNED CU DOMAIN CONFIGURATION (ENHANCED)
# ==========================================
CORE_MATERIALS = [
...
# NEW: Reliability and Application domain
...
NANOMATERIALS_PATTERNS = [
    r'\b(?:nanotwinned\s+(?:copper|cu))\b',
    r'\b(?:nt\s+cu|copper\s+nanotwins?)\b',
    r'\b(?:electrodeposit(?:ed|ion)\s+(?:nanotwinned\s+)?(?:copper|cu))\b',
    r'\b(?:twin\s*(?:boundary|density|spacing|lamella|variant|migration|nucleation|thickness|fraction))\b',
    ...
    r'\b(?:grain\s*(?:boundary|size|refinement|growth|boundary\s*character|texture))\b',
    r'\b(?:yield\s*strength|tensile\s*strength|hardness|ductility|elongation)\s*(?:of|for)\s*(?:\d+(?:\.\d+)?\s*(?:mpa|gpa|hv|vickers))\b',
    r'\b(?:pulsed\s*(?:reverse\s*)?electrodeposition|dc\s*electrodeposition|galvanostatic|potentiostatic)\b',
    r'\b(?:tem|hrtem|stem|ebsd|xrd|apt|atom\s*probe)\b',
    r'\b(?:dft|md|molecular\s*dynamics|phase\s*field|finite\s*element|ddd|cpfem)\b',
    r'\b(?:\d+(?:\.\d+)?\s*(?:gpa|mpa|hv|vickers|nm|um|micrometer|angstrom|a))\b',
    r'\b(?:sfe|gsfe|crss|gnd|ssd|sft|eam|meam)\b',
    r'\b(?:hall\s*petch|inverse\s*hall\s*petch|taylor\s*relation|voigt|reuss|hill)\b',
    r'\b(?:electromigration|stress\s*migration|interconnect|tsv|damascene)\b',
    r'\b(?:kirkendall\s*void|cu-cu\s*bonding|hybrid\s*bonding|rdl|redistribution\s*layer)\b',
    r'\b(?:in\s*situ\s*tem|in\s*situ\s*hrtem|in\s*situ\s*ebsd)\b',
    r'\b(?:machine\s*learning\s*potential|neural\s*network\s*potential|active\s*learning)\b',
    r'\b(?:gelatin|methylene\s*blue|ctab|sh110|mps-peg|jgb|sps)\b'
]
...
def normalize_nanomaterials_term(concept: str) -> str:
    ...
    concept = re.sub(r'\bnt-cu\b', 'nanotwinned cu', concept)
    concept = re.sub(r'\bcopper\s+nanotwins?\b', 'nanotwinned cu', concept)
    concept = re.sub(r'\belectrodeposit(?:ed|ion)\s+(?:nanotwinned\s+)?(?:copper|cu)\b', 'electrodeposited nanotwinned cu', concept)
    ...
    concept = re.sub(r'\bpulsed\s*(?:reverse\s*)?electrodeposition\b', 'pulsed electrodeposition', concept)
    concept = re.sub(r'\bdc\s*electrodeposition\b', 'dc electrodeposition', concept)
    ...
    concept = re.sub(r'\bin\s*situ\s*tem\b', 'in situ tem', concept)
    concept = re.sub(r'\bin\s*situ\s*hrtem\b', 'in situ hrtem', concept)
    concept = re.sub(r'\bin\s*situ\s*ebsd\b', 'in situ ebsd', concept)
    concept = re.sub(r'\bkirkendall\s*void\b', 'kirkendall void', concept)
    concept = re.sub(r'\bcu-cu\s*bonding\b', 'cu-cu bonding', concept)
    concept = re.sub(r'\bhybrid\s*bonding\b', 'hybrid bonding', concept)
    concept = re.sub(r'\bredistribution\s*layer\b', 'rdl', concept)
    concept = re.sub(r'\bwinand\s*diagram\b', 'winand diagram', concept)
    concept = re.sub(r'\bgcw\s*model\b', 'gcw model', concept)
    return concept
    ...
    for keyword in ALL_DOMAIN_KEYWORDS:
        for match in re.finditer(r'\b' + re.escape(keyword) + r'\b', text_lower):
            ...
            for phrase in context_phrases:
                ...
                if is_valid_nanomaterials_concept(concept):
                    concepts.add(concept)
    material_prop_pattern = r'\b([A-Z][a-z]+(?:\d+(?:\.\d+)?)?(?:[\s\-][A-Z][a-z]?\d*)+)\b\s+(?:with|having|exhibiting|showing|demonstrating|achieving|reaching|delivering|providing|offering)\s+(?:a\s+)?([\d\.]+\s*(?:gpa|mpa|hv|nm|um|µm|angstrom|a|wh/kg|mah/g|%iacs|ms/m))\b'
...
def detect_cross_domain_bridges(nx_graph: nx.Graph, valid_concepts: List[str],
    ...
    for concept in valid_concepts:
        ...
        if len(unique_cats) < 2:
            continue
        # Bridge score: betweenness * number of unique categories connected
...
def analyze_network_motifs(nx_graph: nx.Graph) -> Dict[str, Any]:
    ...
    # Star motifs: nodes with high degree but low clustering
    try:
...
def export_publication_figure(nx_graph, valid_concepts, concept_abstract_map, 
                              ...
                              filename="nano_graph_pub.png") -> bytes:
    ...
    try:
        ...
        nx.draw(nx_graph, pos, 
                ...
                width=0.5,
                alpha=0.9)
        plt.title("Nanotwinned Cu Concept Graph", fontsize=14, fontweight='bold', pad=20)
        buf = io.BytesIO()
...
def generate_analysis_report(nx_graph, valid_concepts, concept_abstract_map, 
    ...
    report = []
    report.append("# Nanotwinned Cu Concept Graph Analysis Report")
    ...
    report.append("---")
    report.append("*Report generated by Nanomaterials-ConceptGraph*")
    return "\n".join(report)
...
def get_nanomaterials_category_color(concept: str, cmap_colors: Optional[List[str]] = None) -> str:
    ...
    concept_lower = concept.lower()
    if any(c in concept_lower for c in ['nanotwinned cu', 'nt cu', 'nanotwin', 'columnar twin']):
        return "#D32F2F"
    elif any(c in concept_lower for c in ['twin boundary', 'twin density', 'twin spacing', 'ctb', 'itb', 'twin fraction']):
        return "#E91E63"
        ...
        return "#FF9800"
    elif any(c in concept_lower for c in ['grain boundary', 'grain size', 'triple junction', 'csl', 'texture']):
        return "#795548"
        ...
        return "#FF5722"
    elif any(c in concept_lower for c in ['electrodeposition', 'electroplating', 'pulsed', 'dc electrodeposition', 'bath', 'electrolyte', 'additive', 'brightener', 'gelatin', 'winand']):
        return "#00BCD4"
        ...
        return "#3F51B5"
    elif any(c in concept_lower for c in ['dft', 'molecular dynamics', 'md', 'phase field', 'finite element', 'ddd', 'cpfem', 'eam', 'meam']):
        return "#4CAF50"
    elif any(c in concept_lower for c in ['machine learning', 'neural network', 'gnn', 'potential', 'active learning', 'deep learning']):
        return "#8E24AA"
    elif any(c in concept_lower for c in ['electrical conductivity', 'resistivity', 'thermal conductivity']):
        return "#FFC107"
    elif any(c in concept_lower for c in ['electromigration', 'stress migration', 'interconnect', 'tsv', 'damascene', 'reliability', 'kirkendall', 'cu-cu bonding', 'hybrid bonding', 'rdl']):
        return "#18FFFF"
...
def render_graph_pyvis(nx_graph, concept_abstract_map, physics_enabled=True,
    ...
    for u, v in nx_graph.edges():
        ...
        # "hover" mode: only tooltip, no static label (default)
    ...
    try:
        ...
        st.download_button("Download Interactive Graph (HTML)", data=html_bytes,
                          file_name="nanotwinned_cu_concept_graph.html", mime="text/html")
...
def render_sunburst_chart(labels, parents, values, cmap_name="viridis", label_size=11, width=800, height=600, theme=None, branchvalues="total"):
    ...
    fig.update_layout(
        title="<b>Electrodeposited Nanotwinned Cu Research Domain Hierarchy</b><br><i>Size = concept frequency</i>",
...
def render_sidebar():
    with st.sidebar:
        ...
        st.markdown("- Electrodeposited nanotwinned Cu (NT Cu)")
        st.markdown("- Twin boundaries (CTB/ITB, twin spacing, twin density)")
        st.markdown("- Electrodeposition processes (pulsed, DC, additives, bath composition)")
        st.markdown("- Mechanical properties (strength, ductility, hardness)")
        st.markdown("- Electrical & thermal properties (conductivity, electromigration)")
        st.markdown("- Characterization (TEM, EBSD, XRD, APT)")
        st.markdown("- Computational methods (MD, DFT, CPFEM, Phase Field)")
        st.markdown("- NEW: Reliability & Interconnect (TSV, Cu-Cu bonding, Kirkendall voids)")
        st.markdown("- NEW: Machine Learning & Neural Network Potentials")
...
# ==========================================
def main():
    st.title("Nanomaterials-ConceptGraph: Electrodeposited Nanotwinned Cu Explorer")
    st.caption("Large-corpus concept graph builder for electrodeposited nanotwinned copper | Optimized for twin boundaries, electrodeposition processes, and interconnect applications")
    render_sidebar()
        ...
        with export_tab:
            ...
            if st.button("Generate Publication Figure"):
                ...
                if pub_bytes:
                    st.download_button("Download Publication PNG", data=pub_bytes,
                                     file_name="nano_graph_publication.png", mime="image/png")
            ...
            if st.button("Generate Markdown Report"):
                ...
                st.download_button("Download Report (Markdown)", data=report.encode('utf-8'),
                                  file_name="nanotwinned_cu_analysis_report.md", mime="text/markdown")
