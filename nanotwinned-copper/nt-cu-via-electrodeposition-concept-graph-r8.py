...
# -*- coding: utf-8 -*-
"""
Nanotwinned Cu Quantitative Descriptor Graph v6.2 (nt-Cu Edition)
==================================================================
Multi-level reasoning concept graph for numerical/quantitative description
of electrodeposited nanotwinned copper (nt-Cu).
Focus: Mechanical, Microstructural, Synthesis, Reliability, Computational Descriptors.
...
replaced with those for Nanotwinned Cu quantitative descriptors.
...
DOMAIN: Nanotwinned Copper Quantitative Descriptors
- Core materials: nanotwinned Cu, electrodeposited nt-Cu
- Mechanical properties: yield strength, UTS, hardness, ductility, Young modulus
- Microstructure: twin boundaries, stacking faults, dislocations, deformation twinning
- Phenomena: Hall-Petch, strain hardening, electromigration, Kirkendall voids
- Synthesis: electrodeposition, additives (SPS, PEG, gelatin), pulse parameters
- Characterization: TEM, EBSD, XRD, APT
- Computational: DFT, MD, phase field, CPFEM, ML potentials
- Reliability: interconnects, Cu-Cu bonding, electromigration resistance
...
Run:
    streamlit run ntcu_concept_graph_v6.2_llm.py
...
class DomainOntology:
    ...
    def _build_ontology(self) -> None:
        ...
        self._add_concept("electrodeposited_nt_cu", ConceptType.PROCESS,
            synonyms={"electrodeposition", "electroplating", "pulsed electrodeposition", "dc electrodeposition", "pulse reverse electrodeposition", "galvanostatic deposition", "potentiostatic deposition"},
            definition="Electrochemical deposition process to produce nanotwinned Cu.")

        # === MECHANICAL PROPERTIES ===
        self._add_concept("yield_strength", ConceptType.PROPERTY, synonyms={"ys", "tensile yield strength", "flow stress"}, definition="Stress at which material begins to plastically deform.")
        self._add_concept("uts", ConceptType.PROPERTY, synonyms={"ultimate tensile strength", "tensile strength"}, definition="Maximum tensile stress a material can withstand.")
        self._add_concept("hardness", ConceptType.PROPERTY, synonyms={"nanoindentation hardness", "vickers hardness", "microhardness"}, definition="Resistance to localized plastic deformation.")
        self._add_concept("ductility", ConceptType.PROPERTY, synonyms={"elongation", "uniform elongation", "total elongation", "strain to failure"}, definition="Ability to deform plastically before fracture.")
        self._add_concept("young_modulus", ConceptType.PROPERTY, synonyms={"elastic modulus", "shear modulus", "bulk modulus", "poisson ratio"}, definition="Stiffness of the material in the elastic regime.")
        ...
        self._add_concept("hall_petch", ConceptType.PHENOMENON, synonyms={"grain boundary strengthening", "inverse hall petch", "twin boundary strengthening"}, definition="Strengthening due to grain size refinement or twin boundaries.")
        self._add_concept("strain_hardening", ConceptType.PHENOMENON, synonyms={"work hardening", "strain rate sensitivity", "srs", "dynamic strain aging"}, definition="Increase in strength due to plastic deformation.")
        ...
        # === CHARACTERIZATION ===
        self._add_concept("tem", ConceptType.METHOD, synonyms={"hrtem", "stem", "transmission electron microscopy", "high resolution tem", "scanning transmission electron microscopy", "aberration corrected tem", "atomic resolution"}, definition="Transmission electron microscopy for microstructure analysis.")
        self._add_concept("ebsd", ConceptType.METHOD, synonyms={"electron backscatter diffraction", "grain boundary character distribution", "texture", "crystallographic texture", "preferred orientation", "(111) texture", "fiber texture"}, definition="Electron backscatter diffraction for orientation mapping.")
        self._add_concept("xrd", ConceptType.METHOD, synonyms={"x-ray diffraction", "phase identification", "strain measurement"}, definition="X-ray diffraction for phase and texture analysis.")
        self._add_concept("apt", ConceptType.METHOD, synonyms={"atom probe tomography", "field ion microscopy", "fim"}, definition="Atom probe tomography for 3D elemental mapping.")

        # === COMPUTATIONAL METHODS ===
        self._add_concept("dft", ConceptType.MODEL, synonyms={"ab initio", "first principles", "density functional theory"}, definition="First-principles electronic structure calculations.")
        self._add_concept("md", ConceptType.MODEL, synonyms={"molecular dynamics", "classical md", "ab initio md", "embedded atom method", "eam", "meam"}, definition="Atomistic simulations using empirical potentials.")
        self._add_concept("phase_field", ConceptType.MODEL, synonyms={"phase field method", "pfm"}, definition="Mesoscale phase-field modeling of microstructure evolution.")
        self._add_concept("cpfem", ConceptType.MODEL, synonyms={"crystal plasticity finite element", "discrete dislocation dynamics", "ddd", "finite element method", "representative volume element", "rve"}, definition="Crystal plasticity finite element method for polycrystal deformation.")

        # === RELIABILITY & INTERCONNECTS ===
        self._add_concept("electromigration", ConceptType.PHENOMENON, synonyms={"em resistance", "stress migration", "mean time to failure", "mttf", "black's equation", "current crowding", "joule heating"}, definition="Mass transport due to high current densities.")
        self._add_concept("interconnect", ConceptType.MATERIAL, synonyms={"tsv", "through silicon via", "damascene", "dual damascene", "redistribution layer", "rdl", "cu pillar", "cu bump", "microbump", "fine pitch"}, definition="Interconnect structures in microelectronics.")
        self._add_concept("cu_cu_bonding", ConceptType.PROCESS, synonyms={"hybrid bonding", "direct bonding", "low temperature bonding", "surface diffusion creep"}, definition="Direct Cu-to-Cu bonding for 3D integration.")
        self._add_concept("kirkendall_void", ConceptType.PHENOMENON, synonyms={"void formation", "imc", "intermetallic compound", "cu6sn5", "cu3sn"}, definition="Void formation due to unequal diffusion fluxes.")

        # === MACHINE LEARNING ===
        self._add_concept("ml_potential", ConceptType.MODEL, synonyms={"machine learning potential", "neural network potential", "nnp", "active learning", "gaussian process", "deep learning", "graph neural network", "gnn", "transformer"}, definition="Machine-learned interatomic potentials.")
...
class AdvancedConceptResolver:
    ...
    def _context_disambiguation(
        ...
        ntcu_indicators = [
            'twin', 'boundary', 'spacing', 'density', 'stacking fault', 'dislocation',
            'strength', 'hardness', 'ductility', 'elastic modulus', 'hall petch',
            'electrodeposition', 'additive', 'current density', 'pulse',
            'tem', 'ebsd', 'xrd', 'apt', 'dft', 'md', 'phase field',
            'electromigration', 'interconnect', 'bonding', 'kirkendall'
        ]
        if any(ind in context_lower for ind in ntcu_indicators):
            if 'twin' in text and 'spacing' in text:
                return 'twin_boundary'
            if 'strength' in text or 'hardness' in text:
                return 'yield_strength' if 'yield' in text else 'hardness'
            if 'electrodeposition' in text or 'additive' in text:
                return 'electrodeposited_nt_cu'
            if 'tem' in text or 'ebsd' in text:
                return 'tem' if 'tem' in text else 'ebsd'
            if 'dft' in text or 'md' in text:
                return 'dft' if 'dft' in text else 'md'
            if 'electromigration' in text or 'interconnect' in text:
...
# ============================================================================
# ENHANCED CONCEPT EXTRACTOR (nt-Cu focused)
# ============================================================================
class EnhancedConceptExtractor:
    ...
    def _build_extraction_patterns(self) -> None:
        ...
        self.synthesis_patterns = [
            ...
            r'\bcurrent\s+density\b|\bpulse\s+(?:on|off)\s+time\b|\bduty\s+ratio\b|\bstirring\s+speed\b|\bbath\s+temperature\b',
        ]
        self.characterization_patterns = [
            r'\b(?:tem|hrtem|stem|ebsd|xrd|apt|atom\s+probe|sem|eds)\b',
        ]
        ...
        self.all_patterns = (
            self.material_patterns + self.mechanical_patterns +
            self.synthesis_patterns + self.characterization_patterns +
            self.computational_patterns + self.reliability_patterns
        )
...
CORE_MATERIALS = [
    ...
    "copper twin", "cu twin", "twin structured copper", "columnar twinned copper",
    "nanoscale twins copper", "nt-cu", "high density nanotwinned cu"
]
MECHANICAL_PROPERTIES = [
    ...
    "twinning dislocation", "shockley partial", "frank partial", "burgers vector",
    "deformation twinning", "mechanical twinning", "twin migration", "detwinning",
    "fracture toughness", "ductility", "elongation", "uniform elongation", "necking"
]
SYNTHESIS_METHODS = [
    "electrodeposition", "electroplating", "pulsed electrodeposition", "dc electrodeposition",
    "pulse reverse electrodeposition", "galvanostatic deposition", "potentiostatic deposition",
    ...
    "pulse off time", "duty ratio", "pulse frequency", "pulse on time", "stirring speed",
    "bath temperature", "cathodic overpotential", "winand diagram", "gcw model"
]
STRUCTURE_CHARACTERIZATION = [
    ...
    "aberration corrected tem", "atomic resolution", "energy dispersive x-ray spectroscopy", "eds",
    ...
    "coincidence site lattice", "csl", "sigma boundary", "sigma 3", "random boundary",
    ...
    "preferred orientation", "(111) texture", "fiber texture", "dislocation density", "in situ tem"
]
COMPUTATIONAL_METHODS = [
    ...
    "representative volume element", "rve", "molecular statics", "nudged elastic band", "neb",
    ...
    "machine learning", "neural network", "gnn", "cnn", "rnn", "transformer", "active learning"
]
FUNCTIONAL_PROPERTIES = [
    ...
    "stress migration", "sm", "corrosion resistance", "oxidation resistance",
    "interconnect", "through silicon via", "tsv", "flip chip", "wire bonding", "damascene",
    "dual damascene", "current crowding", "joule heating", "electrical reliability"
]
RELIABILITY_APPLICATIONS = [
    ...
    "kirkendall void", "redistribution layer", "rdl", "cu pillar", "cu bump", "microbump"
]
...
NANOMATERIALS_CATEGORY_MAPPING = {
    ...
    'annealing|recrystallization|grain\s*growth|oriented\s*attachment|coalescence|ripening|thermal': 'thermal_processing',
...
def normalize_nanotwinned_term(concept: str) -> str:
    ...
    concept = re.sub(r'\bnanotwinned\s+copper\b|\bnt\s+cu\b|\bnt-cu\b|\bcopper\s+nanotwins?\b', 'nanotwinned_cu', concept)
    concept = re.sub(r'\belectrodeposit(?:ed|ion)\s+(?:nanotwinned\s+)?(?:copper|cu)\b|\belectroplating\b', 'electrodeposited_nt_cu', concept)
    concept = re.sub(r'\byield\s*strength\b', 'yield_strength', concept)
    concept = re.sub(r'\bultimate\s+tensile\s+strength\b|\btensile\s+strength\b', 'uts', concept)
    ...
    concept = re.sub(r'\bcoherent\s+twin\s+boundary\b|\bincoherent\s+twin\s+boundary\b', 'twin_boundary', concept)
    ...
    concept = re.sub(r'\btransmission\s+electron\s+microscopy\b|\bhigh\s+resolution\s+tem\b', 'tem', concept)
    ...
    concept = re.sub(r'\bdensity\s+functional\s+theory\b|\bab\s+initio\b|\bfirst\s+principles\b', 'dft', concept)
    ...
    concept = re.sub(r'\bcu-cu\s+bonding\b|\bhybrid\s+bonding\b', 'cu_cu_bonding', concept)
    return concept
...
def extract_concepts_from_text(text: str) -> List[str]:
    ...
    noun_pattern = (
        r'\b(?:[a-z]+(?:[-\s]?[a-z]+){0,2}[-\s]?)?'
        r'(?:strength|hardness|ductility|modulus|twin|dislocation|fault|electrodeposition|additive|interconnect|electromigration)\b'
    )
...
def extract_concepts_from_abstracts(
    ...
    for idx, row in df.iterrows():
        ...
        metrics = extract_doc_metrics(combined_text)
    ...
    for concept in concepts:
        ...
        for pattern, category in NANOMATERIALS_CATEGORY_MAPPING.items():
            if re.search(pattern, concept, re.I):
                ...
                break
        if not matched:
            if any(re.search(p, concept, re.I) for p in [r'twin', r'boundary', r'stacking', r'dislocation']):
                concept_to_abstract[concept] = 'microstructure'
            elif any(re.search(p, concept, re.I) for p in [r'strength', r'hardness', r'ductility', r'modulus']):
                concept_to_abstract[concept] = 'mechanical_property'
            elif any(re.search(p, concept, re.I) for p in [r'electrodeposition', r'additive', r'current']):
                concept_to_abstract[concept] = 'synthesis'
            elif any(re.search(p, concept, re.I) for p in [r'tem', r'ebsd', r'xrd', r'apt']):
                concept_to_abstract[concept] = 'characterization'
            elif any(re.search(p, concept, re.I) for p in [r'dft', r'md', r'phase', r'cpfem']):
                concept_to_abstract[concept] = 'computational_method'
            elif any(re.search(p, concept, re.I) for p in [r'electromigration', r'interconnect', r'bonding']):
                concept_to_abstract[concept] = 'reliability_application'
            else:
...
def export_publication_figure(
    ...
    filename="ntcu_graph_pub.png",
) -> bytes:
    try:
        pos = nx.spring_layout(nx_graph, seed=42, k=2.5, iterations=200)
        plt.figure(figsize=figsize, dpi=dpi)
        node_colors = [get_cu_category_color(n) for n in nx_graph.nodes()]
        node_sizes = [
...
# ============================================================================
def get_cu_category_color(concept: str, cmap_colors: Optional[List[str]] = None) -> str:
    if cmap_colors:
    ...
    for pattern, cat in NANOMATERIALS_CATEGORY_MAPPING.items():
        if re.search(pattern, concept_lower):
            ...
            break
    color_map = {
        'nanotwinned_cu': '#E74C3C',   # red
        'electrodeposited_nt_cu': '#3498DB',  # blue
        'mechanical_property': '#2ECC71',     # green
        'microstructure': '#F39C12',          # orange
        'phenomena': '#9B59B6',               # purple
        'synthesis': '#1ABC9C',               # teal
        'characterization': '#E67E22',        # dark orange
        'computational_method': '#2980B9',    # dark blue
        'reliability_application': '#D35400', # burnt orange
        'general': '#95A5A6'                  # grey
    }
    return color_map.get(category, '#95A5A6')

# Alias for compatibility
get_mpea_category_color = get_cu_category_color
...
def render_pyvis_graph(
    ...
    for i, node in enumerate(nx_graph.nodes()):
        ...
        color = get_cu_category_color(node, cmap_colors)
    ...
    try:
        ...
        if use_abbreviated_labels and label_map:
            ...
            html_content = html_content.replace('</body>', f'<div id="ntcu-label-map-data" style="display:none;">{label_map_json}</div></body>')
        os.unlink(tmp_path)
    except Exception as e:
    ...
    .ntcu-legend {{ font-size: {node_legend_font_size}px !important; }}
            ...
            var checkExist = setInterval(function() {
                if (typeof network !== 'undefined' && network !== null && network.body && network.body.data) {
                    ...
                    (function initLabelMap() {
                        var hidden = document.getElementById('ntcu-label-map-data');
                    ...
                    function showEdgeInfoPanel(nodeId, connectedEdges) {
                        ...
                        html += '<button id="btn-short" onclick="window._ntcuSetLabelMode(\'short\')" style="padding:4px 10px;border:none;border-radius:6px;font-size:10px;font-weight:700;cursor:pointer;background:#D32F2F;color:white;">Short</button>';
                        html += '<button id="btn-full" onclick="window._ntcuSetLabelMode(\'full\')" style="padding:4px 10px;border:none;border-radius:6px;font-size:10px;font-weight:700;cursor:pointer;background:transparent;color:#64748b;">Full</button>';
                        html += '</div>';
                        ...
                        window._ntcuSetLabelMode = function(mode) {
    ...
    try:
        ...
        st.download_button(
            ...
            data=html_bytes,
            file_name="ntcu_concept_graph.html",
            mime="text/html"
        )
    ...
    if use_abbreviated_labels and label_map:
        ...
        for i, (short, full) in enumerate(sorted_legend):
            with cols[i % 4]:
                st.markdown(f"""<div class='ntcu-legend' style='padding:8px; border-radius:6px; background-color:{theme.get('tooltip_bg', '#f8fafc')}; border-left:4px solid {theme.get('highlight_bg', '#ff6b6b')}; margin-bottom:6px;'>
...
_SUNBURST_CATEGORY_COLORS = {
    "Core Materials":           "#E74C3C",
    "Mechanical Properties":    "#2ECC71",
    "Microstructure":           "#F39C12",
    "Phenomena":                "#9B59B6",
    "Synthesis":                "#1ABC9C",
    "Characterization":         "#E67E22",
    "Computational Methods":    "#2980B9",
    "Reliability & Interconnects": "#D35400",
}
    ...
    if export_format == "GraphML":
        try:
            if include_metadata:
                ...
                nx_graph.graph['tool'] = 'ntCu-ConceptGraph'
            try:
                nx.write_graphml_lxml(nx_graph, "ntcu_graph.graphml")
            except Exception:
                nx.write_graphml(nx_graph, "ntcu_graph.graphml")
            with open("ntcu_graph.graphml", "rb") as f:
                return f.read(), "application/graphml+xml", "ntcu_graph.graphml"
        except Exception as e:
        ...
        return json_str.encode('utf-8'), "application/json", "ntcu_graph_full.json"
    elif export_format == "JSON (Compact)":
        data = nx.node_link_data(nx_graph)
        json_str = json.dumps(data, indent=2, default=str)
        return json_str.encode('utf-8'), "application/json", "ntcu_graph.json"
        ...
        return csv_df.to_csv(index=False).encode('utf-8'), "text/csv", "ntcu_edges_enhanced.csv"
        ...
        return csv_df.to_csv(index=False).encode('utf-8'), "text/csv", "ntcu_nodes_enhanced.csv"
    elif export_format == "PNG":
        try:
            pos = nx.spring_layout(nx_graph, seed=42)
            plt.figure(figsize=(14, 12), dpi=300)
            node_colors = [
                get_cu_category_color(n) for n in nx_graph.nodes()
            ]
            nx.draw(
            ...
            buf.seek(0)
            plt.close()
            return buf.read(), "image/png", "ntcu_graph.png"
        except Exception as e:
        ...
        try:
            ...
            buf.seek(0)
            plt.close()
            return buf.read(), "image/svg+xml", "ntcu_graph.svg"
        except Exception as e:
        ...
        try:
            if include_metadata:
                nx_graph.graph['created'] = datetime.now().isoformat()
                nx_graph.graph['version'] = '6.2'
            nx.write_gexf(nx_graph, "ntcu_graph.gexf")
            with open("ntcu_graph.gexf", "rb") as f:
                return f.read(), "application/xml", "ntcu_graph.gexf"
        except Exception as e:
...
def extract_doc_metrics(text: str) -> Dict[str, Any]:
    """Extracts quantitative metrics typical for nt-Cu literature."""
    metrics: Dict[str, Any] = {}
...
def render_sidebar() -> None:
    with st.sidebar:
        ...
        st.markdown("- **Core Materials:** Nanotwinned Cu, Electrodeposited nt-Cu")
        st.markdown("- **Mechanical Properties:** Yield strength, UTS, Hardness, Ductility, Elastic modulus")
        st.markdown("- **Microstructure:** Twin boundaries, Stacking faults, Dislocations, Deformation twinning")
        st.markdown("- **Phenomena:** Hall-Petch, Strain hardening, Electromigration, Kirkendall voids")
        st.markdown("- **Synthesis:** Electrodeposition, Additives (SPS, PEG, gelatin), Pulse parameters")
        st.markdown("- **Characterization:** TEM, EBSD, XRD, APT")
        st.markdown("- **Computational Methods:** DFT, MD, Phase field, CPFEM, ML potentials")
        st.markdown("- **Reliability & Interconnects:** Electromigration resistance, Cu-Cu bonding")
...
class FallbackAnalyzer(LLMQueryAnalyzer):
    PROBLEM_KEYWORDS = {
        CuCoreProblem.SYNTHESIS_OPTIMIZATION: {"electrodeposition", "additive", "peg", "sps", "gelatin", "current density", "pulse"},
        CuCoreProblem.MICROSTRUCTURE_CONTROL: {"twin", "boundary", "spacing", "density", "stacking fault", "grain"},
        CuCoreProblem.MECHANICAL_PROPERTY_ENHANCEMENT: {"strength", "ductility", "hardness", "yield", "uts", "elongation"},
        CuCoreProblem.RELIABILITY_APPLICATION: {"electromigration", "interconnect", "tsv", "bonding", "reliability", "void"},
        CuCoreProblem.COMPUTATIONAL_MATERIALS: {"dft", "md", "simulation", "potential", "phase field", "cpfem"},
    }
...
class OpenAIQueryAnalyzer(LLMQueryAnalyzer):
    ...
    def analyze_query(self, query: str, ontology: Any) -> QueryAnalysisResult:
        ...
        system_prompt = """You are an expert in electrodeposited nanotwinned copper (nt-Cu) and materials science. Analyze the user's query and return ONLY valid JSON with:
...
class GraphRAGAnswerGenerator:
    ...
    def generate_ground_response(self, query: str, analysis: QueryAnalysisResult, subgraph: nx.Graph, concept_abstract_map: Dict[str, List[int]], all_texts: Union[List[str], Dict[int, str]], max_docs_per_concept: int = 2) -> str:
        ...
        nl = chr(10)
        prompt = "You are an expert in nanotwinned copper (nt-Cu) and materials science. Answer the user's query based *strictly* on the provided graph context and evidence snippets." + nl
        ...
        if evidence_snippets:
            ...
            prompt += "No direct text snippets found. Rely on your general nt-Cu knowledge but note the lack of specific retrieved context." + nl
...
class QuerySessionManager:
    SESSION_KEY = "ntcu_query_session"
    @classmethod
...
def render_llm_query_panel(ontology: Any, expander: DynamicOntologyExpander, full_graph: nx.Graph) -> Optional[QueryAnalysisResult]:
    ...
    query = st.sidebar.text_area("Your Cu question:", value=selected_example, height=100, key="llm_query_input", placeholder="e.g., How does SPS influence twin boundary formation?")
...
def render_llm_qa_tab(analysis_data: Dict, ontology: Any):
    ...
    with col1: query = st.text_input("Enter your Cu research question:", placeholder="e.g., How does SPS influence twin boundary formation?")
...
def main() -> None:
    ...
    st.caption(
        "Multi-level reasoning concept graph for electrodeposited nanotwinned copper (nt-Cu) | "
        "Focus: Mechanical, Microstructural, Synthesis, Reliability, Computational Descriptors | "
    ...
    if st.session_state.analysis_data is not None:
        ...
        tab_names.append("🤖 LLM-Guided Q&A")  # <--- ADD THIS NEW TAB
        ...
        with tabs[tab_idx]:
            ...
            if top_scores.empty:
                ...
                st.download_button(
                    ...
                    file_name="ntcu_research_directions.csv", mime="text/csv",
                )
        ...
        with tabs[tab_idx]:
            ...
            if st.button("Generate Publication Figure"):
                ...
                if pub_bytes:
                    st.download_button(
                        ...
                        data=pub_bytes,
                        file_name="ntcu_graph_publication.png",
                        mime="image/png",
                    )
            st.markdown("---")
            ...
            if st.button("Generate Markdown Report"):
                ...
                st.download_button(
                    ...
                    file_name="ntcu_analysis_report.md",
                    mime="text/markdown",
                )
            ...
            st.download_button(
                ...
                data=csv_concepts,
                file_name="ntcu_concepts_enhanced.csv", mime="text/csv",
            )
