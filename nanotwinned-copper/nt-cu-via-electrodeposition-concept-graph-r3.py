...
- NEW: Interactive graph editing (remove/merge/rename nodes, add edges, filter)
- NEW: Enhanced sunburst with category filter and branch value modes
- NEW: Concept timeline, co-occurrence heatmap, t-SNE, community detection,
...
# ==========================================
# NANOTWINNED CU DOMAIN CONFIGURATION
# ==========================================
CORE_MATERIALS = [
...
SYNTHESIS_METHODS = [
    ...
    "electrolyte", "copper sulfate", "cupric sulfate", "cus04", "sulfuric acid", "h2so4",
...
STRUCTURE_CHARACTERIZATION = [
    ...
    "preferred orientation", "(111) texture", "fiber texture",
...
NANOMATERIALS_CATEGORY_MAPPING = {
    r'nanotwinned\s*(?:copper|cu)|nt\s*cu|copper\s*nanotwin|columnar\s*twin': 'nanotwinned_cu',
    ...
    r'electrodeposition|electroplating|pulsed\s*(?:reverse\s*)?electrodeposition|dc\s*electrodeposition|galvanostatic|potentiostatic|bath|electrolyte|additive|brightener|leveler|suppressor': 'electrodeposition_process',
    r'annealing|recrystallization|grain\s*growth|oriented\s*attachment|coalescence|ripening|thermal': 'thermal_processing',
    r'tem|hrtem|stem|ebsd|xrd|apt|atom\s*probe|synchrotron|xas|exafs|sem': 'microscopy_characterization',
    ...
    r'electromigration|stress\s*migration|interconnect|tsv|damascene|reliability': 'reliability_application',
    ...
    for keyword in ALL_DOMAIN_KEYWORDS:
        for match in re.finditer(r'\b' + re.escape(keyword) + r'\b', text_lower):
            ...
            for phrase in context_phrases:
                ...
                if is_valid_nanomaterials_concept(concept):
                    concepts.add(concept)
    material_prop_pattern = r'\b([A-Z][a-z]+(?:\d+(?:\.\d+)?)?(?:[\s\-][A-Z][a-z]?\d*)+)\b\s+(?:with|having|exhibiting|showing|demonstrating|achieving|reaching|delivering|providing|offering)\s+(?:a\s+)?([\d\.]+\s*(?:gpa|mpa|hv|nm|um|µm|angstrom|å|wh/kg|mah/g|%iacs|ms/m))\b'
...
def abstract_concepts_to_categories(concepts: List[str]) -> Dict[str, str]:
    ...
    for concept in concepts:
        ...
        if not matched:
            ...
            elif any(re.search(p, concept, re.I) for p in [r'\belectrodeposit', r'\belectroplat', r'\bpulsed', r'\badditive']):
                concept_to_abstract[concept] = 'electrodeposition_process'
            elif any(re.search(p, concept, re.I) for p in [r'\btem', r'\bebsd', r'\bxrd', r'\bapt']):
...
def get_nanomaterials_category_color(concept: str, cmap_colors: Optional[List[str]] = None) -> str:
    ...
    if any(c in concept_lower for c in ['nanotwinned cu', 'nt cu', 'nanotwin', 'columnar twin']):
        ...
        return "#FF5722"
    elif any(c in concept_lower for c in ['electrodeposition', 'electroplating', 'pulsed', 'dc electrodeposition', 'bath', 'electrolyte', 'additive', 'brightener']):
        return "#00BCD4"
    elif any(c in concept_lower for c in ['annealing', 'recrystallization', 'grain growth', 'thermal']):
        return "#8BC34A"
    elif any(c in concept_lower for c in ['tem', 'hrtem', 'stem', 'ebsd', 'xrd', 'apt', 'synchrotron', 'sem']):
        return "#3F51B5"
        ...
        return "#FFC107"
    elif any(c in concept_lower for c in ['electromigration', 'stress migration', 'interconnect', 'tsv', 'damascene', 'reliability']):
        return "#18FFFF"
...
def apply_graph_edits(nx_graph, valid_concepts, concept_to_id, id_to_concept, concept_abstract_map,
    ...
    """
    Apply interactive edits to the graph: remove nodes, merge nodes, add edges, filter by degree/frequency.
    Returns updated graph, valid_concepts, concept_to_id, id_to_concept, concept_abstract_map.
    """
    edited = False
    ...
    if nodes_to_merge and merge_name and len(nodes_to_merge) >= 2:
        # Collect all edges from merged nodes
        merged_edges = {}
        merged_freq = 0
...
def render_concept_timeline(df_filtered, valid_concepts, concept_abstract_map, theme=None):
    """Plot concept frequency over time (if Year data exists)."""
    if theme is None:
        theme = THEME_PRESETS["Bright (Default)"]
    ...
    # Get top 10 concepts by frequency
...
def render_cooccurrence_heatmap(nx_graph, valid_concepts, concept_abstract_map, top_n=30, theme=None):
    """Render a co-occurrence heatmap among top concepts."""
    if theme is None:
        theme = THEME_PRESETS["Bright (Default)"]
...
def render_tsne_projection(valid_concepts, concept_abstract_map, embed_model, theme=None):
    """2D t-SNE projection of concept embeddings, colored by category."""
    if theme is None:
        theme = THEME_PRESETS["Bright (Default)"]
    if len(valid_concepts) < 5:
...
def render_community_detection(nx_graph, valid_concepts, concept_abstract_map, theme=None):
    """Visualize community detection with interactive coloring."""
    if theme is None:
        theme = THEME_PRESETS["Bright (Default)"]
    if len(nx_graph.nodes()) < 3:
...
def render_concept_growth(df_filtered, valid_concepts, concept_abstract_map, theme=None):
    """Bar chart showing concept growth rates (early vs recent years)."""
    if theme is None:
        theme = THEME_PRESETS["Bright (Default)"]
    if "Year" not in df_filtered.columns or df_filtered["Year"].isna().all():
...
def render_bubble_chart(nx_graph, valid_concepts, concept_abstract_map, distill_df, theme=None):
    """Bubble chart: degree vs frequency, size = distillation efficiency, color = category."""
    if theme is None:
        theme = THEME_PRESETS["Bright (Default)"]
    if len(valid_concepts) < 3:
