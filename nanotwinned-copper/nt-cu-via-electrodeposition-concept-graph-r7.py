...
# -*- coding: utf-8 -*-
"""
Nanotwinned Cu Quantitative Descriptor Graph v6.2 (Cu Edition)
====================================================================
Multi-level reasoning concept graph for numerical/quantitative description
of Electrodeposited Nanotwinned Copper.
Focus: Synthesis, Microstructure, Mechanical Properties, Reliability.

This is a TRUE architectural port of the SIB-ConceptGraph codebase,
preserving every memory-safe pattern, visualization pattern, and session-state
management pattern from the original SIB code. The domain ontology and extraction
patterns have been replaced with those for Nanotwinned Cu quantitative descriptors.
...
DOMAIN: Electrodeposited Nanotwinned Copper Quantitative Descriptors
- Core materials: nanotwinned Cu, electrodeposited nanotwinned Cu
- Twin boundaries: CTB, ITB, twin spacing, twin density, twin fraction
- Mechanical properties: yield strength, UTS, hardness, ductility, Hall-Petch
- Synthesis: pulsed electrodeposition, DC, additives (SPS, PEG, gelatin)
- Characterization: TEM, HRTEM, EBSD, XRD, APT
- Computational: MD, DFT, Phase field, CPFEM, Machine Learning potentials
- Reliability: electromigration, stress migration, Cu-Cu bonding, TSV, Kirkendall voids
...
Run:
    streamlit run nanotwinned_cu_concept_graph_v6.2.py
...
# ============================================================================
st.set_page_config(
    page_title="Nanotwinned Cu Quantitative Descriptor Graph v6.2",
    page_icon="🔬",
    layout="wide",
...
# ============================================================================
# ENHANCED ONTOLOGY & NLP REASONING SYSTEM (NANOTWINNED Cu)
# ============================================================================
...
class DomainOntology:
    """Comprehensive ontology for Nanotwinned Cu Quantitative Descriptors."""
    ...
    def _build_ontology(self) -> None:
        # === CORE MATERIALS ===
        self._add_concept("nanotwinned_cu", ConceptType.MATERIAL,
            synonyms={"nt cu", "nanotwinned copper", "nt-cu", "copper nanotwins", "columnar twinned cu", "electrodeposited nt cu", "copper twin", "nanoscale twins cu", "high density nanotwinned cu"},
            definition="Copper with nanoscale twin boundaries, typically produced by electrodeposition.")
        self._add_concept("electrodeposited_nt_cu", ConceptType.PROCESS,
            synonyms={"electrodeposition", "electroplating", "pulsed electrodeposition", "dc electrodeposition", "pulse reverse electrodeposition", "galvanostatic deposition", "potentiostatic deposition", "electrochemical deposition"},
            definition="Electrochemical deposition process to produce nanotwinned Cu.")

        # === MECHANICAL PROPERTIES ===
        self._add_concept("yield_strength", ConceptType.PROPERTY,
            synonyms={"ys", "tensile yield strength", "flow stress"},
            definition="Stress at which material begins to plastically deform.")
        self._add_concept("uts", ConceptType.PROPERTY,
            synonyms={"ultimate tensile strength", "tensile strength"},
            definition="Maximum tensile stress a material can withstand.")
        self._add_concept("hardness", ConceptType.PROPERTY,
            synonyms={"nanoindentation hardness", "vickers hardness", "microhardness"},
            definition="Resistance to localized plastic deformation.")
        self._add_concept("ductility", ConceptType.PROPERTY,
            synonyms={"elongation", "uniform elongation", "total elongation", "strain to failure"},
            definition="Ability to deform plastically before fracture.")
        self._add_concept("young_modulus", ConceptType.PROPERTY,
            synonyms={"elastic modulus", "shear modulus", "bulk modulus", "poisson ratio"},
            definition="Stiffness of the material in the elastic regime.")

        # === TWIN BOUNDARIES ===
        self._add_concept("twin_boundary", ConceptType.MICROSTRUCTURE,
            synonyms={"ctb", "coherent twin boundary", "itb", "incoherent twin boundary", "twin spacing", "twin density", "twin thickness", "twin fraction", "twin volume fraction", "twin lamella"},
            definition="Interface separating two twin-related crystal domains.")
        self._add_concept("stacking_fault", ConceptType.MICROSTRUCTURE,
            synonyms={"sfe", "gsfe", "shockley partial", "frank partial", "stacking fault energy", "generalized stacking fault energy"},
            definition="Planar defect in crystal stacking sequence.")

        # === DISLOCATION MECHANICS ===
        self._add_concept("dislocation", ConceptType.PHENOMENON,
            synonyms={"burgers vector", "slip system", "slip plane", "slip direction", "cross slip", "climb", "glide", "gnd", "ssd", "geometrically necessary dislocation", "statistically stored dislocation"},
            definition="Line defects responsible for plastic deformation.")
        self._add_concept("deformation_twinning", ConceptType.PHENOMENON,
            synonyms={"mechanical twinning", "growth twinning", "annealing twinning", "twin migration", "detwinning", "twin nucleation", "twin thickening"},
            definition="Formation of twins during deformation or processing.")

        # === HARDENING MECHANISMS ===
        self._add_concept("hall_petch", ConceptType.PHENOMENON,
            synonyms={"grain boundary strengthening", "inverse hall petch", "twin boundary strengthening"},
            definition="Strengthening due to grain size refinement or twin boundaries.")
        self._add_concept("strain_hardening", ConceptType.PHENOMENON,
            synonyms={"work hardening", "strain hardening", "strain rate sensitivity", "srs", "dynamic strain aging"},
            definition="Increase in strength due to plastic deformation.")

        # === SYNTHESIS PROCESSES ===
        self._add_concept("additive", ConceptType.PROCESS,
            synonyms={"brightener", "leveler", "suppressor", "polyethylene glycol", "peg", "chloride ion", "cl-", "bis-(3-sulfopropyl) disulfide", "sps", "janus green b", "jgb", "gelatin", "methylene blue", "ctab", "sh110", "mps-peg"},
            definition="Organic or inorganic additives in electrodeposition bath to control grain structure.")
        self._add_concept("electrodeposition_parameter", ConceptType.PARAMETER,
            synonyms={"current density", "deposition rate", "throwing power", "pulse off time", "duty ratio", "pulse frequency", "pulse on time", "stirring speed", "bath temperature", "cathodic overpotential"},
            definition="Processing parameters controlling electrodeposition.")

        # === CHARACTERIZATION ===
        self._add_concept("tem", ConceptType.METHOD,
            synonyms={"hrtem", "stem", "transmission electron microscopy", "high resolution tem", "scanning transmission electron microscopy", "aberration corrected tem", "atomic resolution"},
            definition="Transmission electron microscopy for microstructure analysis.")
        self._add_concept("ebsd", ConceptType.METHOD,
            synonyms={"electron backscatter diffraction", "grain boundary character distribution", "texture", "crystallographic texture", "preferred orientation", "(111) texture", "fiber texture"},
            definition="Electron backscatter diffraction for orientation mapping.")
        self._add_concept("xrd", ConceptType.METHOD,
            synonyms={"x-ray diffraction", "phase identification", "strain measurement"},
            definition="X-ray diffraction for phase and texture analysis.")
        self._add_concept("apt", ConceptType.METHOD,
            synonyms={"atom probe tomography", "field ion microscopy", "fim"},
            definition="Atom probe tomography for 3D elemental mapping.")

        # === COMPUTATIONAL METHODS ===
        self._add_concept("dft", ConceptType.MODEL,
            synonyms={"ab initio", "first principles", "density functional theory"},
            definition="First-principles electronic structure calculations.")
        self._add_concept("md", ConceptType.MODEL,
            synonyms={"molecular dynamics", "classical md", "ab initio md", "embedded atom method", "eam", "meam"},
            definition="Atomistic simulations using empirical potentials.")
        self._add_concept("phase_field", ConceptType.MODEL,
            synonyms={"phase field method", "pfm"},
            definition="Mesoscale phase-field modeling of microstructure evolution.")
        self._add_concept("cpfem", ConceptType.MODEL,
            synonyms={"crystal plasticity finite element", "discrete dislocation dynamics", "ddd", "finite element method", "representative volume element", "rve"},
            definition="Crystal plasticity finite element method for polycrystal deformation.")

        # === RELIABILITY & INTERCONNECTS ===
        self._add_concept("electromigration", ConceptType.PHENOMENON,
            synonyms={"em resistance", "stress migration", "mean time to failure", "mttf", "black's equation", "current crowding", "joule heating"},
            definition="Mass transport due to high current densities.")
        self._add_concept("interconnect", ConceptType.MATERIAL,
            synonyms={"tsv", "through silicon via", "damascene", "dual damascene", "redistribution layer", "rdl", "cu pillar", "cu bump", "microbump", "fine pitch"},
            definition="Interconnect structures in microelectronics.")
        self._add_concept("cu_cu_bonding", ConceptType.PROCESS,
            synonyms={"hybrid bonding", "direct bonding", "low temperature bonding", "surface diffusion creep"},
            definition="Direct Cu-to-Cu bonding for 3D integration.")
        self._add_concept("kirkendall_void", ConceptType.PHENOMENON,
            synonyms={"void formation", "imc", "intermetallic compound", "cu6sn5", "cu3sn"},
            definition="Void formation due to unequal diffusion fluxes.")

        # === MACHINE LEARNING ===
        self._add_concept("ml_potential", ConceptType.MODEL,
            synonyms={"machine learning potential", "neural network potential", "nnp", "active learning", "gaussian process", "deep learning", "graph neural network", "gnn", "transformer"},
            definition="Machine-learned interatomic potentials.")
    ...
    def _build_causal_chains(self) -> None:
        ...
        causal_chains = [
            ("nanotwinned_cu", RelationshipType.INFLUENCES, "yield_strength", 0.85),
            ("nanotwinned_cu", RelationshipType.INFLUENCES, "uts", 0.80),
            ("nanotwinned_cu", RelationshipType.INFLUENCES, "ductility", 0.70),
            ("twin_boundary", RelationshipType.INFLUENCES, "yield_strength", 0.90),
            ("twin_boundary", RelationshipType.INFLUENCES, "hardness", 0.85),
            ("twin_boundary", RelationshipType.INFLUENCES, "ductility", -0.70),
            ("dislocation", RelationshipType.CAUSES, "strain_hardening", 0.80),
            ("dislocation", RelationshipType.CAUSES, "yield_strength", 0.75),
            ("stacking_fault", RelationshipType.INFLUENCES, "deformation_twinning", 0.85),
            ("deformation_twinning", RelationshipType.CAUSES, "twin_boundary", 0.90),
            ("additive", RelationshipType.INFLUENCES, "twin_boundary", 0.80),
            ("additive", RelationshipType.INFLUENCES, "nanotwinned_cu", 0.85),
            ("electrodeposition_parameter", RelationshipType.INFLUENCES, "nanotwinned_cu", 0.90),
            ("electrodeposition_parameter", RelationshipType.INFLUENCES, "twin_boundary", 0.80),
            ("hall_petch", RelationshipType.CAUSES, "yield_strength", 0.85),
            ("hall_petch", RelationshipType.CAUSES, "hardness", 0.80),
            ("strain_hardening", RelationshipType.CAUSES, "uts", 0.75),
            ("strain_hardening", RelationshipType.CAUSES, "ductility", -0.70),
            ("tem", RelationshipType.DETECTS, "twin_boundary", 0.90),
            ("ebsd", RelationshipType.DETECTS, "twin_boundary", 0.85),
            ("xrd", RelationshipType.DETECTS, "nanotwinned_cu", 0.70),
            ("dft", RelationshipType.COMPUTES, "stacking_fault", 0.90),
            ("md", RelationshipType.SIMULATES, "dislocation", 0.85),
            ("md", RelationshipType.SIMULATES, "deformation_twinning", 0.80),
            ("phase_field", RelationshipType.SIMULATES, "twin_boundary", 0.75),
            ("cpfem", RelationshipType.SIMULATES, "yield_strength", 0.80),
            ("cpfem", RelationshipType.SIMULATES, "ductility", 0.75),
            ("electromigration", RelationshipType.CAUSES, "interconnect", -0.85),
            ("electromigration", RelationshipType.CAUSES, "kirkendall_void", 0.80),
            ("cu_cu_bonding", RelationshipType.FORMS, "interconnect", 0.90),
            ("kirkendall_void", RelationshipType.CAUSES, "interconnect", -0.85),
            ("ml_potential", RelationshipType.ENABLES, "dft", 0.70),
            ("ml_potential", RelationshipType.ENABLES, "md", 0.75),
            ("nanotwinned_cu", RelationshipType.HYPONYM, "copper", 1.0),
            ("electrodeposited_nt_cu", RelationshipType.HYPONYM, "electrodeposition", 1.0),
            ("twin_boundary", RelationshipType.HYPONYM, "grain_boundary", 0.9),
        ]
...
_HIERARCHY_PARENTS: Dict[str, Tuple[str, int]] = {
    # --- Root domain ---
    "nanotwinned_cu": (None, 0),
    "electrodeposited_nt_cu": (None, 0),

    # --- Tier 1: Core Materials ---
    "nanotwinned_cu": ("Core Materials", 1),

    # --- Tier 1: Mechanical Properties ---
    "yield_strength": ("Mechanical Properties", 1),
    "uts": ("Mechanical Properties", 1),
    "hardness": ("Mechanical Properties", 1),
    "ductility": ("Mechanical Properties", 1),
    "young_modulus": ("Mechanical Properties", 1),

    # --- Tier 1: Microstructure ---
    "twin_boundary": ("Microstructure", 1),
    "stacking_fault": ("Microstructure", 1),
    "dislocation": ("Microstructure", 1),
    "deformation_twinning": ("Microstructure", 1),

    # --- Tier 1: Phenomena ---
    "hall_petch": ("Phenomena", 1),
    "strain_hardening": ("Phenomena", 1),
    "electromigration": ("Phenomena", 1),
    "kirkendall_void": ("Phenomena", 1),

    # --- Tier 1: Synthesis ---
    "additive": ("Synthesis", 1),
    "electrodeposition_parameter": ("Synthesis", 1),
    "cu_cu_bonding": ("Synthesis", 1),

    # --- Tier 1: Characterization ---
    "tem": ("Characterization", 1),
    "ebsd": ("Characterization", 1),
    "xrd": ("Characterization", 1),
    "apt": ("Characterization", 1),

    # --- Tier 1: Computational ---
    "dft": ("Computational Methods", 1),
    "md": ("Computational Methods", 1),
    "phase_field": ("Computational Methods", 1),
    "cpfem": ("Computational Methods", 1),
    "ml_potential": ("Computational Methods", 1),

    # --- Tier 1: Reliability & Interconnect ---
    "interconnect": ("Reliability & Interconnects", 1),
}
...
def get_hierarchy_label(concept_key: str,
    ...
    ----------
    concept_key : str
        The canonical name key used in the ontology (e.g. "yield_strength").
    style : str
        "arrow"   → "Mechanical Properties → Yield Strength"
        "bracket" → "Mechanical Properties [Yield Strength]"
        "dot"     → "Mechanical Properties · Yield Strength"
...
def get_hierarchy_path(concept_key: str) -> List[str]:
    ...
    ["Nanotwinned Cu", "Mechanical Properties", "Yield Strength"].
    ...
    if entry is None or entry[0] is None:
        return ["Nanotwinned Cu", leaf]

    parent_label = entry[0]
    return ["Nanotwinned Cu", parent_label, leaf]
...
def build_sunburst_data(
    ...
    # --- Root node ---
    root_id = "Nanotwinned Cu"
    ids.append(root_id)
    labels.append("Nanotwinned Cu")
...
class AdvancedConceptResolver:
    ...
    def _context_disambiguation(
        ...
        mechanical_indicators = [
            'strength', 'hardness', 'ductility', 'stress', 'strain',
            'yield', 'tensile', 'twin', 'dislocation', 'grain'
        ]
        if any(ind in context_lower for ind in mechanical_indicators):
            if 'twin' in text or 'boundary' in text:
                return "twin_boundary"
            if 'strength' in text or 'yield' in text:
...
# ============================================================================
# ENHANCED CONCEPT EXTRACTOR (Cu-focused)
# ============================================================================
class EnhancedConceptExtractor:
    ...
    def _build_extraction_patterns(self) -> None:
        # Nanotwinned Cu specific patterns
        self.core_material_patterns = [
            r'\b(?:nanotwinned\s+(?:copper|cu)|nt\s+cu|nt-cu|copper\s+nanotwins?|columnar\s+twinned\s+cu)\b',
            r'\belectrodeposit(?:ed|ion)\s+(?:nanotwinned\s+)?(?:copper|cu)\b',
        ]
        self.twin_patterns = [
            r'\b(?:coherent\s*twin\s*boundary|ctb|incoherent\s*twin\s*boundary|itb)\b',
            r'\btwin\s*(?:spacing|density|thickness|fraction|volume\s*fraction|lamella)\b',
            r'\btwin\s*(?:migration|nucleation|detwinning|thickening)\b',
        ]
        self.stack_fault_patterns = [
            r'\b(?:stacking\s*fault|sfe|gsfe|shockley\s*partial|frank\s*partial)\b',
        ]
        self.dislocation_patterns = [
            r'\bdislocation\s*(?:density|nucleation|glide|climb|tangle|cell|arrangement)\b',
            r'\b(?:burgers\s*vector|slip\s*(?:system|plane|direction)|cross\s*slip|gnd|ssd)\b',
        ]
        self.mechanical_property_patterns = [
            r'\b(?:yield\s*strength|ultimate\s*tensile\s*strength|uts|tensile\s*strength)\b',
            r'\bhardness\b', r'\bductility\b', r'\belongation\b',
            r'\belastic\s*modulus\b', r'\byoung\s*modulus\b',
            r'\bhall\s*petch\b', r'\binverse\s*hall\s*petch\b',
        ]
        self.synthesis_patterns = [
            r'\belectrodeposition\b', r'\belectroplating\b',
            r'\bpulsed\s*(?:reverse\s*)?electrodeposition\b',
            r'\bdc\s*electrodeposition\b',
            r'\badditive\b', r'\bbrightener\b', r'\bleveler\b', r'\bsuppressor\b',
            r'\b(?:peg|polyethylene\s*glycol|sps|bis-(3-sulfopropyl)\s*disulfide|gelatin|methylene\s*blue|ctab|sh110|mps-peg|chloride\s*ion)\b',
        ]
        self.characterization_patterns = [
            r'\b(?:tem|hrtem|stem|transmission\s*electron\s*microscopy|high\s*resolution\s*tem)\b',
            r'\bebsd\b', r'\belectron\s*backscatter\s*diffraction\b',
            r'\bxrd\b', r'\bx-ray\s*diffraction\b',
            r'\bapt\b', r'\batom\s*probe\s*tomography\b',
            r'\bin\s*situ\s*(?:tem|hrtem|ebsd)\b',
        ]
        self.computational_patterns = [
            r'\b(?:dft|ab\s*initio|first\s*principles|density\s*functional\s*theory)\b',
            r'\b(?:md|molecular\s*dynamics|eam|meam)\b',
            r'\bphase\s*field\b',
            r'\b(?:cpfem|finite\s*element|crystal\s*plasticity|ddd)\b',
            r'\b(?:machine\s*learning\s*potential|neural\s*network\s*potential|active\s*learning|gnn)\b',
        ]
        self.reliability_patterns = [
            r'\belectromigration\b',
            r'\b(?:stress\s*migration|interconnect|tsv|damascene|rdl|cu\s*pillar|microbump)\b',
            r'\b(?:cu-cu\s*bonding|hybrid\s*bonding|low\s*temperature\s*bonding)\b',
            r'\b(?:kirkendall\s*void|intermetallic\s*compound|imc|cu6sn5|cu3sn)\b',
        ]
        self.all_patterns = (
            self.core_material_patterns +
            self.twin_patterns +
...
# ============================================================================
# NANOTWINNED Cu KEYWORDS & PATTERNS
# ============================================================================
CORE_MATERIALS = [
    "nanotwinned copper", "nanotwinned cu", "nt cu", "copper nanotwins",
    "nanotwinned copper film", "nanotwinned copper foil", "nanotwinned copper wire",
    "nanotwinned copper coating", "nanotwinned copper layer",
    "electrodeposited nanotwinned cu", "electrodeposited nt cu",
    "copper twin", "cu twin", "twin structured copper", "twin structured cu",
    "columnar twinned copper", "columnar twinned cu",
    "nanoscale twins copper", "nanoscale twins cu",
    "nt-cu", "nt-cu film", "nt-cu foil", "nt-cu interconnect",
    "high density nanotwinned cu", "high density nt cu",
    "ultrafine nanotwinned cu", "ultrafine nt cu"
]
...
MECHANICAL_PROPERTIES = [
    ...
    "tensile property", "compressive property", "shear property",
    "strain to failure", "work of fracture", "toughness"
]

SYNTHESIS_METHODS = [
    "electrodeposition", "electroplating", "pulsed electrodeposition", "dc electrodeposition",
    "pulse reverse electrodeposition", "periodic reverse electrodeposition", "pr electrodeposition",
    "galvanostatic deposition", "potentiostatic deposition", "electrochemical deposition",
    "mild electrodeposition", "high rate electrodeposition", "template assisted electrodeposition",
    "additive", "brightener", "leveler", "suppressor", "polyethylene glycol", "peg",
    "chloride ion", "cl-", "bis-(3-sulfopropyl) disulfide", "sps", "janus green b", "jgb",
    "current density", "deposition rate", "throwing power", "bath composition",
    "electrolyte", "copper sulfate", "cupric sulfate", "cuso4", "sulfuric acid", "h2so4",
    "annealing", "recrystallization", "grain growth", "secondary recrystallization",
    "abnormal grain growth", "thermal annealing", "electromigration",
    "magnetron sputtering", "sputtering", "dc sputtering", "rf sputtering",
    "physical vapor deposition", "pvd",
    "gelatin", "methylene blue", "ctab", "sh110", "mps-peg",
    "pulse off time", "duty ratio", "pulse frequency", "pulse on time",
    "stirring speed", "bath temperature", "cathodic overpotential",
    "winand diagram", "gcw model", "gelatin controlled winand"
]
...
STRUCTURE_CHARACTERIZATION = [
    ...
    "coincidence site lattice", "csl", "sigma boundary", "sigma 3", "sigma 9", "random boundary",
    "twin density", "twin spacing", "twin thickness", "twin volume fraction", "twin fraction",
    "twin lamella", "twin variant", "twin boundary migration", "twin boundary energy",
    "columnar grain", "columnar structure", "epitaxial growth", "texture", "crystallographic texture",
    "preferred orientation", "(111) texture", "fiber texture", "(220) texture",
    "surface morphology", "roughness", "cross section", "plan view",
    ...
    "stacking fault", "stacking fault tetrahedron", "sft", "microtwins", "nanotwins",
    "in situ tem", "in situ hrtem", "in situ ebsd"
]
...
COMPUTATIONAL_METHODS = [
    ...
    "crystal plasticity finite element", "representative volume element", "rve",
    ...
    "embedded atom method", "eam", "angular electron density", "meam",
    "finite difference", "cohesive zone model", "czm",
    "machine learning", "neural network", "gnn", "cnn", "rnn", "transformer",
    "active learning", "bayesian optimization", "gaussian process",
    "deep learning", "reinforcement learning", "graph neural network"
]

FUNCTIONAL_PROPERTIES = [
    "electrical conductivity", "resistivity", "sheet resistance", "contact resistance", "specific resistivity",
    "thermal conductivity", "thermal expansion", "specific heat", "thermal stability",
    "electromigration", "em resistance", "mean time to failure", "mttf", "black's equation",
    "stress migration", "sm", "time dependent dielectric breakdown", "tdbd",
    "corrosion resistance", "oxidation resistance", "electrochemical stability",
    "interconnect", "through silicon via", "tsv", "flip chip", "wire bonding", "damascene",
    "dual damascene", "print wiring board", "pwb", "printed circuit board", "pcb",
    "solder", "solder joint", "under bump metallization", "ubm",
    "conductive ink", "flexible electronics", "stretchable electronics", "wearable",
    "current crowding", "joule heating", "electrical reliability",
    "kirkendall void", "cu-cu bonding", "hybrid bonding", "redistribution layer", "rdl",
    "cu pillar", "cu bump", "microbump", "fine pitch"
]

RELIABILITY_APPLICATIONS = [
    "electromigration resistance", "thermal cycling", "thermomigration",
    "mean time to failure", "reliability", "failure analysis",
    "intermetallic compound", "imc", "cu6sn5", "cu3sn",
    "solder joint reliability", "ball grid array", "bga",
    "chip scale package", "csp", "wafer level packaging", "wlp",
    "3d integration", "3d ic", "fan out wafer level packaging", "fowlp",
    "system in package", "sip", "heterogeneous integration",
    "cu-cu direct bonding", "low temperature bonding", "surface diffusion creep"
]

ALL_DOMAIN_KEYWORDS = (CORE_MATERIALS + MECHANICAL_PROPERTIES + SYNTHESIS_METHODS +
                       STRUCTURE_CHARACTERIZATION + COMPUTATIONAL_METHODS +
                       FUNCTIONAL_PROPERTIES + RELIABILITY_APPLICATIONS)

NANOMATERIALS_PATTERNS = [
    r'\b(?:nanotwinned\s+(?:copper|cu)|nt\s+cu|copper\s+nanotwins?|columnar\s+twinned\s+cu)\b',
    r'\belectrodeposit(?:ed|ion)\s+(?:nanotwinned\s+)?(?:copper|cu)\b',
    r'\b(?:coherent\s*twin\s*boundary|ctb|incoherent\s*twin\s*boundary|itb)\b',
    r'\btwin\s*(?:spacing|density|thickness|fraction|volume\s*fraction|lamella|migration|nucleation|detwinning|thickening)\b',
    r'\b(?:stacking\s*fault|sfe|gsfe|shockley\s*partial|frank\s*partial)\b',
    r'\bdislocation\s*(?:density|nucleation|glide|climb|tangle|cell|arrangement|burgers\s*vector|slip\s*(?:system|plane|direction)|cross\s*slip|gnd|ssd)\b',
    r'\b(?:yield\s*strength|ultimate\s*tensile\s*strength|uts|tensile\s*strength|hardness|ductility|elongation|elastic\s*modulus|young\s*modulus|hall\s*petch|inverse\s*hall\s*petch|fracture\s*toughness)\b',
    r'\belectrodeposition\b|\belectroplating\b|\bpulsed\s*(?:reverse\s*)?electrodeposition\b|\bdc\s*electrodeposition\b',
    r'\badditive\b|\bbrightener\b|\bleveler\b|\bsuppressor\b|\b(?:peg|sps|gelatin|methylene\s*blue|ctab|sh110|mps-peg)\b',
    r'\b(?:tem|hrtem|stem|ebsd|xrd|apt)\b',
    r'\b(?:dft|ab\s*initio|md|phase\s*field|cpfem|ddd)\b',
    r'\b(?:machine\s*learning\s*potential|neural\s*network\s*potential|active\s*learning|gnn)\b',
    r'\belectromigration\b|\bstress\s*migration\b|\binterconnect\b|\btsv\b|\bdamascene\b|\brdl\b|\bcu-cu\s*bonding\b|\bhybrid\s*bonding\b|\bkirkendall\s*void\b',
]

NANOMATERIALS_CATEGORY_MAPPING = {
    r'nanotwinned\s*(?:copper|cu)|nt\s*cu|copper\s+nanotwin|columnar\s*twin|nt-cu': 'nanotwinned_cu',
    r'twin\s*(?:boundary|density|spacing|lamella|variant|migration|nucleation|thickening|detwinning|thickness|fraction)|ctb|itb': 'twin_boundary',
    r'stacking\s*fault|sfe|gsfe|shockley\s*partial|frank\s*partial': 'stacking_fault',
    r'dislocation|burgers\s*vector|slip\s*(?:system|plane|direction)|cross\s*slip|climb|glide|gnd|ssd': 'dislocation_mechanics',
    r'yield\s*strength|tensile\s*strength|uts|hardness|elastic\s*modulus|young\s*modulus|ductility|elongation|fracture\s*toughness': 'mechanical_property',
    r'grain\s*boundary|grain\s*size|triple\s*junction|csl|sigma\s*boundary|random\s*boundary|texture': 'grain_boundary',
    r'electrodeposition|electroplating|pulsed\s*(?:reverse\s*)?electrodeposition|dc\s*electrodeposition|galvanostatic|potentiostatic|bath|electrolyte|additive|brightener|leveler|suppressor|gelatin|winand': 'electrodeposition_process',
    r'annealing|recrystallization|grain\s*growth|oriented\s*attachment|coalescence|ripening|thermal': 'thermal_processing',
    r'tem|hrtem|stem|ebsd|xrd|apt|atom\s*probe|synchrotron|xas|exafs|sem|in\s*situ': 'microscopy_characterization',
    r'dft|ab\s*initio|first\s*principles|molecular\s*dynamics|md|phase\s*field|fem|ddd|cpfem|eam|meam': 'computational_method',
    r'machine\s*learning|neural\s*network|gnn|cnn|rnn|transformer|potential|active\s*learning|bayesian|deep\s*learning': 'machine_learning',
    r'electrical\s*conductivity|resistivity|thermal\s*conductivity|thermal\s*expansion': 'transport_property',
    r'electromigration|stress\s*migration|interconnect|tsv|damascene|reliability|kirkendall|cu-cu\s*bonding|hybrid\s*bonding|rdl|cu\s*pillar|microbump': 'reliability_application',
...
def is_valid_nanotwinned_concept(concept: str) -> bool:
...
def normalize_nanotwinned_term(concept: str) -> str:
    concept = concept.lower().strip()
    concept = re.sub(r'\bnanotwinned\s+copper\b', 'nanotwinned_cu', concept)
    concept = re.sub(r'\bnt\s+cu\b', 'nanotwinned_cu', concept)
    concept = re.sub(r'\bnt-cu\b', 'nanotwinned_cu', concept)
    concept = re.sub(r'\bcopper\s+nanotwins?\b', 'nanotwinned_cu', concept)
    concept = re.sub(r'\belectrodeposit(?:ed|ion)\s+(?:nanotwinned\s+)?(?:copper|cu)\b', 'electrodeposited_nt_cu', concept)
    concept = re.sub(r'\byield\s*strength\b', 'yield_strength', concept)
    concept = re.sub(r'\bultimate\s*tensile\s*strength\b', 'uts', concept)
    concept = re.sub(r'\btensile\s*strength\b', 'uts', concept)
    concept = re.sub(r'\belastic\s*modulus\b', 'young_modulus', concept)
    concept = re.sub(r'\bstacking\s*fault\s*energy\b', 'stacking_fault', concept)
    concept = re.sub(r'\bcoherent\s*twin\s*boundary\b', 'twin_boundary', concept)
    concept = re.sub(r'\bincoherent\s*twin\s*boundary\b', 'twin_boundary', concept)
    ...
    concept = re.sub(r'\belectrodeposition\b', 'electrodeposited_nt_cu', concept)
    concept = re.sub(r'\belectroplating\b', 'electrodeposited_nt_cu', concept)
    concept = re.sub(r'\bpulsed\s*(?:reverse\s*)?electrodeposition\b', 'electrodeposited_nt_cu', concept)
    concept = re.sub(r'\bdc\s*electrodeposition\b', 'electrodeposited_nt_cu', concept)
    concept = re.sub(r'\bembedded\s*atom\s*method\b', 'eam', concept)
    concept = re.sub(r'\bmodified\s*embedded\s*atom\s*method\b', 'meam', concept)
    ...
    concept = re.sub(r'\bab\s*initio\b', 'ab_initio', concept)
    concept = re.sub(r'\bfirst\s*principles\b', 'first_principles', concept)
    concept = re.sub(r'\bmolecular\s*dynamics\b', 'md', concept)
    concept = re.sub(r'\bphase\s*field\b', 'phase_field', concept)
    concept = re.sub(r'\bfinite\s*element\b', 'finite_element', concept)
    concept = re.sub(r'\bdiscrete\s*dislocation\s*dynamics\b', 'ddd', concept)
    concept = re.sub(r'\bcrystal\s*plasticity\s*finite\s*element\b', 'cpfem', concept)
    concept = re.sub(r'\bin\s*situ\s*tem\b', 'in_situ_tem', concept)
    concept = re.sub(r'\bin\s*situ\s*hrtem\b', 'in_situ_hrtem', concept)
    concept = re.sub(r'\bin\s*situ\s*ebsd\b', 'in_situ_ebsd', concept)
    concept = re.sub(r'\bkirkendall\s*void\b', 'kirkendall_void', concept)
    concept = re.sub(r'\bcu-cu\s*bonding\b', 'cu_cu_bonding', concept)
    concept = re.sub(r'\bhybrid\s*bonding\b', 'cu_cu_bonding', concept)
    concept = re.sub(r'\bredistribution\s*layer\b', 'rdl', concept)
    concept = re.sub(r'\bwinand\s*diagram\b', 'winand_diagram', concept)
    concept = re.sub(r'\bgcw\s*model\b', 'gcw_model', concept)
    return concept
...
def extract_concepts_from_text(text: str) -> List[str]:
    ...
    for m in matches:
        ...
        if is_valid_nanotwinned_concept(concept):
            concepts.add(concept)
    for keyword in ALL_DOMAIN_KEYWORDS:
        for match in re.finditer(r'\b' + re.escape(keyword) + r'\b', text_lower):
            ...
            for phrase in context_phrases:
                ...
                if is_valid_nanotwinned_concept(concept):
                    concepts.add(concept)
    material_prop_pattern = r'\b([A-Z][a-z]+(?:\d+(?:\.\d+)?)?(?:[\s\-][A-Z][a-z]?\d*)+)\b\s+(?:with|having|exhibiting|showing|demonstrating|achieving|reaching|delivering|providing|offering)\s+(?:a\s+)?([\d\.]+\s*(?:gpa|mpa|hv|nm|um|µm|angstrom|a|%iacs|ms/m))\b'
...
def normalize_and_filter_concepts(
    ...
    for doc_idx, concepts in enumerate(all_concepts):
        ...
        for c in concepts:
            if c not in seen_in_doc and is_valid_nanotwinned_concept(c):
                concept_counts[c] += 1
    ...
    for concept in concepts:
        ...
        if not matched:
            if any(re.search(p, concept, re.I) for p in [r'\bnanotwinned', r'\bnt\s*cu', r'\btwin']):
                concept_to_abstract[concept] = 'nanotwinned_cu'
            elif any(re.search(p, concept, re.I) for p in [r'\belectrodeposit', r'\belectroplat', r'\bpulsed', r'\badditive', r'\bgelatin', r'\bwinand']):
                concept_to_abstract[concept] = 'electrodeposition_process'
            elif any(re.search(p, concept, re.I) for p in [r'\btem', r'\bebsd', r'\bxrd', r'\bapt', r'\bin\s*situ']):
                concept_to_abstract[concept] = 'microscopy_characterization'
            elif any(re.search(p, concept, re.I) for p in [r'\bdft', r'\bmd', r'\bmolecular\s*dynamics', r'\bphase\s*field']):
                concept_to_abstract[concept] = 'computational_method'
            elif any(re.search(p, concept, re.I) for p in [r'\bmachine\s*learning', r'\bneural\s*network', r'\bgnn', r'\bdeep\s*learning']):
                concept_to_abstract[concept] = 'machine_learning'
            else:
...
def export_publication_figure(
    ...
    filename="nt_cu_graph_pub.png",
) -> bytes:
    try:
        pos = nx.spring_layout(nx_graph, seed=42, k=2.5, iterations=200)
        plt.figure(figsize=figsize, dpi=dpi)
        node_colors = [get_nanotwinned_category_color(n) for n in nx_graph.nodes()]
        node_sizes = [
        ...
        plt.title(
            "Nanotwinned Cu Quantitative Descriptor Graph",
...
def generate_analysis_report(
    ...
    report: List[str] = []
    report.append("# Nanotwinned Cu Quantitative Descriptor Graph Analysis Report")
    report.append(
    ...
    report.append("")
    report.append("---")
    report.append("*Report generated by Nanotwinned Cu Quantitative Descriptor Graph v6.2*")
    return "\n".join(report)
...
# ============================================================================
def get_nanotwinned_category_color(
    ...
    concept_lower = concept.lower()
    category = 'general'
    for pattern, cat in NANOMATERIALS_CATEGORY_MAPPING.items():
        if re.search(pattern, concept_lower):
            category = cat
            break
    color_map = {
        'nanotwinned_cu': '#D32F2F',
        'twin_boundary': '#E91E63',
        'stacking_fault': '#9C27B0',
        'dislocation_mechanics': '#FF9800',
        'mechanical_property': '#F44336',
        'grain_boundary': '#795548',
        'electrodeposition_process': '#00BCD4',
        'thermal_processing': '#8BC34A',
        'microscopy_characterization': '#3F51B5',
        'computational_method': '#4CAF50',
        'machine_learning': '#8E24AA',
        'transport_property': '#FFC107',
        'reliability_application': '#18FFFF',
        'corrosion_stability': '#FFEB3B',
        'damage_mechanics': '#FF5722',
        'deformation_instability': '#F06292',
        'general': '#9E9E9E',
    }
...
def render_pyvis_graph(
    ...
    for i, node in enumerate(nx_graph.nodes()):
        ...
        color = get_nanotwinned_category_color(node, cmap_colors)
    ...
    /* FIX: Explicitly prevent truncation in the edge info panel header */
    ...
    try:
        ...
        st.download_button(
            ...
            data=html_bytes,
            file_name="nt_cu_concept_graph.html",
            mime="text/html"
        )
...
_SUNBURST_CATEGORY_COLORS = {
    "Core Materials":                 "#D32F2F",
    "Mechanical Properties":          "#F44336",
    "Microstructure":                 "#E91E63",
    "Phenomena":                      "#FF9800",
    "Synthesis":                      "#00BCD4",
    "Characterization":               "#3F51B5",
    "Computational Methods":          "#4CAF50",
    "Reliability & Interconnects":    "#18FFFF",
}
    ...
    if export_format == "GraphML":
        try:
            if include_metadata:
                ...
                nx_graph.graph['tool'] = 'NanotwinnedCu-ConceptGraph'
            try:
                nx.write_graphml_lxml(nx_graph, "nt_cu_graph.graphml")
            except Exception:
                nx.write_graphml(nx_graph, "nt_cu_graph.graphml")
            with open("nt_cu_graph.graphml", "rb") as f:
                return f.read(), "application/graphml+xml", "nt_cu_graph.graphml"
        except Exception as e:
        ...
        if include_metadata:
            data['metadata'] = {
                ...
                'version': '6.2',
                'tool': 'NanotwinnedCu-ConceptGraph',
        ...
        return json_str.encode('utf-8'), "application/json", "nt_cu_graph_full.json"
    elif export_format == "JSON (Compact)":
        data = nx.node_link_data(nx_graph)
        json_str = json.dumps(data, indent=2, default=str)
        return json_str.encode('utf-8'), "application/json", "nt_cu_graph.json"
        ...
        return csv_df.to_csv(index=False).encode('utf-8'), "text/csv", "nt_cu_edges_enhanced.csv"
        ...
        return csv_df.to_csv(index=False).encode('utf-8'), "text/csv", "nt_cu_nodes_enhanced.csv"
    elif export_format == "PNG":
        try:
            pos = nx.spring_layout(nx_graph, seed=42)
            plt.figure(figsize=(14, 12), dpi=300)
            node_colors = [
                get_nanotwinned_category_color(n) for n in nx_graph.nodes()
            ]
            nx.draw(
            ...
            buf.seek(0)
            plt.close()
            return buf.read(), "image/png", "nt_cu_graph.png"
        except Exception as e:
        ...
        try:
            ...
            buf.seek(0)
            plt.close()
            return buf.read(), "image/svg+xml", "nt_cu_graph.svg"
        except Exception as e:
        ...
        try:
            if include_metadata:
                nx_graph.graph['created'] = datetime.now().isoformat()
                nx_graph.graph['version'] = '6.2'
            nx.write_gexf(nx_graph, "nt_cu_graph.gexf")
            with open("nt_cu_graph.gexf", "rb") as f:
                return f.read(), "application/xml", "nt_cu_graph.gexf"
        except Exception as e:
...
def render_reasoning_dashboard(
    ...
    synonym_examples = [
        ("nanotwinned copper", "nanotwinned_cu"),
        ("nt cu", "nanotwinned_cu"),
        ("yield strength", "yield_strength"),
        ("ultimate tensile strength", "uts"),
        ("coherent twin boundary", "twin_boundary"),
    ]
...
def extract_doc_metrics(text: str) -> Dict[str, Any]:
    ...
    metrics: Dict[str, Any] = {}
    strength_matches = re.findall(r'(\d+(?:\.\d+)?)\s*(?:gpa|mpa)', text, re.I)
    if strength_matches:
        metrics['strength_mpa_gpa'] = [float(m) for m in strength_matches]
    size_matches = re.findall(r'(\d+(?:\.\d+)?)\s*(?:nm|um|µm)', text, re.I)
    if size_matches:
        metrics['size_nm_um'] = [float(m) for m in size_matches]
    twin_matches = re.findall(r'(?:twin\s*(?:spacing|thickness|lamella\s*thickness)\s*(?:of|is|=|:)?\s*)?(\d+(?:\.\d+)?)\s*(?:nm|µm|um)', text, re.I)
    if twin_matches:
        metrics['twin_spacing_nm'] = [float(m) for m in twin_matches]
    twin_frac_matches = re.findall(r'(\d+(?:\.\d+)?)\s*(?:%|percent)\s*(?:twin|twinning|volume\s*fraction)', text, re.I)
    if twin_frac_matches:
        metrics['twin_fraction_pct'] = [float(m) for m in twin_frac_matches]
    grain_matches = re.findall(r'(?:grain\s*size\s*(?:of|is|=|:)?\s*)?(\d+(?:\.\d+)?)\s*(?:nm|um|µm)', text, re.I)
    if grain_matches:
        metrics['grain_size_nm'] = [float(m) for m in grain_matches]
    current_density_matches = re.findall(r'(\d+(?:\.\d+)?)\s*(?:ma/cm2|ma\s*cm-2|a/dm2|a\s*dm-2)', text, re.I)
    if current_density_matches:
        metrics['current_density'] = [float(m) for m in current_density_matches]
    conductivity_matches = re.findall(r'(\d+(?:\.\d+)?)\s*(?:%iacs|ms/m|%[ \t]*iacs)', text, re.I)
    if conductivity_matches:
        metrics['conductivity_pct_iacs'] = [float(m) for m in conductivity_matches]
    return metrics
...
def render_sidebar() -> None:
    with st.sidebar:
        ...
        st.subheader("🔬 Nanotwinned Cu Focus Areas")
        st.markdown("- **Core Material:** Nanotwinned Cu (NT Cu)")
        st.markdown("- **Microstructure:** Twin boundaries (CTB/ITB), twin spacing, density, stacking faults")
        st.markdown("- **Mechanical Properties:** Yield strength, UTS, hardness, ductility, Hall-Petch")
        st.markdown("- **Synthesis:** Electrodeposition, pulsed/DC, additives (SPS, PEG, gelatin)")
        st.markdown("- **Characterization:** TEM, EBSD, XRD, APT, in situ techniques")
        st.markdown("- **Computational:** DFT, MD, Phase field, CPFEM, Machine Learning potentials")
        st.markdown("- **Reliability:** Electromigration, TSV, Cu-Cu bonding, Kirkendall voids")
        st.subheader("🧠 NLP Reasoning Options")
        st.session_state['use_ontology'] = st.checkbox(
            "Use ontology-based resolution", value=True,
            help="Maps synonyms like 'NT Cu', 'nanotwinned copper' to canonical concepts",
        )
        ...
        st.session_state['use_relationship_extraction'] = st.checkbox(
            ...
            help="Identifies causal links between processing, structure, and properties",
        )
...
# ============================================================================
# 1. QUERY ANALYSIS DATA STRUCTURES (adapted for Nanotwinned Cu)
# ============================================================================
class CuCoreProblem(Enum):
    ...
    RELIABILITY_APPLICATION = "reliability_application"
    COMPUTATIONAL_MATERIALS = "computational_materials"
    GENERAL = "general"
    MULTI_PROBLEM = "multi_problem"

@dataclass
class CuProblemDefinition:
    problem_id: CuCoreProblem
    title: str
...
# Pre-defined Cu Problem Definitions
CU_PROBLEM_DEFINITIONS: Dict[CuCoreProblem, CuProblemDefinition] = {
    CuCoreProblem.SYNTHESIS_OPTIMIZATION: CuProblemDefinition(
        problem_id=CuCoreProblem.SYNTHESIS_OPTIMIZATION, title="Electrodeposition & Additive Optimization",
        scientific_description="Controlling electrodeposition parameters (current density, pulse profile, additives) to achieve desired nanotwinned Cu microstructure.",
        root_cause="Complex interplay between bath chemistry, additives (SPS, PEG, gelatin), and deposition conditions determines twin density and grain structure.",
        key_concepts=["electrodeposited_nt_cu", "additive", "electrodeposition_parameter", "twin_boundary"],
        key_relationships=[("additive", "INFLUENCES", "twin_boundary"), ("electrodeposition_parameter", "INFLUENCES", "twin_boundary")],
        solution_directions=["Optimize pulse reverse waveforms", "Explore novel additive combinations", "Use machine learning for bath control"],
        relevant_materials=["nanotwinned_cu", "electrodeposited_nt_cu"],
        relevant_phenomena=["twin_boundary", "grain_growth"],
        relevant_properties=["twin_density", "grain_size"],
        example_queries=["How does SPS influence twin boundary formation?", "What is the role of gelatin in nanotwinned Cu electrodeposition?"],
        visualization_focus=["synthesis_process", "additive_effects"]
    ),
    CuCoreProblem.MICROSTRUCTURE_CONTROL: CuProblemDefinition(
        problem_id=CuCoreProblem.MICROSTRUCTURE_CONTROL, title="Twin Boundary Engineering",
        scientific_description="Controlling twin spacing, density, and fraction to tailor mechanical and electrical properties.",
        root_cause="Twin boundary formation is sensitive to stacking fault energy, deposition conditions, and post-deposition annealing.",
        key_concepts=["twin_boundary", "stacking_fault", "dislocation", "hall_petch"],
        key_relationships=[("twin_boundary", "INFLUENCES", "yield_strength"), ("stacking_fault", "INFLUENCES", "twin_boundary")],
        solution_directions=["Adjust twin spacing via deposition parameters", "Use annealing to stabilize twin structure", "Implement in situ characterization"],
        relevant_materials=["nanotwinned_cu"],
        relevant_phenomena=["deformation_twinning", "recrystallization"],
        relevant_properties=["twin_spacing", "twin_density", "hardness"],
        example_queries=["What is the optimal twin spacing for maximum strength?", "How does annealing affect twin stability?"],
        visualization_focus=["twin_boundary_map", "microstructure_evolution"]
    ),
    CuCoreProblem.MECHANICAL_PROPERTY_ENHANCEMENT: CuProblemDefinition(
        problem_id=CuCoreProblem.MECHANICAL_PROPERTY_ENHANCEMENT, title="Strength-Ductility Trade-off",
        scientific_description="Nanotwinned Cu offers high strength but often at the expense of ductility.",
        root_cause="Twin boundaries act as strong barriers to dislocation motion, but their high density can lead to brittle behavior.",
        key_concepts=["yield_strength", "ductility", "uts", "twin_boundary", "dislocation"],
        key_relationships=[("twin_boundary", "INFLUENCES", "yield_strength"), ("twin_boundary", "INFLUENCES", "ductility")],
        solution_directions=["Design graded twin structures", "Introduce secondary phases", "Optimize grain size distribution"],
        relevant_materials=["nanotwinned_cu"],
        relevant_phenomena=["strain_hardening", "deformation_twinning"],
        relevant_properties=["yield_strength", "ductility", "uts"],
        example_queries=["How can we improve ductility of nanotwinned Cu?", "What mechanisms govern the strength-ductility trade-off?"],
        visualization_focus=["mechanical_property_map", "stress_strain_curve"]
    ),
    CuCoreProblem.RELIABILITY_APPLICATION: CuProblemDefinition(
        problem_id=CuCoreProblem.RELIABILITY_APPLICATION, title="Reliability & Interconnect Applications",
        scientific_description="Nanotwinned Cu is used in 3D integration, TSV, and bonding due to its high electromigration resistance.",
        root_cause="Twin boundaries reduce grain boundary diffusion and suppress void formation.",
        key_concepts=["electromigration", "interconnect", "cu_cu_bonding", "kirkendall_void"],
        key_relationships=[("electromigration", "CAUSES", "kirkendall_void"), ("cu_cu_bonding", "FORMS", "interconnect")],
        solution_directions=["Optimize bonding conditions", "Use nanotwinned Cu for diffusion barriers", "In-situ monitoring of electromigration"],
        relevant_materials=["nanotwinned_cu"],
        relevant_phenomena=["electromigration", "kirkendall_void"],
        relevant_properties=["mean_time_to_failure", "electrical_conductivity"],
        example_queries=["How does nanotwinned Cu improve electromigration lifetime?", "What is the role of twin boundaries in Cu-Cu bonding?"],
        visualization_focus=["reliability_map", "bonding_interface"]
    ),
    CuCoreProblem.COMPUTATIONAL_MATERIALS: CuProblemDefinition(
        problem_id=CuCoreProblem.COMPUTATIONAL_MATERIALS, title="Computational Modeling of Nanotwinned Cu",
        scientific_description="Atomistic and continuum models to predict properties of nanotwinned Cu.",
        root_cause="High computational cost for large-scale simulations and accurate interatomic potentials.",
        key_concepts=["dft", "md", "phase_field", "ml_potential"],
        key_relationships=[("dft", "COMPUTES", "stacking_fault"), ("md", "SIMULATES", "dislocation")],
        solution_directions=["Develop machine-learned potentials", "Multiscale modeling framework", "High-throughput screening"],
        relevant_materials=["nanotwinned_cu"],
        relevant_phenomena=["deformation_twinning", "dislocation_motion"],
        relevant_properties=["stacking_fault_energy", "twin_boundary_energy"],
        example_queries=["What is the stacking fault energy of nanotwinned Cu from DFT?", "How can MD simulate twin migration?"],
        visualization_focus=["computational_model", "simulation_results"]
    ),
    CuCoreProblem.GENERAL: CuProblemDefinition(
        problem_id=CuCoreProblem.GENERAL, title="General Cu Inquiry",
        scientific_description="General inquiry about nanotwinned Cu.",
        root_cause="N/A", key_concepts=["nanotwinned_cu"], key_relationships=[], solution_directions=[],
        relevant_materials=[], relevant_phenomena=[], relevant_properties=[],
        example_queries=["What is nanotwinned copper?"], visualization_focus=["general_overview"]
    ),
    CuCoreProblem.MULTI_PROBLEM: CuProblemDefinition(
        problem_id=CuCoreProblem.MULTI_PROBLEM, title="Multi-Problem Inquiry",
        scientific_description="Inquiry spanning multiple Cu problems.",
...
class QueryAnalysisResult:
    ...
    primary_problem: CuCoreProblem
    secondary_problems: List[CuCoreProblem]
...
# ============================================================================
# 2. LLM QUERY ANALYZERS (Abstract + Implementations) - same as SIB but using CuProblem
# ============================================================================
...
class FallbackAnalyzer(LLMQueryAnalyzer):
    PROBLEM_KEYWORDS = {
        CuCoreProblem.SYNTHESIS_OPTIMIZATION: {"electrodeposition", "additive", "pulse", "bath", "current density", "gelatin", "sps"},
        CuCoreProblem.MICROSTRUCTURE_CONTROL: {"twin", "twin boundary", "twin spacing", "stacking fault", "annealing", "recrystallization"},
        CuCoreProblem.MECHANICAL_PROPERTY_ENHANCEMENT: {"strength", "ductility", "hardness", "uts", "yield", "tensile", "hall-petch"},
        CuCoreProblem.RELIABILITY_APPLICATION: {"electromigration", "interconnect", "tsv", "bonding", "reliability", "kirkendall"},
        CuCoreProblem.COMPUTATIONAL_MATERIALS: {"dft", "md", "molecular dynamics", "phase field", "potential", "machine learning"},
    }
    ...
    def analyze_query(self, query: str, ontology: Any) -> QueryAnalysisResult:
        ...
        primary = max(problem_scores, key=problem_scores.get) if sum(problem_scores.values()) > 0 else CuCoreProblem.GENERAL
        ...
        inferred = []
        if primary != CuCoreProblem.GENERAL:
            pdef = CU_PROBLEM_DEFINITIONS[primary]
        ...
        priorities = {}
        pdef = CU_PROBLEM_DEFINITIONS.get(primary, CU_PROBLEM_DEFINITIONS[CuCoreProblem.GENERAL])
...
class OpenAIQueryAnalyzer(LLMQueryAnalyzer):
    ...
    def analyze_query(self, query: str, ontology: Any) -> QueryAnalysisResult:
        ...
        system_prompt = """You are an expert in electrodeposited nanotwinned copper. Analyze the user's query and return ONLY valid JSON with:
        1. "primary_problem": One of: synthesis_optimization, microstructure_control, mechanical_property_enhancement, reliability_application, computational_materials, general, multi_problem
        ...
        try:
            ...
            problem_map = {p.value: p for p in CuCoreProblem}
            primary = problem_map.get(parsed.get("primary_problem", "general"), CuCoreProblem.GENERAL)
            ...
            return QueryAnalysisResult(
                ...
                visualization_focus=CU_PROBLEM_DEFINITIONS[primary].visualization_focus, reasoning_chain=parsed.get("reasoning_chain", ["LLM analysis completed"]), confidence=0.85
            )
        except Exception as e:
...
class LocalLLMQueryAnalyzer(LLMQueryAnalyzer):
    ...
    def analyze_query(self, query: str, ontology: Any) -> QueryAnalysisResult:
        if not self.is_available():
            return FallbackAnalyzer().analyze_query(query, ontology)
        prompt = (
            f"[INST] You are an expert in nanotwinned Cu. Analyze: '{query}'. "
...
# ============================================================================
# 3. DYNAMIC ONTOLOGY EXPANDER (unchanged)
# ============================================================================
class DynamicOntologyExpander:
    ...
    def _create_bridge_concept(self, missing_concept: str, source_query: str, problem: CuCoreProblem) -> Optional[Dict]:
        bridge_name = f"query_bridge_{missing_concept.replace(' ', '_').lower()}"
        if bridge_name in self.ontology.concepts: return None
        pdef = CU_PROBLEM_DEFINITIONS.get(problem, CU_PROBLEM_DEFINITIONS[CuCoreProblem.GENERAL])
...
class GraphRAGAnswerGenerator:
    ...
    def generate_ground_response(self, query: str, analysis: QueryAnalysisResult, subgraph: nx.Graph, concept_abstract_map: Dict[str, List[int]], all_texts: Union[List[str], Dict[int, str]], max_docs_per_concept: int = 2) -> str:
        ...
        nl = chr(10)
        prompt = "You are an expert in electrodeposited nanotwinned copper. Answer the user's query based *strictly* on the provided graph context and evidence snippets." + nl
        ...
        if evidence_snippets:
            ...
            prompt += "No direct text snippets found. Rely on your general knowledge but note the lack of specific retrieved context." + nl
...
# ============================================================================
# 7. STREAMLIT UI INTEGRATORS (unchanged except references to CuProblem)
# ============================================================================
def render_llm_query_panel(ontology: Any, expander: DynamicOntologyExpander, full_graph: nx.Graph) -> Optional[QueryAnalysisResult]:
    ...
    example_queries = [q for pdef in CU_PROBLEM_DEFINITIONS.values() for q in pdef.example_queries[:1]]
    selected_example = st.sidebar.selectbox("Or select an example:", [""] + example_queries, key="example_query_select")
    query = st.sidebar.text_area("Your question:", value=selected_example, height=100, key="llm_query_input", placeholder="e.g., How does twin spacing affect yield strength?")
...
def render_llm_qa_tab(analysis_data: Dict, ontology: Any):
    ...
    with col1: query = st.text_input("Enter your research question:", placeholder="e.g., How does SPS influence twin boundary formation in electrodeposited Cu?")
...
def main() -> None:
    st.title(
        "🔬 Nanotwinned Cu Quantitative Descriptor Graph v6.2"
    )
    st.caption(
        "Multi-level reasoning concept graph for numerical/quantitative description of Electrodeposited Nanotwinned Copper | "
        "Focus: Synthesis, Microstructure, Mechanical Properties, Reliability | "
        ...
        with tabs[tab_idx]:
            ...
            if top_scores.empty:
                ...
                st.download_button(
                    ...
                    file_name="nt_cu_research_directions.csv", mime="text/csv",
                )
        ...
        with tabs[tab_idx]:
            ...
            if st.button("Generate Publication Figure"):
                ...
                if pub_bytes:
                    st.download_button(
                        ...
                        data=pub_bytes,
                        file_name="nt_cu_graph_publication.png",
                        mime="image/png",
                    )
            st.markdown("---")
            ...
            if st.button("Generate Markdown Report"):
                ...
                st.download_button(
                    ...
                    file_name="nt_cu_analysis_report.md",
                    mime="text/markdown",
                )
            ...
            st.download_button(
                ...
                data=csv_concepts,
                file_name="nt_cu_concepts_enhanced.csv", mime="text/csv",
            )
