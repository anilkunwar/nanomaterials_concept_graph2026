...
- FIXED: Preprocess malformed Scopus CSVs (mixed delimiters, doubled quotes, trailing semicolons)
"""
...
def parse_single_csv_section(section_content: str, source_filename: str, section_index: int) -> List[Dict[str, Any]]:
    ...
    """
    try:
        # --- FIX FOR MALFORMED SCOPUS CSVs (e.g., categoryA, categoryC) ---
        # 1. Standardize delimiters: Replace mixed delimiters with doubled quotes
        #    (e.g., ,"" or ;"") with standard CSV formatting (",")
        section_content = re.sub(r'[,;]""', '","', section_content)

        # 2. Remove excessive trailing semicolons from each line.
        #    We only strip if there are 5+ trailing semicolons to avoid removing
        #    legitimate single semicolons at the end of a field.
        lines = []
