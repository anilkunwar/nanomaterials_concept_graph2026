...
def render_graph_pyvis(nx_graph, concept_abstract_map, physics_enabled=True,
    ...
    for u, v in nx_graph.edges():
        ...
        net.add_edge(
            ...
            dashes=True if is_inferred else False,  # Dashed for inferred edges
            title=edge_tooltip
        )

        # v3.0: Edge label display modes
        if edge_label_mode == "all":
            # Add label to edge (limited support in PyVis, using title)
            pass
        elif edge_label_mode == "threshold" and w > max_weight * 0.7:
            pass
...
def render_sunburst_chart(labels, parents, values, cmap_name="viridis", 
    ...
    # v3.0: Symbol-based representation
    SYMBOL_LIBRARY = ['✦', '★', '●', '■', '▲', '◆', '⬟', '⬢', '◉', '◈', '◇', '○']
    ...
    if use_symbols:
        ...
        for i, lab in enumerate(labels):
            d = depths[i]
            if d == 0:
                # Root level = single symbol
                node_symbols[lab] = SYMBOL_LIBRARY[0]
                display_labels.append(f"{SYMBOL_LIBRARY[0]} {lab}")
            else:
                ...
                # Display the last 2-3 symbols in the chain + truncated name
                display_labels.append(f"{''.join(chain[-2:])} {lab[:15]}")
    else:
