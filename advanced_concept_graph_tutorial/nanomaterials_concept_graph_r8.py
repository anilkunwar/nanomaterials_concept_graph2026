...
def get_nanomaterials_category_color(concept: str, cmap_colors: Optional[List[str]] = None) -> str:
    ...
    if any(c in concept_lower for c in ['nanotwinned cu', 'nt cu', 'nanotwin']):
        ...
        return "#388E3C"  # Green
    # v3.0: Defect engineering colors
...
def get_concept_category(concept: str) -> str:
    ...
    if any(c in concept_lower for c in ['nanotwinned cu', 'nt cu', 'nanotwin', 'cu@ag', 'cu/ag', 'core shell', 'defect engineered ag', 'ag defect']):
        ...
        return 'functional'
    # v3.0: Defect engineering category
...
def render_graph_pyvis(nx_graph, concept_abstract_map, physics_enabled=True,
    ...
    """
    v3.0: Publication-ready PyVis with:
    - N1, N2... abbreviated labels inside circle nodes
    - HTML legend mapping abbreviations to full names
    - Edge value inspection (weight, type, confidence, inference status)
    ...
    # v3.0: N1/N2 abbreviation mapping
    label_map = {}  # short_label -> full_label
    n_counter = 1
    ...
    for i, node in enumerate(nx_graph.nodes()):
        ...
        if use_abbreviated_labels and len(original_label) > max_label_length:
            ...
            node_shape = 'circle'  # Forces label inside the node
        ...
        # v3.0: Enhanced tooltip with full info
    ...
    # v3.0: Dynamic edge opacity and enhanced tooltips
    ...
    color_map = {
        ...
        'hypernym':     'rgba(156, 39, 176, 0.5)',    # Purple for hierarchy
        'hyponym':      'rgba(156, 39, 176, 0.3)',
        'causes':       'rgba(244, 67, 54, 0.6)',     # Red for causal
        'influences':   'rgba(255, 152, 0, 0.6)',     # Orange for influence
    ...
    for u, v in nx_graph.edges():
        ...
        # v3.0: Edge tooltip with full inspection data
        ...
        # v3.0: Edge label display modes — FIXED with proper label/font kwargs
    ...
    # v3.0: HTML Legend for N1/N2 abbreviations
...
def render_sunburst_chart(labels, parents, values, cmap_name="viridis", 
                            ...
                            use_symbols=True):
    """
    v3.0: Enhanced sunburst with symbol-based hierarchical representation.
    Uses symbol chains (✦★●■▲◆) to represent depth and category.
    """
    ...
    # v3.0: Symbol-based representation — PURE symbols in chart, labels in legend
    SYMBOL_LIBRARY = ['✦', '★', '●', '■', '▲', '◆', '⬟', '⬢', '◉', '◈', '◇', '○']
    ...
    if use_symbols:
        ...
        for i, lab in enumerate(labels):
            ...
            if d == 0:
                ...
                # Add own symbol based on depth and sibling index
                siblings = [labels[j] for j in range(len(labels)) if parents[j] == parents[i] and depths[j] == d]
                ...
                # Show ONLY the last 2-3 symbols in the chain, NO text
    ...
    fig = go.Figure(go.Sunburst(
        ids=unique_ids,
        labels=display_labels if use_symbols else unique_ids,
        parents=parent_ids,
        values=values,
    ...
    # v3.0: Symbol legend — FIXED with depth-grouped proportional bars
    if use_symbols:
        ...
        # Build legend entries with metadata from the computed data
        legend_entries = []
        for i, lab in enumerate(labels):
            d = depths[i]
            sym = display_labels[i] if use_symbols else lab
            # Get color from our computed palette
            try:
                color = plot_colors[i]
            except:
                color = '#9E9E9E'
        ...
        # Sort by depth then by value (frequency) descending
        legend_entries.sort(key=lambda x: (x['depth'], -x['value']))

        # Group by depth with headers
        depth_names = {0: "🌐 Root Domain", 1: "📁 Major Category", 2: "📄 Sub-category",
                       3: "🔍 Concept Group", 4: "📎 Specific Concept", 5: "📌 Detailed Term"}
        ...
        for entry in legend_entries:
            ...
            # Proportional bar based on value
