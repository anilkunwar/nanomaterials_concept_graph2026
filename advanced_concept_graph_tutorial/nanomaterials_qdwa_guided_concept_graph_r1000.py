...
# ============================================================================
# ENHANCED ONTOLOGY & NLP REASONING SYSTEM (NANOMATERIALS)
# ============================================================================
...
# Hand-curated abbreviation map for every ontology concept
_SHORT_NAME_MAP: Dict[str, str] = {
    "nanotwinned_copper":               "nt-Cu",
    "core_shell_cuag":                  "Cu@Ag",
    "defect_engineered_ag":             "def-Ag",
    "nanoparticle":                     "NP",
    "thin_film":                        "Film",
    "twin_boundary":                    "TB",
    "coherent_twin_boundary":           "CTB",
    "incoherent_twin_boundary":         "ITB",
    "stacking_fault":                   "SF",
    "dislocation":                      "Disl",
    "grain_boundary":                   "GB",
    "vacancy":                          "Vac",
    ...
    "electrical_conductivity":          "σ_e",
    "thermal_conductivity":             "κ",
    "electrodeposition":                "ED",
    "sputtering":                       "Sput",
    "chemical_vapor_deposition":        "CVD",
    "annealing":                        "Ann",
    "severe_plastic_deformation":       "SPD",
    "defect_engineering":               "DefEng",
    "irradiation":                      "Irrad",
    "transmission_electron_microscopy": "TEM",
    "electron_backscatter_diffraction": "EBSD",
    "xray_diffraction":                 "XRD",
...
def get_short_name(canonical_name: str) -> str:
    ...
    Falls back to initials if the concept is not in the curated map
    (e.g. a dynamically discovered concept).
    """
...
def format_node_for_pyvis(
    node_id: str,
    concept_key: str,
    label_mode: NodeLabelMode = NodeLabelMode.FULL_NAME,
    external_label_text: str = "",
    external_font_size: int = 14,
    external_font_color: str = "#333333",
    external_label_align: str = "left",
    node_color: str = "#5B9BD5",
    node_size: int = 25,
    title_text: str = "",
    **extra_node_attrs,
) -> Dict[str, Any]:
    """
    Build the complete attribute dict for a single pyvis node,
    respecting the chosen label display mode.
    """
    if label_mode == NodeLabelMode.FULL_NAME:
        internal_label = concept_key.replace("_", " ").title()
        font_config = {
            "color": "#FFFFFF",
            "size": 12,
            "face": "Arial",
        ...
        internal_label = get_short_name(concept_key)
        font_config = {
            "color": "#FFFFFF",
            "size": 11,
            "face": "Arial",
        ...
        fallback = concept_key.replace("_", " ").title()
        internal_label = external_label_text if external_label_text else fallback
        font_config = {
            "color": external_font_color,
            "size": external_font_size,
            "face": "Arial",
            "align": external_label_align,
            "bold": {
                "color": external_font_color,
                "size": external_font_size + 1,
            },
        }
    else:
        internal_label = concept_key.replace("_", " ").title()
        font_config = {"color": "#FFFFFF", "size": 12, "face": "Arial", "align": "center"}

    tooltip = title_text if title_text else concept_key.replace("_", " ").title()
...
def add_graph_nodes_to_pyvis(
    ...
    if concept_type_colors is None:
        concept_type_colors = {
            ConceptType.MATERIAL:       "#E74C3C",
            ConceptType.MICROSTRUCTURE: "#3498DB",
            ConceptType.PROPERTY:       "#2ECC71",
            ConceptType.PROCESS:        "#F39C12",
            ConceptType.METHOD:         "#9B59B6",
            ConceptType.PHENOMENON:     "#1ABC9C",
            ConceptType.PARAMETER:      "#E67E22",
            ConceptType.MODEL:          "#34495E",
            ConceptType.GENERAL:        "#95A5A6",
        }

    weights_list = [node_weights.get(n, 1.0) for n in graph.nodes]
    w_min = min(weights_list) if weights_list else 1.0
    ...
    for node in graph.nodes:
        ...
        node_size = int(s_min + (s_max - s_min) * (w - w_min) / w_range)
        node_size = max(s_min, min(s_max, node_size))

        if node in color_map:
            node_color = color_map[node]
        elif ontology and node in ontology.concepts:
            ct = ontology.concepts[node].concept_type
            node_color = concept_type_colors.get(ct, "#95A5A6")
        else:
            node_color = "#95A5A6"

        tip_parts = [node.replace("_", " ").title()]
        if ontology and node in ontology.concepts:
            cn = ontology.concepts[node]
        ...
        node_attrs = format_node_for_pyvis(
            node_id=node,
            concept_key=node,
            label_mode=label_mode,
            external_label_text=ext_label,
            external_font_size=external_font_size,
            external_font_color=external_font_color,
            external_label_align=external_label_align,
...
class DomainOntology:
    """Comprehensive ontology for Nanomaterials Quantitative Descriptors."""
    ...
    def _build_ontology(self) -> None:
        # === NANOMATERIALS (Core Structures) ===
        self._add_concept("nanotwinned_copper", ConceptType.MATERIAL,
            synonyms={"nt cu", "nanotwinned cu", "nt copper", "nanotwinned copper"},
            definition="Copper with nanoscale twin boundaries exhibiting high strength and conductivity")
        self._add_concept("core_shell_cuag", ConceptType.MATERIAL,
            synonyms={"cu@ag", "cu/ag", "core-shell cu-ag", "copper-silver core shell"},
            definition="Core-shell copper-silver nanoparticles with engineered interfaces")
        self._add_concept("defect_engineered_ag", ConceptType.MATERIAL,
            synonyms={"defect engineered silver", "ag defect", "silver defect", "vacancy ag"},
            definition="Silver nanoparticles with engineered defect structures (vacancies, dislocations)")
        self._add_concept("nanoparticle", ConceptType.MATERIAL,
            synonyms={"nps", "nanoparticles", "nano particles"},
            definition="Nanoscale particles (1-100nm) with high surface-to-volume ratio")
        self._add_concept("thin_film", ConceptType.MATERIAL,
            synonyms={"thin films", "coating", "nanoscale film"},
            definition="Thin layers of material ranging from fractions of a nanometer to several microns")

        # === MICROSTRUCTURES & DEFECTS ===
        self._add_concept("twin_boundary", ConceptType.MICROSTRUCTURE,
            synonyms={"tb", "twin interface", "nanotwin"},
            definition="Symmetrical intergrowth of two adjacent crystal lattices")
        self._add_concept("coherent_twin_boundary", ConceptType.MICROSTRUCTURE,
            synonyms={"ctb", "coherent twin"},
            definition="Twin boundary with perfect atomic matching across the interface")
        self._add_concept("incoherent_twin_boundary", ConceptType.MICROSTRUCTURE,
            synonyms={"itb", "incoherent twin"},
            definition="Twin boundary with atomic mismatch or steps")
        self._add_concept("stacking_fault", ConceptType.MICROSTRUCTURE,
            synonyms={"sf", "planar defect"},
            definition="Planar defect involving an error in the stacking sequence of atomic layers")
        self._add_concept("dislocation", ConceptType.MICROSTRUCTURE,
            synonyms={"disl", "line defect", "edge dislocation", "screw dislocation"},
            definition="Linear crystallographic defect governing plastic deformation")
        self._add_concept("grain_boundary", ConceptType.MICROSTRUCTURE,
            synonyms={"gb", "grain interface"},
            definition="Interface between two grains of a crystalline material")
        self._add_concept("vacancy", ConceptType.MICROSTRUCTURE,
            synonyms={"point defect", "lattice vacancy", "void"},
            definition="Point defect where an atom is missing from its regular lattice site")
        self._add_concept("interstitial", ConceptType.MICROSTRUCTURE,
            synonyms={"self interstitial", "interstitial defect"},
            definition="Point defect where an extra atom occupies an interstitial site")
        ...
        self._add_concept("ultimate_tensile_strength", ConceptType.PROPERTY,
            synonyms={"uts", "tensile strength"},
            definition="Maximum stress a material can withstand while being stretched")
        self._add_concept("youngs_modulus", ConceptType.PROPERTY,
            synonyms={"elastic modulus", "e-modulus", "modulus of elasticity"},
            definition="Measure of a material's stiffness")
        self._add_concept("hardness", ConceptType.PROPERTY,
            synonyms={"hv", "vickers hardness", "nanoindentation hardness"},
            definition="Resistance to localized plastic deformation")
        self._add_concept("ductility", ConceptType.PROPERTY,
            synonyms={"elongation", "strain to failure", "plasticity"},
            definition="Ability of a material to undergo significant plastic deformation before rupture")
        self._add_concept("stacking_fault_energy", ConceptType.PROPERTY,
            synonyms={"sfe", "gsfe"},
            definition="Energy per unit area of the fault, dictating deformation twinning and dislocation cross-slip")
        self._add_concept("electrical_conductivity", ConceptType.PROPERTY,
            synonyms={"conductivity", "resistivity"},
        ...
        self._add_concept("thermal_conductivity", ConceptType.PROPERTY,
            synonyms={"thermal transport"},
            definition="Measure of a material's ability to conduct heat")

        # === SYNTHESIS & PROCESSES ===
        self._add_concept("electrodeposition", ConceptType.PROCESS,
            synonyms={"electroplating", "electrochemical deposition"},
            definition="Process of depositing a layer of metal on a conductive surface using electric current")
        self._add_concept("sputtering", ConceptType.PROCESS,
            synonyms={"magnetron sputtering", "pvd", "physical vapor deposition"},
            definition="Physical vapor deposition process where material is ejected from a target")
        self._add_concept("chemical_vapor_deposition", ConceptType.PROCESS,
            synonyms={"cvd"},
            definition="Vacuum deposition method to produce high-purity solid films")
        self._add_concept("annealing", ConceptType.PROCESS,
            synonyms={"heat treatment", "thermal treatment"},
            definition="Heat treatment that alters microstructure to reduce defects and relieve stress")
        self._add_concept("severe_plastic_deformation", ConceptType.PROCESS,
            synonyms={"spd", "ecap", "hpt", "high pressure torsion", "equal channel angular pressing"},
            definition="Process involving very large strains to produce ultrafine-grained materials")
        self._add_concept("defect_engineering", ConceptType.PROCESS,
            synonyms={"defect design", "defect tailoring"},
            definition="Controlled introduction and manipulation of defects to tune properties")
        self._add_concept("irradiation", ConceptType.PROCESS,
            synonyms={"radiation damage", "ion implantation", "ion beam"},
            definition="Exposure to radiation to induce point defects and microstructural changes")

        # === CHARACTERIZATION & MODELING METHODS ===
        self._add_concept("transmission_electron_microscopy", ConceptType.METHOD,
            synonyms={"tem", "hrtem", "high resolution tem", "electron microscopy"},
            definition="Imaging technique using a beam of electrons transmitted through a specimen")
        self._add_concept("electron_backscatter_diffraction", ConceptType.METHOD,
            synonyms={"ebsd", "ebSD"},
            definition="Technique for studying the crystallographic structure of materials")
        self._add_concept("xray_diffraction", ConceptType.METHOD,
            synonyms={"xrd", "x-ray diffraction"},
            definition="Technique for analyzing the crystal structure of materials")
        self._add_concept("density_functional_theory", ConceptType.METHOD,
            synonyms={"dft", "ab initio", "first principles"},
            definition="Computational quantum mechanical modeling method for electronic structure")
        self._add_concept("molecular_dynamics", ConceptType.METHOD,
            synonyms={"md", "atomistic simulation"},
            definition="Computer simulation method for analyzing the physical movements of atoms")
    ...
    def _build_causal_chains(self) -> None:
        ...
        causal_chains = [
            ("electrodeposition", RelationshipType.CAUSES, "twin_boundary", 0.90),
            ("electrodeposition", RelationshipType.INFLUENCES, "nanotwinned_copper", 0.95),
            ("sputtering", RelationshipType.CAUSES, "thin_film", 0.90),
            ("sputtering", RelationshipType.INFLUENCES, "core_shell_cuag", 0.85),
            ("annealing", RelationshipType.MODIFIES, "grain_boundary", 0.80),
            ("annealing", RelationshipType.CAUSES, "coherent_twin_boundary", 0.75),
            ("irradiation", RelationshipType.CAUSES, "vacancy", 0.95),
            ("irradiation", RelationshipType.CAUSES, "dislocation", 0.85),
            ("defect_engineering", RelationshipType.INFLUENCES, "defect_engineered_ag", 0.90),
            ("severe_plastic_deformation", RelationshipType.CAUSES, "grain_boundary", 0.90),
            
            # === MICROSTRUCTURE -> PROPERTIES ===
            ("twin_boundary", RelationshipType.INFLUENCES, "yield_strength", 0.90),
            ("twin_boundary", RelationshipType.INFLUENCES, "ductility", 0.75),
            ("coherent_twin_boundary", RelationshipType.INFLUENCES, "electrical_conductivity", 0.85),
            ("incoherent_twin_boundary", RelationshipType.INFLUENCES, "yield_strength", 0.70),
            ("stacking_fault_energy", RelationshipType.INFLUENCES, "twin_boundary", 0.95),
            ("stacking_fault_energy", RelationshipType.CONSTRAINS, "ductility", 0.80),
            ("grain_boundary", RelationshipType.INFLUENCES, "yield_strength", 0.85),
            ("dislocation", RelationshipType.INFLUENCES, "yield_strength", 0.80),
            ("vacancy", RelationshipType.INFLUENCES, "electrical_conductivity", -0.75),
            ("vacancy", RelationshipType.INFLUENCES, "thermal_conductivity", -0.70),
            
            # === MATERIALS -> PROPERTIES ===
            ("nanotwinned_copper", RelationshipType.INFLUENCES, "yield_strength", 0.90),
            ("nanotwinned_copper", RelationshipType.INFLUENCES, "electrical_conductivity", 0.85),
            ("core_shell_cuag", RelationshipType.INFLUENCES, "thermal_conductivity", 0.80),
            ("core_shell_cuag", RelationshipType.INFLUENCES, "hardness", 0.75),
            ("defect_engineered_ag", RelationshipType.INFLUENCES, "hardness", 0.85),
            ("defect_engineered_ag", RelationshipType.INFLUENCES, "electrical_conductivity", -0.60),
            
            # === METHODS -> MICROSTRUCTURE/PROPERTIES ===
            ("transmission_electron_microscopy", RelationshipType.DETECTS, "twin_boundary", 0.95),
            ("transmission_electron_microscopy", RelationshipType.DETECTS, "dislocation", 0.90),
            ("electron_backscatter_diffraction", RelationshipType.DETECTS, "grain_boundary", 0.90),
            ("xray_diffraction", RelationshipType.MEASURES, "stacking_fault_energy", 0.70),
            ("density_functional_theory", RelationshipType.COMPUTES, "stacking_fault_energy", 0.90),
            ("molecular_dynamics", RelationshipType.SIMULATES, "dislocation", 0.85),
        ]
...
_HIERARCHY_PARENTS: Dict[str, Tuple[str, int]] = {
    # --- Root domain ---
    "nanotwinned_copper":               (None, 0),
    "core_shell_cuag":                  (None, 0),
    "defect_engineered_ag":             (None, 0),
    "nanoparticle":                     (None, 0),
    "thin_film":                        (None, 0),

    # --- Tier 1: Materials ---
    "nanotwinned_copper":               ("Nanomaterials", 1),
    "core_shell_cuag":                  ("Nanomaterials", 1),
    "defect_engineered_ag":             ("Nanomaterials", 1),
    "nanoparticle":                     ("Nanomaterials", 1),
    "thin_film":                        ("Nanomaterials", 1),

    # --- Tier 1: Microstructures & Defects ---
    "twin_boundary":                    ("Microstructures & Defects", 1),
    "coherent_twin_boundary":           ("Microstructures & Defects", 1),
    "incoherent_twin_boundary":         ("Microstructures & Defects", 1),
    "stacking_fault":                   ("Microstructures & Defects", 1),
    "dislocation":                      ("Microstructures & Defects", 1),
    "grain_boundary":                   ("Microstructures & Defects", 1),
    "vacancy":                          ("Microstructures & Defects", 1),
    "interstitial":                     ("Microstructures & Defects", 1),

    # --- Tier 1: Properties ---
    "yield_strength":                   ("Mechanical & Functional Properties", 1),
    "ultimate_tensile_strength":        ("Mechanical & Functional Properties", 1),
    "youngs_modulus":                   ("Mechanical & Functional Properties", 1),
    "hardness":                         ("Mechanical & Functional Properties", 1),
    "ductility":                        ("Mechanical & Functional Properties", 1),
    "stacking_fault_energy":            ("Mechanical & Functional Properties", 1),
    "electrical_conductivity":          ("Mechanical & Functional Properties", 1),
    "thermal_conductivity":             ("Mechanical & Functional Properties", 1),

    # --- Tier 1: Processes ---
    "electrodeposition":                ("Synthesis & Processing", 1),
    "sputtering":                       ("Synthesis & Processing", 1),
    "chemical_vapor_deposition":        ("Synthesis & Processing", 1),
    "annealing":                        ("Synthesis & Processing", 1),
    "severe_plastic_deformation":       ("Synthesis & Processing", 1),
    "defect_engineering":               ("Synthesis & Processing", 1),
    "irradiation":                      ("Synthesis & Processing", 1),

    # --- Tier 1: Methods ---
    "transmission_electron_microscopy": ("Characterization & Modeling", 1),
    "electron_backscatter_diffraction": ("Characterization & Modeling", 1),
    "xray_diffraction":                 ("Characterization & Modeling", 1),
    "density_functional_theory":        ("Characterization & Modeling", 1),
    "molecular_dynamics":               ("Characterization & Modeling", 1),
}
...
def get_hierarchy_label(concept_key: str,
    ...
    style : str
        "arrow"   → "Mechanical & Functional Properties → Yield Strength"
        "bracket" → "Mechanical & Functional Properties [Yield Strength]"
        "dot"     → "Mechanical & Functional Properties · Yield Strength"
...
def get_hierarchy_path(concept_key: str) -> List[str]:
    ...
    ["Nanomaterials", "Mechanical & Functional Properties", "Yield Strength"].
    ...
    if entry is None or entry[0] is None:
        return ["Nanomaterials", leaf]

    parent_label = entry[0]
    return ["Nanomaterials", parent_label, leaf]
...
def build_sunburst_data(
    ...
    # --- Root node ---
    root_id = "Nanomaterials"
    ids.append(root_id)
    labels.append("Nanomaterials")
...
class AdvancedConceptResolver:
    ...
    def _context_disambiguation(
        ...
        mechanical_indicators = [
            'yield', 'strength', 'hardness', 'ductility', 'modulus', 'stress',
            'strain', 'tensile', 'fatigue', 'creep', 'wear'
        ]
        if any(ind in context_lower for ind in mechanical_indicators):
            if 'yield' in text or 'strength' in text:
                return "yield_strength" if 'yield' in text else "ultimate_tensile_strength"
            if 'hardness' in text:
                return "hardness"
            if 'ductility' in text:
                return "ductility"
        return None
...
# ============================================================================
# ENHANCED CONCEPT EXTRACTOR (Nanomaterials-focused)
# ============================================================================
class EnhancedConceptExtractor:
    ...
    def _build_extraction_patterns(self) -> None:
        # Nanomaterials specific patterns (see NANOMAT_PATTERNS below)
        self.material_patterns = [
            r'\bnanotwinned\s+cu(?:pper)?\b', r'\bnt\s+cu\b',
            r'\bcu@ag\b', r'\bcu/ag\b', r'\bcore[-\s]?shell\s+cu[-\s]?ag\b',
            r'\bdefect[-\s]?engineered\s+(?:ag|silver)\b',
            r'\bnano[-\s]?particles?\b', r'\bnps\b',
            r'\bthin\s+films?\b', r'\bcoating\b',
        ]
        self.microstructure_patterns = [
            r'\btwin\s+boundary\b', r'\bcoherent\s+twin\s+boundary\b',
            r'\bincoherent\s+twin\s+boundary\b', r'\bstacking\s+fault\b',
            r'\bdislocation\b', r'\bgrain\s+boundary\b',
            r'\bvacancy\b', r'\binterstitial\b', r'\bpoint\s+defect\b',
        ]
        self.property_patterns = [
            r'\byield\s+strength\b', r'\bytensile\s+strength\b',
            r'\belastic\s+modulus\b', r'\byoung\s+modulus\b',
            r'\bhardness\b', r'\bductility\b', r'\belongation\b',
            r'\bstacking\s+fault\s+energy\b', r'\bsfe\b',
            r'\belectrical\s+conductivity\b', r'\bthermal\s+conductivity\b',
        ]
        self.process_patterns = [
            r'\belectrodeposition\b', r'\belectroplating\b',
            r'\bsputtering\b', r'\bphysical\s+vapor\s+deposition\b',
            r'\bchemical\s+vapor\s+deposition\b', r'\bcvd\b',
            r'\bannealing\b', r'\bheat\s+treatment\b',
            r'\bsevere\s+plastic\s+deformation\b', r'\becap\b',
            r'\bhigh\s+pressure\s+torsion\b', r'\bhpt\b',
            r'\bdefect\s+engineering\b', r'\birradiation\b',
        ]
        self.method_patterns = [
            r'\btransmission\s+electron\s+microscopy\b', r'\btem\b',
            r'\belectron\s+backscatter\s+diffraction\b', r'\bebsd\b',
            r'\bx[-\s]?ray\s+diffraction\b', r'\bxrd\b',
            r'\bdensity\s+functional\s+theory\b', r'\bdft\b',
            r'\bmolecular\s+dynamics\b', r'\bmd\b',
        ]
        self.all_patterns = (
            self.material_patterns + self.microstructure_patterns +
            self.property_patterns + self.process_patterns +
            self.method_patterns
        )
...
# ============================================================================
# NANOMATERIALS KEYWORDS & PATTERNS
# ============================================================================
NANOMAT_PATTERNS = [
    r'\b(?:nanotwinned|nt)\s+cu(?:pper)?\b', r'\bcu@ag\b', r'\bcu/ag\b',
    r'\bcore[-\s]?shell\s+(?:cu|ag|cu@ag|cu/ag)\b', r'\bdefect[-\s]?engineered\s+(?:ag|silver)\b',
    r'\btwin\s+boundary\b', r'\bcoherent\s+twin\s+boundary\b', r'\bincoherent\s+twin\s+boundary\b',
    r'\bstacking\s+fault\b', r'\bstacking\s+fault\s+energy\b', r'\bdislocation\b', r'\bgrain\s+boundary\b',
    r'\belectrodeposition\b', r'\bsputtering\b', r'\bchemical\s+vapor\s+deposition\b',
    r'\bdensity\s+functional\s+theory\b', r'\bmolecular\s+dynamics\b',
    r'\byield\s+strength\b', r'\btensile\s+strength\b', r'\belastic\s+modulus\b', r'\bhardness\b', r'\bductility\b',
    r'\btransmission\s+electron\s+microscopy\b', r'\belectron\s+backscatter\s+diffraction\b', r'\bx[-\s]?ray\s+diffraction\b',
    r'\bnano(?:particle|wire|rod|sheet|plate|cube|sphere|cluster|composite|structure|crystal|tube)\b',
    r'\b(?:equal[-\s]?channel\s+angular\s+pressing|ecap)\b', r'\bhigh[-\s]?pressure\s+torsion\b',
    r'\b(?:vacancy|interstitial|frenkel|schottky)\b', r'\bdefect\s+(?:engineering|design|tailoring)\b',
]

ALL_NANOMAT_KEYWORDS = [
...
NANOMAT_DESCRIPTOR_MAPPING = {
    r'nanotwinned|nt\s+cu|nanotwin|cu@ag|cu/ag|core[-\s]?shell|defect[-\s]?engineered\s+ag|silver': 'nanomaterial',
    r'twin\s+boundary|stacking\s+fault|dislocation|grain\s+boundary|vacancy|interstitial|point\s+defect': 'microstructure_defect',
    r'yield\s+strength|uts|hardness|elastic\s+modulus|young\s+modulus|ductility|elongation|sfe|conductivity': 'mechanical_functional_property',
    r'electrodeposition|sputtering|cvd|ecap|hpt|annealing|severe\s+plastic\s+deformation|defect\s+engineering|irradiation': 'synthesis_process',
    r'tem|hrtem|ebsd|xrd|apt|dft|molecular\s+dynamics|md|phase\s+field': 'characterization_modeling_method',
}

def is_valid_nanomat_concept(concept: str) -> bool:
    concept_lower = concept.lower()
    has_domain = any(kw.lower() in concept_lower for kw in ALL_NANOMAT_KEYWORDS)
    has_pattern = any(re.search(p, concept, re.I) for p in NANOMAT_PATTERNS)
...
def normalize_nanomat_concept(concept: str) -> str:
    concept = concept.lower().strip()
    # Materials
    concept = re.sub(r'\bnanotwinned\s+cu(?:pper)?\b|\bnt\s+cu\b', 'nanotwinned_copper', concept)
    concept = re.sub(r'\bcu@ag\b|\bcu/ag\b|\bcore[-\s]?shell\s+cu[-\s]?ag\b', 'core_shell_cuag', concept)
    concept = re.sub(r'\bdefect[-\s]?engineered\s+(?:ag|silver)\b', 'defect_engineered_ag', concept)
    concept = re.sub(r'\bnano[-\s]?particles?\b|\bnps\b', 'nanoparticle', concept)
    # Microstructures
    concept = re.sub(r'\bcoherent\s+twin\s+boundary\b|\bctb\b', 'coherent_twin_boundary', concept)
    concept = re.sub(r'\bincoherent\s+twin\s+boundary\b|\bitb\b', 'incoherent_twin_boundary', concept)
    concept = re.sub(r'\btwin\s+boundary\b', 'twin_boundary', concept)
    concept = re.sub(r'\bstacking\s+fault\b', 'stacking_fault', concept)
    concept = re.sub(r'\bdislocation\b', 'dislocation', concept)
    concept = re.sub(r'\bgrain\s+boundary\b|\bgb\b', 'grain_boundary', concept)
    concept = re.sub(r'\bvacancy\b|\bpoint\s+defect\b', 'vacancy', concept)
    concept = re.sub(r'\binterstitial\b', 'interstitial', concept)
    # Properties
    concept = re.sub(r'\byield\s*strength\b|\bys\b', 'yield_strength', concept)
    concept = re.sub(r'\bultimate\s*tensile\s*strength\b|\btensile\s*strength\b|\buts\b', 'ultimate_tensile_strength', concept)
    concept = re.sub(r'\belastic\s*modulus\b|\byoung\s*modulus\b', 'youngs_modulus', concept)
    concept = re.sub(r'\bhardness\b|\bhv\b', 'hardness', concept)
    concept = re.sub(r'\bductility\b|\belongation\b', 'ductility', concept)
    concept = re.sub(r'\bstacking\s*fault\s*energy\b|\bsfe\b', 'stacking_fault_energy', concept)
    concept = re.sub(r'\belectrical\s*conductivity\b|\bresistivity\b', 'electrical_conductivity', concept)
    concept = re.sub(r'\bthermal\s*conductivity\b', 'thermal_conductivity', concept)
    # Processes
    concept = re.sub(r'\belectrodeposition\b|\belectroplating\b', 'electrodeposition', concept)
    concept = re.sub(r'\bsputtering\b|\bmagnetron\s*sputtering\b|\bpvd\b', 'sputtering', concept)
    concept = re.sub(r'\bchemical\s*vapor\s*deposition\b|\bcvd\b', 'chemical_vapor_deposition', concept)
    concept = re.sub(r'\bannealing\b|\bheat\s*treatment\b', 'annealing', concept)
    concept = re.sub(r'\bsevere\s*plastic\s*deformation\b|\bspd\b|\becap\b|\bhpt\b', 'severe_plastic_deformation', concept)
    concept = re.sub(r'\bdefect\s*engineering\b|\bdefect\s*design\b', 'defect_engineering', concept)
    concept = re.sub(r'\birradiation\b|\bradiation\s*damage\b', 'irradiation', concept)
    # Methods
    concept = re.sub(r'\btransmission\s*electron\s*microscopy\b|\btem\b|\bhrtem\b', 'transmission_electron_microscopy', concept)
    concept = re.sub(r'\belectron\s*backscatter\s*diffraction\b|\bebsd\b', 'electron_backscatter_diffraction', concept)
    concept = re.sub(r'\bx[-\s]?ray\s*diffraction\b|\bxrd\b', 'xray_diffraction', concept)
    concept = re.sub(r'\bdensity\s*functional\s*theory\b|\bdft\b', 'density_functional_theory', concept)
    concept = re.sub(r'\bmolecular\s*dynamics\b|\bmd\b', 'molecular_dynamics', concept)
    return concept
...
# ============================================================================
# QDWA & MICROTRANSFORMER PUBLICATION-Quality STYLING ENGINE
# ============================================================================
def plotly_continuous_scale(cmap_key: str, n: int = 256) -> List[str]:
    """Return a list of hex colors from a matplotlib colormap for Plotly."""
    return get_colormap_colors(cmap_key, n)

def render_qdwa_customization_panel():
    """Publication-quality customization panel for QDWA & Microtransformer charts."""
    with st.expander("🎨 Publication-Quality Customization (50+ Colormaps, Fonts, Layout)", expanded=False):
        c1, c2, c3 = st.columns(3)
        with c1:
            cmaps = list(SUPPORTED_COLORMAPS.keys())
            default_idx = cmaps.index(st.session_state.get("qdwa_cmap", "viridis")) if st.session_state.get("qdwa_cmap", "viridis") in cmaps else 0
            st.selectbox("Colormap (50+ Options)", options=cmaps, index=default_idx,
...
def render_qdwa_sankey(analysis: QDWAAnalysis, theme: Dict[str, str] = None):
    ...
    fig = apply_qdwa_pub_style(fig, is_axial=False)
...
def render_qdwa_radar_chart(analysis: QDWAAnalysis, theme: Dict[str, str] = None):
    ...
    # Note: is_axial=False for polar charts
    fig = apply_qdwa_pub_style(fig, is_axial=False)
    st.plotly_chart(fig, use_container_width=True)
...
def render_qdwa_bar_comparison(analysis: QDWAAnalysis, theme: Dict[str, str] = None):
    ...
    with col1:
        ...
        fig1 = px.bar(df_alloc, x="Category", y=["Raw Evidence", "Smoothed W_k"], barmode="group",
                      title="Raw Evidence vs. Smoothed Weight W_k", color="Smoothed W_k", color_continuous_scale=scale)
        fig1.update_layout(xaxis_tickangle=-45)
        fig1 = apply_qdwa_pub_style(fig1, is_axial=True)
    ...
    with col2:
        ...
        fig2 = go.Figure(go.Bar(x=["H(W)", "H_max=ln(3)", "Deficit"],
                                y=[H, H_max, H_max - H],
                                marker_color=[scale[50], scale[150], scale[200]]))
        fig2.update_layout(title=f"Entropy → c = {analysis.concentration:.4f}", height=400)
        fig2 = apply_qdwa_pub_style(fig2, is_axial=True)
...
def render_qdwa_heatmap(analysis: QDWAAnalysis, theme: Dict[str, str] = None):
    ...
    # Dynamic colormap injection
    scale = plotly_continuous_scale(st.session_state.get("qdwa_cmap", "viridis"))

    fig = px.imshow(df.values, x=df.columns, y=df.index, color_continuous_scale=scale,
    ...
    fig = apply_qdwa_pub_style(fig, is_axial=True)
    st.plotly_chart(fig, use_container_width=True)
...
def render_qdwa_full_dashboard(analysis: QDWAAnalysis, graph: nx.Graph = None, theme: Dict[str, str] = None):
    ...
    # 👇 INJECT CUSTOMIZATION PANEL HERE 👇
...
def render_chord_diagram(token_labels, routing_np, scale=None, theme=None):
    ...
    fig = px.imshow(routing_np, labels=dict(x="Expert", y="Token", color="Activation"),
                    ...
                    title="Token-to-Expert Routing (Heatmap)", color_continuous_scale=scale, aspect="auto")
    fig = apply_qdwa_pub_style(fig, is_axial=True)
    st.plotly_chart(fig, use_container_width=True)
...
def render_advanced_microtransformer_tab(analysis_data, ontology):
    ...
    if st.button("🔍 Run Microtransformer Path Analysis", type="primary"):
        ...
        fig_heat = px.imshow(routing_np, labels=dict(x="Expert", y="Token", color="Activation"),
                             ...
                             color_continuous_scale=scale, aspect="auto")
        fig_heat = apply_qdwa_pub_style(fig_heat, is_axial=True)
        st.plotly_chart(fig_heat, use_container_width=True)
        # 2. Bar chart (Upgraded)
        ...
        fig_bar = go.Figure(go.Bar(x=expert_labels, y=avg_per_expert,
                                   ...
                                   marker_color=scale[:len(expert_labels)]))
        fig_bar.update_layout(title="Average Expert Activation", xaxis_tickangle=-45)
        fig_bar = apply_qdwa_pub_style(fig_bar, is_axial=True)
        ...
        fig_sankey = apply_qdwa_pub_style(fig_sankey, is_axial=False)
        st.plotly_chart(fig_sankey, use_container_width=True)
        # 4. Chord diagram (Upgraded)
        if st.checkbox("Show Chord Diagram"):
            render_chord_diagram(token_labels, routing_np, scale)
        # 5. Interpretation
...
def export_publication_figure(
    ...
    filename="nanomat_graph_pub.png",
) -> bytes:
    try:
        pos = nx.spring_layout(nx_graph, seed=42, k=2.5, iterations=200)
        plt.figure(figsize=figsize, dpi=dpi)
        node_colors = [get_nanomat_category_color(n) for n in nx_graph.nodes()]
        node_sizes = [
        ...
        plt.title(
            "Nanomaterials Quantitative Descriptor Graph",
...
def generate_analysis_report(
    ...
    report: List[str] = []
    report.append("# Nanomaterials Quantitative Descriptor Graph Analysis Report")
    report.append(
...
# ============================================================================
def get_nanomat_category_color(
    ...
    concept_lower = concept.lower()
    category = 'general'
    for pattern, cat in NANOMAT_DESCRIPTOR_MAPPING.items():
        if re.search(pattern, concept_lower):
            category = cat
            break
    color_map = {
        'nanomaterial': '#1f77b4',
        'microstructure_defect': '#ff7f0e',
        'mechanical_functional_property': '#2ca02c',
        'synthesis_process': '#d62728',
        'characterization_modeling_method': '#9467bd',
        'general': '#bcbd22'
    }
...
def render_pyvis_graph(
    ...
    node_size_multiplier=2.0, max_node_size=150,
    ...
    for i, node in enumerate(nx_graph.nodes()):
        ...
        size = int(np.clip((15 + freq * 2.0) * node_size_multiplier, 15, max_node_size))
        color = get_nanomat_category_color(node, cmap_colors)
        degree = int(nx_graph.degree(node))

        original_label = node
        # Determine base label based on mode
        if label_mode == NodeLabelMode.SHORT_NAME:
            label = get_short_name(node)
        elif label_mode == NodeLabelMode.NO_NAME:
            label = ""
        elif label_mode == NodeLabelMode.EXTERNAL_LABEL:
            fallback = get_hierarchy_label(node, style="arrow") if node in _HIERARCHY_PARENTS else node
            label = external_label_text if external_label_text else fallback
        else:
            label = get_hierarchy_label(node, style="arrow") if node in _HIERARCHY_PARENTS else node

        # Abbreviated labels override if enabled (legacy mode, mutually exclusive with new modes)
        ...
        # Font configuration based on label mode
        if label_mode == NodeLabelMode.EXTERNAL_LABEL and not use_abbreviated_labels:
            inside_font_size = max(8, min(int(external_font_size), 60))
            font_dict = {
                'color': external_font_color,
            ...
            inside_font_size = max(8, min(int(node_label_size), 60))
    ...
    try:
        ...
        st.download_button(
            ...
            data=html_bytes,
            file_name="nanomat_concept_graph.html",
            mime="text/html"
        )
...
_SUNBURST_CATEGORY_COLORS = {
    "Nanomaterials":                     "#E74C3C",
    "Microstructures & Defects":         "#E67E22",
    "Mechanical & Functional Properties": "#2ECC71",
    "Synthesis & Processing":            "#F1C40F",
    "Characterization & Modeling":       "#3498DB",
}
    ...
    if export_format == "GraphML":
        try:
            ...
            try:
                ...
                nx.write_graphml(nx_graph, "nanomat_graph.graphml")
            with open("nanomat_graph.graphml", "rb") as f:
                return f.read(), "application/graphml+xml", "nanomat_graph.graphml"
        except Exception as e:
        ...
        return json_str.encode('utf-8'), "application/json", "nanomat_graph_full.json"
    elif export_format == "JSON (Compact)":
        data = nx.node_link_data(nx_graph)
        json_str = json.dumps(data, indent=2, default=str)
        return json_str.encode('utf-8'), "application/json", "nanomat_graph.json"
        ...
        return csv_df.to_csv(index=False).encode('utf-8'), "text/csv", "nanomat_edges_enhanced.csv"
        ...
        return csv_df.to_csv(index=False).encode('utf-8'), "text/csv", "nanomat_nodes_enhanced.csv"
    elif export_format == "PNG":
        try:
            pos = nx.spring_layout(nx_graph, seed=42)
            plt.figure(figsize=(14, 12), dpi=300)
            node_colors = [
                get_nanomat_category_color(n) for n in nx_graph.nodes()
            ]
            nx.draw(
            ...
            buf.seek(0)
            plt.close()
            return buf.read(), "image/png", "nanomat_graph.png"
        except Exception as e:
        ...
        try:
            ...
            buf.seek(0)
            plt.close()
            return buf.read(), "image/svg+xml", "nanomat_graph.svg"
        except Exception as e:
            ...
            with status:
                ...
                def _process_single_row(idx, row, allowed_concepts=None):
                    ...
                    strength_matches = re.findall(
                        r'(\d+(?:\.\d+)?)\s*(?:mpa|gpa|ksi)', text, re.I
                    )
                    if strength_matches:
                        metrics['strength_mpa'] = [float(m) for m in strength_matches]
                    hardness_matches = re.findall(
                        r'(\d+(?:\.\d+)?)\s*(?:hv|gpa|mpa)\s*hardness', text, re.I
                    )
                    if hardness_matches:
                        metrics['hardness'] = [float(m) for m in hardness_matches]
                    conductivity_matches = re.findall(
                        r'(\d+(?:\.\d+)?)\s*(?:ms/m|s/m|%iacs)', text, re.I
                    )
                    if conductivity_matches:
                        metrics['conductivity'] = [float(m) for m in conductivity_matches]
        ...
        with tabs[tab_idx]:
            ...
            if viz_choice == "PyVis (Interactive)":
                render_pyvis_graph(
                    ...
                    node_size_multiplier=st.session_state.get('node_size_multiplier', 2.0),
                    max_node_size=st.session_state.get('max_node_size', 150),
        ...
        with tabs[tab_idx]:
            ...
            if top_scores.empty:
                ...
                st.download_button(
                    ...
                    file_name="nanomat_research_directions.csv", mime="text/csv",
                )
        ...
        with tabs[tab_idx]:
            ...
            if st.button("Generate Publication Figure"):
                ...
                if pub_bytes:
                    st.download_button(
                        ...
                        data=pub_bytes,
                        file_name="nanomat_graph_publication.png",
                        mime="image/png",
                    )
            st.markdown("---")
            ...
            if st.button("Generate Markdown Report"):
                ...
                st.download_button(
                    ...
                    file_name="nanomat_analysis_report.md",
                    mime="text/markdown",
                )
            ...
            concept_list_df = pd.DataFrame({
                ...
                'concept_type': [
                    nx_graph.nodes[c].get('concept_type', 'general') if c in nx_graph else 'general'
                    for c in valid_concepts
                ],
                'definition': [
                    nx_graph.nodes[c].get('definition', '') if c in nx_graph else ''
                    for c in valid_concepts
                ],
            })
            ...
            st.download_button(
                ...
                data=csv_concepts,
                file_name="nanomat_concepts_enhanced.csv", mime="text/csv",
            )
