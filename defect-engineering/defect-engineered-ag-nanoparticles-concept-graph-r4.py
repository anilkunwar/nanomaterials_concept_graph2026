...
# -*- coding: utf-8 -*-
"""
Defect-Engineered Ag Nanoparticle Concept Graph v6.2
=====================================================================
Multi-level reasoning concept graph for defect-engineered silver nanoparticles (Ag NPs).
Focus: Defect engineering, stress-mediated diffusion, pressureless sintering,
...
replaced with those for defect-engineered Ag nanoparticles.
...
DOMAIN: Defect-Engineered Ag Nanoparticles
- Core materials: silver nanoparticles (Ag NPs), fcc silver, power electronics (SiC/GaN)
- Defects: stacking faults, deformation twins, partial dislocations, lattice strain, hydrostatic stress
- Diffusion: stress-mediated diffusion, pipe diffusion, vacancy formation energy, migration barriers
- Sintering: pressureless sintering, ultra-low-temperature sintering, sintering neck, die-attach
- Methods: phase-field modelling, molecular dynamics, DFT, gated attention AI, AC-TEM, GPA, Williamson-Hall
- Synthesis: controlled plastic deformation, polyol method, centrifugation, die-casting
...
Run:
    streamlit run ag_defect_concept_graph_v6.2_llm.py
...
# ============================================================================
st.set_page_config(
    page_title="Defect-Engineered Ag Nanoparticle Concept Graph v6.2",
    page_icon="🧪",
    layout="wide",
...
# ============================================================================
# ENHANCED ONTOLOGY & NLP REASONING SYSTEM (DEFECT-ENGINEERED Ag NPs)
# ============================================================================
...
EDGE_COLOR_REGISTRY: Dict[RelationshipType, str] = {
    ...
    RelationshipType.PREDICTS:          "#00BFFF",   # deep sky blue
...
class DomainOntology:
    """Comprehensive ontology for defect-engineered Ag nanoparticles."""
    ...
    def _build_ontology(self) -> None:
        # === Core Materials ===
        self._add_concept("silver_nanoparticles", ConceptType.MATERIAL,
            synonyms={"ag nanoparticles", "nano-silver", "ag nps", "ag nanocrystals"},
            definition="Silver nanoparticles used as base material for defect engineering")
        self._add_concept("fcc_silver", ConceptType.MATERIAL,
            synonyms={"fcc ag", "face-centered cubic silver", "face-centered cubic ag"},
            definition="Face-centered cubic crystal structure of silver")
        self._add_concept("sic_gan_electronics", ConceptType.MATERIAL,
            synonyms={"sic", "gan", "power electronics", "lead-free solders", "sn-based alloys"},
            definition="Power electronics materials requiring low-temperature packaging")

        # === Defects & Microstructure ===
        self._add_concept("stacking_faults", ConceptType.PROPERTY,
            synonyms={"intrinsic stacking fault", "isf", "extrinsic stacking fault", "esf", "planar defects"},
            definition="Planar defects in the crystal lattice stacking sequence")
        self._add_concept("deformation_twins", ConceptType.PROPERTY,
            synonyms={"coherent twin boundaries", "ctb", "nanotwinning", "twin boundary"},
            definition="Twin boundaries formed by plastic deformation")
        self._add_concept("partial_dislocations", ConceptType.PROPERTY,
            synonyms={"shockley partial dislocations", "shockley partial", "dislocation slip", "slip planes"},
            definition="Partial dislocations bounding stacking faults")
        self._add_concept("lattice_strain", ConceptType.PROPERTY,
            synonyms={"microstrain", "lattice distortion", "lattice microstrain"},
            definition="Elastic strain in the crystal lattice")
        self._add_concept("hydrostatic_stress", ConceptType.PROPERTY,
            synonyms={"von mises stress", "local microstress", "microstress gradients", "eigenstrain", "configurational force"},
            definition="Stress fields driving diffusion and deformation")

        # === Diffusion & Sintering Mechanisms ===
        self._add_concept("stress_mediated_diffusion", ConceptType.PHENOMENON,
            synonyms={"stress-assisted diffusion", "atomic diffusion", "mass transport"},
            definition="Diffusion accelerated by local stress fields")
        self._add_concept("pipe_diffusion", ConceptType.PHENOMENON,
            synonyms={"pipe-diffusion channels", "grain boundary diffusion", "surface diffusion"},
            definition="Fast diffusion along defects like dislocations and boundaries")
        self._add_concept("vacancy_formation_energy", ConceptType.PARAMETER,
            synonyms={"vacancy formation", "migration energy", "migration barrier", "diffusion activation energy", "activation energy"},
            definition="Energy barriers for atomic migration")
        self._add_concept("pressureless_sintering", ConceptType.PROCESS,
            synonyms={"ultra-low-temperature sintering", "low-temperature sintering", "sintering temperature reduction", "low-thermal-budget packaging"},
            definition="Sintering process without external pressure at low temperatures")
        self._add_concept("sintering_neck", ConceptType.PROPERTY,
            synonyms={"surface micro-melting", "laplace pressure"},
            definition="Formation of necks between particles during sintering")

        # === Computational & AI Methods ===
        self._add_concept("phase_field_modelling", ConceptType.METHOD,
            synonyms={"pfm", "phase-field fft model", "phase-field approach", "allen-cahn equation", "fourier spectral method"},
            definition="Phase-field modeling for mesoscale microstructure evolution")
        self._add_concept("molecular_dynamics", ConceptType.METHOD,
            synonyms={"md", "lammps", "embedded atom method", "eam potential"},
            definition="Atomistic simulation of atomic movements and defects")
        self._add_concept("density_functional_theory", ConceptType.METHOD,
            synonyms={"dft", "vasp", "projector augmented wave", "paw", "generalized gradient approximation", "gga", "generalized stacking fault energy", "gsfe"},
            definition="Quantum mechanical calculations for electronic structure and energies")
        self._add_concept("gated_attention_ai", ConceptType.METHOD,
            synonyms={"gated attention", "gated angular distance weighting", "g-adw", "transformer-inspired attention", "cross-attention interpolation", "machine learning interpolators"},
            definition="AI/ML methods for predicting material properties across scales")

        # === Synthesis & Characterization ===
        self._add_concept("controlled_plastic_deformation", ConceptType.PROCESS,
            synonyms={"plastic deformation", "plastic flow", "uniaxial compression", "die-casting", "high-speed centrifugation"},
            definition="Synthesis via inducing plastic deformation to create defects")
        self._add_concept("polyol_method", ConceptType.PROCESS,
            synonyms={"chemical reduction", "thermal reduction", "ethylene glycol"},
            definition="Chemical synthesis method for nanoparticles")
        self._add_concept("aberration_corrected_tem", ConceptType.METHOD,
            synonyms={"ac-tem", "tem", "stem"},
            definition="High-resolution transmission electron microscopy")
        self._add_concept("geometric_phase_analysis", ConceptType.METHOD,
            synonyms={"gpa", "fast fourier transform", "fft"},
            definition="Technique for strain mapping from HRTEM images")
        self._add_concept("williamson_hall_method", ConceptType.METHOD,
            synonyms={"w-h method", "full width at half maximum", "fwhm", "xrd"},
            definition="XRD analysis method for crystallite size and microstrain")
    ...
    def _build_causal_chains(self) -> None:
        causal_chains = [
            # Defects → Properties
            ("stacking_faults", RelationshipType.INFLUENCES, "lattice_strain", 0.85),
            ("deformation_twins", RelationshipType.INFLUENCES, "lattice_strain", 0.80),
            ("partial_dislocations", RelationshipType.CAUSES, "stacking_faults", 0.90),
            ("controlled_plastic_deformation", RelationshipType.CAUSES, "deformation_twins", 0.95),
            ("controlled_plastic_deformation", RelationshipType.CAUSES, "stacking_faults", 0.90),
            # Stress & Diffusion
            ("lattice_strain", RelationshipType.CAUSES, "hydrostatic_stress", 0.85),
            ("hydrostatic_stress", RelationshipType.DRIVES, "stress_mediated_diffusion", 0.92),
            ("stress_mediated_diffusion", RelationshipType.CAUSES, "atomic_diffusion", 0.88),
            ("partial_dislocations", RelationshipType.ENABLES, "pipe_diffusion", 0.85),
            ("vacancy_formation_energy", RelationshipType.CONSTRAINS, "atomic_diffusion", 0.75),
            # Sintering Mechanism
            ("atomic_diffusion", RelationshipType.CAUSES, "sintering_neck", 0.90),
            ("pipe_diffusion", RelationshipType.ACCELERATES, "sintering_neck", 0.85),
            ("sintering_neck", RelationshipType.ENABLES, "pressureless_sintering", 0.95),
            ("pressureless_sintering", RelationshipType.ENABLES, "power_electronics_packaging", 0.80),
            # Methods ↔ Materials
            ("molecular_dynamics", RelationshipType.MODELS, "stress_mediated_diffusion", 0.90),
            ("phase_field_modelling", RelationshipType.MODELS, "sintering_neck", 0.85),
            ("density_functional_theory", RelationshipType.COMPUTES, "vacancy_formation_energy", 0.95),
            ("gated_attention_ai", RelationshipType.PREDICTS, "vacancy_formation_energy", 0.80),
            ("aberration_corrected_tem", RelationshipType.MEASURES, "stacking_faults", 0.95),
            ("geometric_phase_analysis", RelationshipType.MEASURES, "lattice_strain", 0.90),
            ("williamson_hall_method", RelationshipType.MEASURES, "lattice_strain", 0.85),
        ]
...
_HIERARCHY_PARENTS: Dict[str, Tuple[str, int]] = {
    # --- Root domain ---
    "silver_nanoparticles": (None, 0),
    "fcc_silver": (None, 0),
    "sic_gan_electronics": (None, 0),
    
    # --- Tier 1: Defect & Microstructure ---
    "stacking_faults":             ("Defect & Microstructure", 1),
    "deformation_twins":           ("Defect & Microstructure", 1),
    "partial_dislocations":        ("Defect & Microstructure", 1),
    
    # --- Tier 1: Stress & Strain Field ---
    "lattice_strain":              ("Stress & Strain Field", 1),
    "hydrostatic_stress":          ("Stress & Strain Field", 1),
    
    # --- Tier 1: Diffusion Mechanism ---
    "stress_mediated_diffusion":   ("Diffusion Mechanism", 1),
    "pipe_diffusion":              ("Diffusion Mechanism", 1),
    "vacancy_formation_energy":    ("Diffusion Mechanism", 1),
    "sintering_neck":              ("Diffusion Mechanism", 1),
    
    # --- Tier 1: Sintering & Application ---
    "pressureless_sintering":      ("Sintering & Application", 1),
    
    # --- Tier 1: Computational & AI Method ---
    "phase_field_modelling":       ("Computational & AI Method", 1),
    "molecular_dynamics":          ("Computational & AI Method", 1),
    "density_functional_theory":   ("Computational & AI Method", 1),
    "gated_attention_ai":          ("Computational & AI Method", 1),
    
    # --- Tier 1: Synthesis & Processing ---
    "controlled_plastic_deformation": ("Synthesis & Processing", 1),
    "polyol_method":               ("Synthesis & Processing", 1),
    
    # --- Tier 1: Advanced Characterization ---
    "aberration_corrected_tem":    ("Advanced Characterization", 1),
    "geometric_phase_analysis":    ("Advanced Characterization", 1),
    "williamson_hall_method":      ("Advanced Characterization", 1),
}
...
def get_hierarchy_label(concept_key: str,
    ...
    ----------
    concept_key : str
        The canonical name key used in the ontology (e.g. "stacking_faults").
    style : str
        "arrow"   → "Defect & Microstructure → Stacking Faults"
        "bracket" → "Defect & Microstructure [Stacking Faults]"
        "dot"     → "Defect & Microstructure · Stacking Faults"
...
def get_hierarchy_path(concept_key: str) -> List[str]:
    ...
    ["Defect-Engineered Ag NPs", "Defect & Microstructure", "Stacking Faults"].
    ...
    if entry is None or entry[0] is None:
        return ["Defect-Engineered Ag NPs", leaf]

    parent_label = entry[0]
    return ["Defect-Engineered Ag NPs", parent_label, leaf]
...
def build_sunburst_data(
    ...
    # --- Root node ---
    root_id = "Defect-Engineered Ag NPs"
    ids.append(root_id)
    labels.append("Defect-Engineered Ag NPs")
...
class AdvancedConceptResolver:
    ...
    def _context_disambiguation(self, text: str, context: str) -> Optional[str]:
        ...
        ag_indicators = [
            'stacking fault', 'twin', 'partial dislocation', 'planar defect',
            'lattice strain', 'microstrain', 'hydrostatic stress',
            'diffusion', 'vacancy', 'migration barrier', 'pipe diffusion',
            'sintering', 'pressureless', 'packaging', 'die-attach',
            'phase-field', 'dft', 'md', 'molecular dynamics', 'gated attention',
            'tem', 'gpa', 'williamson-hall', 'xrd'
        ]
        if any(ind in context_lower for ind in ag_indicators):
            if 'stacking fault' in text or 'twin' in text:
                return 'stacking_faults' if 'stacking' in text else 'deformation_twins'
            if 'strain' in text or 'stress' in text:
                return 'lattice_strain' if 'strain' in text else 'hydrostatic_stress'
            if 'diffusion' in text or 'vacancy' in text:
                return 'stress_mediated_diffusion' if 'stress' in text else 'vacancy_formation_energy'
            if 'sintering' in text or 'pressureless' in text:
                return 'pressureless_sintering'
            if 'phase-field' in text or 'dft' in text or 'md' in text:
                return 'phase_field_modelling' if 'phase' in text else ('density_functional_theory' if 'dft' in text else 'molecular_dynamics')
        return None
...
# ============================================================================
# ENHANCED CONCEPT EXTRACTOR (Defect-Engineered Ag NPs)
# ============================================================================
class EnhancedConceptExtractor:
    ...
    def _build_extraction_patterns(self) -> None:
        # Defect-engineered Ag NP specific patterns
        ...
        self.processing_patterns = [
            r'\b(?:plastic\s*deformation|centrifugation|die[-\s]*casting|polyol\s*method|solid[-\s]*state\s*reaction)\b'
        ]
        self.characterization_patterns = [
            r'\b(?:aberration[-\s]*corrected\s*tem|ac[-\s]*tem|geometric\s*phase\s*analysis|gpa|williamson[-\s]*hall|fwhm|fast\s*fourier\s*transform|fft)\b'
        ]
        ...
        self.all_patterns = (
            self.defect_patterns + self.stress_patterns + self.diffusion_patterns +
            self.sintering_patterns + self.computational_patterns +
            self.processing_patterns + self.characterization_patterns +
            self.material_patterns
        )
...
class ReasoningEnhancedGraphBuilder:
    ...
    def _infer_cross_domain_bridges(
        ...
        # For Ag-defect domain, bridge between materials and methods/properties
        material_nodes = [
        ...
        # Bridge materials to properties
...
# ============================================================================
# DEFECT-ENGINEERED Ag NPs KEYWORDS
# ============================================================================
MATERIAL_KEYWORDS = ["silver nanoparticles", "ag nanoparticles", "nano-silver", "ag nps", "fcc ag", "sic", "gan", "power electronics"]
DEFECT_KEYWORDS = ["stacking fault", "deformation twin", "intrinsic stacking fault", "isf", "extrinsic stacking fault", "esf", "coherent twin boundary", "ctb", "nanotwinning", "planar defect", "partial dislocation", "shockley partial"]
STRESS_KEYWORDS = ["lattice strain", "microstrain", "hydrostatic stress", "von mises stress", "microstress", "eigenstrain", "configurational force", "lattice distortion"]
DIFFUSION_KEYWORDS = ["stress-mediated diffusion", "stress-assisted diffusion", "vacancy formation energy", "migration barrier", "diffusion activation energy", "mean square displacement", "msd", "atomic diffusion", "mass transport", "pipe-diffusion", "grain boundary diffusion", "surface diffusion"]
SINTERING_KEYWORDS = ["pressureless sintering", "ultra-low-temperature sintering", "sintering temperature", "power electronics packaging", "die-attach", "electronic packaging", "sintering neck", "laplace pressure"]
COMPUTATIONAL_KEYWORDS = ["phase-field modelling", "pfm", "allen-cahn", "molecular dynamics", "md", "lammps", "eam potential", "density-functional theory", "dft", "vasp", "gsfe", "gated attention", "g-adw", "transformer-inspired attention"]
PROCESS_KEYWORDS = ["plastic deformation", "high-speed centrifugation", "die-casting", "uniaxial compression", "polyol method", "solid-state reaction"]
METHOD_KEYWORDS = ["aberration-corrected tem", "ac-tem", "geometric phase analysis", "gpa", "fast fourier transform", "fft", "williamson-hall method", "fwhm", "xrd"]

ALL_DOMAIN_KEYWORDS = (MATERIAL_KEYWORDS + DEFECT_KEYWORDS + STRESS_KEYWORDS + DIFFUSION_KEYWORDS + SINTERING_KEYWORDS + COMPUTATIONAL_KEYWORDS + PROCESS_KEYWORDS + METHOD_KEYWORDS)
...
def is_valid_defect_ag_concept(concept: str) -> bool:
    concept_lower = concept.lower()
    has_domain = any(kw.lower() in concept_lower for kw in ALL_DOMAIN_KEYWORDS)
    has_pattern = any(re.search(p, concept, re.I) for p in AG_DEFECT_PATTERNS)
    generic = {
...
def normalize_defect_ag_concept(concept: str) -> str:
    concept = concept.lower().strip()
    concept = re.sub(r'\bintrinsic\s*stacking\s*fault\b|\bextrinsic\s*stacking\s*fault\b', 'stacking_faults', concept)
    concept = re.sub(r'\bcoherent\s*twin\s*boundaries\b|\bcoherent\s*twin\s*boundary\b|\bdeformation\s*twin\b', 'deformation_twins', concept)
    concept = re.sub(r'\bshockley\s*partial\s*dislocation\b|\bshockley\s*partial\b|\bpartial\s*dislocation\b', 'partial_dislocations', concept)
    concept = re.sub(r'\bstress[-\s]*mediated\s*diffusion\b|\bstress[-\s]*assisted\s*diffusion\b', 'stress_mediated_diffusion', concept)
    concept = re.sub(r'\bpipe[-\s]*diffusion\b', 'pipe_diffusion', concept)
    concept = re.sub(r'\bvacancy\s*formation\s*energy\b|\bmigration\s*barrier\b|\bdiffusion\s*activation\s*energy\b', 'vacancy_formation_energy', concept)
    concept = re.sub(r'\bultra[-\s]*low[-\s]*temperature\s*sintering\b|\bpressureless\s*sintering\b', 'pressureless_sintering', concept)
    concept = re.sub(r'\bphase[-\s]*field\s*modelling\b|\bphase[-\s]*field\s*modeling\b', 'phase_field_modelling', concept)
    concept = re.sub(r'\bgated\s*attention\b|\bg[-\s]*adw\b', 'gated_attention_ai', concept)
    concept = re.sub(r'\bgeneralized\s*stacking\s*fault\s*energy\b|\bgsfe\b', 'density_functional_theory', concept)
    concept = re.sub(r'\baberration[-\s]*corrected\s*tem\b|\bac[-\s]*tem\b', 'aberration_corrected_tem', concept)
    concept = re.sub(r'\bgeometric\s*phase\s*analysis\b|\bgpa\b', 'geometric_phase_analysis', concept)
    concept = re.sub(r'\bwilliamson[-\s]*hall\b', 'williamson_hall_method', concept)
    concept = re.sub(r'\bsilver\s*nanoparticles\b|\bag\s*nanoparticles\b|\bnano[-\s]*silver\b|\bag\s*nps\b', 'silver_nanoparticles', concept)
    concept = re.sub(r'\bface[-\s]*centered\s*cubic\s*silver\b|\bfcc\s*ag\b', 'fcc_silver', concept)
    return concept
...
def extract_concepts_from_text(text: str) -> List[str]:
    concepts: Set[str] = set()
    text_lower = text.lower()
    for pattern in AG_DEFECT_PATTERNS:
    ...
    noun_pattern = (
        r'\b(?:[a-z]+(?:[-\s]?[a-z]+){0,2}[-\s]?)?'
        r'(?:stacking fault|twin|dislocation|strain|stress|diffusion|vacancy|sintering|packaging|phase-field|dft|md|tem|gpa|fwhm)\b'
    )
    matches = re.findall(noun_pattern, text, re.I)
    for m in matches:
        concept = m.lower().strip()
        if is_valid_defect_ag_concept(concept):
            concepts.add(concept)
...
def extract_concepts_from_abstracts(
    ...
    for idx, row in df.iterrows():
        ...
        metrics = extract_doc_metrics(combined_text)  # uses the new extract_doc_metrics
        all_metrics.append(metrics)
        concepts = extract_concepts_from_text(combined_text)
        normalized = [normalize_defect_ag_concept(c) for c in concepts]
...
def normalize_and_filter_concepts(
    ...
    for doc_idx, concepts in enumerate(all_concepts):
        ...
        for c in concepts:
            if c not in seen_in_doc and is_valid_defect_ag_concept(c):
                concept_counts[c] += 1
...
def abstract_concepts_to_categories(concepts: List[str]) -> Dict[str, str]:
    ...
    for concept in concepts:
        matched = False
        for pattern, category in AG_DEFECT_DESCRIPTOR_MAPPING.items():
            if re.search(pattern, concept, re.I):
                concept_to_abstract[concept] = category
                matched = True
                break
        if not matched:
            if any(re.search(p, concept, re.I) for p in [r'stacking', r'twin', r'dislocation']):
                concept_to_abstract[concept] = 'defect_microstructure'
            elif any(re.search(p, concept, re.I) for p in [r'strain', r'stress']):
                concept_to_abstract[concept] = 'stress_strain_field'
            elif any(re.search(p, concept, re.I) for p in [r'diffusion', r'vacancy', r'migration']):
                concept_to_abstract[concept] = 'diffusion_mechanism'
            elif any(re.search(p, concept, re.I) for p in [r'sintering', r'packaging', r'die']):
                concept_to_abstract[concept] = 'sintering_application'
            elif any(re.search(p, concept, re.I) for p in [r'phase', r'dft', r'md', r'attention']):
                concept_to_abstract[concept] = 'computational_ai_method'
            elif any(re.search(p, concept, re.I) for p in [r'plastic', r'polyol', r'centrifugation']):
                concept_to_abstract[concept] = 'synthesis_processing'
            elif any(re.search(p, concept, re.I) for p in [r'tem', r'gpa', r'williamson', r'xrd']):
                concept_to_abstract[concept] = 'advanced_characterization'
            elif any(re.search(p, concept, re.I) for p in [r'ag', r'silver', r'nanoparticle']):
...
def export_publication_figure(
    ...
    filename="ag_defect_graph_pub.png",
) -> bytes:
    try:
        pos = nx.spring_layout(nx_graph, seed=42, k=2.5, iterations=200)
        plt.figure(figsize=figsize, dpi=dpi)
        node_colors = [get_defect_ag_category_color(n) for n in nx_graph.nodes()]
        node_sizes = [
        ...
        plt.title(
            "Defect-Engineered Ag Nanoparticle Concept Graph",
...
def generate_analysis_report(
    ...
    report: List[str] = []
    report.append("# Defect-Engineered Ag Nanoparticle Concept Graph Analysis Report")
    report.append(
    ...
    report.append("")
    report.append("---")
    report.append("*Report generated by Defect-Engineered Ag Nanoparticle Concept Graph v6.2*")
    return "\n".join(report)
...
# ============================================================================
def get_defect_ag_category_color(concept: str, cmap_colors: Optional[List[str]] = None) -> str:
    if cmap_colors:
        return cmap_colors[hash(concept) % len(cmap_colors)]
    concept_lower = concept.lower()
    category = 'general'
    for pattern, cat in AG_DEFECT_DESCRIPTOR_MAPPING.items():
        if re.search(pattern, concept_lower):
            category = cat
            break
    color_map = {
        'core_material_system': '#E74C3C',     # red
        'defect_microstructure': '#F39C12',    # orange
        'stress_strain_field': '#9B59B6',      # purple
        'diffusion_mechanism': '#3498DB',      # blue
        'sintering_application': '#2ECC71',    # green
        'computational_ai_method': '#1ABC9C',  # teal
        'synthesis_processing': '#E67E22',     # dark orange
        'advanced_characterization': '#2980B9',# dark blue
        'general': '#95A5A6'                   # grey
    }
    return color_map.get(category, '#95A5A6')

# Alias for compatibility
get_mpea_category_color = get_defect_ag_category_color
...
def render_pyvis_graph(
    ...
    for i, node in enumerate(nx_graph.nodes()):
        ...
        color = get_defect_ag_category_color(node, cmap_colors)
    ...
    try:
        ...
        st.download_button(
            ...
            data=html_bytes,
            file_name="ag_defect_concept_graph.html",
            mime="text/html"
        )
...
_SUNBURST_CATEGORY_COLORS = {
    "Core Material System":       "#E74C3C",
    "Defect & Microstructure":    "#F39C12",
    "Stress & Strain Field":      "#9B59B6",
    "Diffusion Mechanism":        "#3498DB",
    "Sintering & Application":    "#2ECC71",
    "Computational & AI Method":  "#1ABC9C",
    "Synthesis & Processing":     "#E67E22",
    "Advanced Characterization":  "#2980B9",
}
...
def build_category_hierarchy(
    ...
    - Root (center): "Defect-Engineered Ag NPs"
    - Ring 1: Categories
    ...
    root_label = "Defect-Engineered Ag NPs"
    ...
    if export_format == "GraphML":
        try:
            if include_metadata:
                ...
                nx_graph.graph['tool'] = 'Ag-Defect-ConceptGraph'
            try:
                nx.write_graphml_lxml(nx_graph, "ag_defect_graph.graphml")
            except Exception:
                nx.write_graphml(nx_graph, "ag_defect_graph.graphml")
            with open("ag_defect_graph.graphml", "rb") as f:
                return f.read(), "application/graphml+xml", "ag_defect_graph.graphml"
        except Exception as e:
        ...
        return json_str.encode('utf-8'), "application/json", "ag_defect_graph_full.json"
    elif export_format == "JSON (Compact)":
        data = nx.node_link_data(nx_graph)
        json_str = json.dumps(data, indent=2, default=str)
        return json_str.encode('utf-8'), "application/json", "ag_defect_graph.json"
        ...
        return csv_df.to_csv(index=False).encode('utf-8'), "text/csv", "ag_defect_edges_enhanced.csv"
        ...
        return csv_df.to_csv(index=False).encode('utf-8'), "text/csv", "ag_defect_nodes_enhanced.csv"
    elif export_format == "PNG":
        try:
            pos = nx.spring_layout(nx_graph, seed=42)
            plt.figure(figsize=(14, 12), dpi=300)
            node_colors = [
                get_defect_ag_category_color(n) for n in nx_graph.nodes()
            ]
            nx.draw(
            ...
            buf.seek(0)
            plt.close()
            return buf.read(), "image/png", "ag_defect_graph.png"
        except Exception as e:
        ...
        try:
            ...
            buf.seek(0)
            plt.close()
            return buf.read(), "image/svg+xml", "ag_defect_graph.svg"
        except Exception as e:
        ...
        try:
            if include_metadata:
                nx_graph.graph['created'] = datetime.now().isoformat()
                nx_graph.graph['version'] = '6.2'
            nx.write_gexf(nx_graph, "ag_defect_graph.gexf")
            with open("ag_defect_graph.gexf", "rb") as f:
                return f.read(), "application/xml", "ag_defect_graph.gexf"
        except Exception as e:
...
def render_reasoning_dashboard(
    ...
    synonym_examples = [
        ("ag nanoparticle", "silver_nanoparticles"),
        ("stacking fault energy", "stacking_faults"),
        ("pressureless sintering", "pressureless_sintering"),
        ("phase field modeling", "phase_field_modelling"),
...
def extract_doc_metrics(text: str) -> Dict[str, Any]:
    """Regex metric extraction for Defect-Engineered Ag Nanoparticle literature."""
    metrics: Dict[str, Any] = {}
    ...
    sf_energy_matches = re.findall(r'(\d+(?:\.\d+)?)\s*(?:mj/m²|mj/m\^2|erg/cm\^2)', text, re.I)
    if sf_energy_matches:
        metrics['stacking_fault_energy'] = [float(m) for m in sf_energy_matches]
    
    # Vacancy formation / migration / activation energy
    energy_matches = re.findall(r'(?:vacancy\s*formation|migration\s*barrier|activation\s*energy)\s*(?:of|is|=|:)?\s*(\d+(?:\.\d+)?)\s*(?:ev|kj/mol)', text, re.I)
    if energy_matches:
        metrics['activation_energy_ev'] = [float(m) for m in energy_matches]
    
    # Sintering temperature
    sinter_temp_matches = re.findall(r'sinter(?:ing)?\s*(?:temperature)?\s*(?:of|is|=|:|at)?\s*(\d+(?:\.\d+)?)\s*(?:°c|k|celsius)', text, re.I)
    if sinter_temp_matches:
        metrics['sintering_temperature_c'] = [float(m) for m in sinter_temp_matches]
    
    # Shear strength
    shear_matches = re.findall(r'shear\s*strength\s*(?:of|is|=|:)?\s*(\d+(?:\.\d+)?)\s*(?:mpa|gpa)', text, re.I)
    if shear_matches:
        metrics['shear_strength_mpa'] = [float(m) for m in shear_matches]
    
    # Conductivities
    cond_matches = re.findall(r'(?:electrical|thermal)\s*conductivity\s*(?:of|is|=|:)?\s*(\d+(?:\.\d+)?)\s*(?:s/cm|ms/cm|s/m|w/mk|w/m·k)', text, re.I)
    if cond_matches:
        metrics['conductivity'] = [float(m) for m in cond_matches]
        
    # Mean Square Displacement
    msd_matches = re.findall(r'msd\s*(?:of|is|=|:)?\s*(\d+(?:\.\d+)?)\s*(?:Å²|angstrom\^2|nm\^2)', text, re.I)
    if msd_matches:
        metrics['msd_angstrom_sq'] = [float(m) for m in msd_matches]
...
def render_sidebar() -> None:
    with st.sidebar:
        ...
        st.subheader("🔬 Defect-Engineered Ag NPs Focus Areas")
        st.markdown("- **Core Materials:** Ag nanoparticles, FCC silver, SiC/GaN electronics")
        st.markdown("- **Defects & Microstructure:** Stacking faults, Deformation twins, Partial dislocations")
        st.markdown("- **Stress & Strain:** Lattice strain, Hydrostatic stress, Microstress gradients")
        st.markdown("- **Diffusion:** Stress-mediated diffusion, Pipe diffusion, Vacancy formation energy")
        st.markdown("- **Sintering:** Pressureless sintering, Low-temperature packaging, Sintering neck")
        st.markdown("- **Methods:** Phase-field, MD, DFT, Gated attention AI, AC-TEM, GPA, Williamson-Hall")
        st.markdown("- **Synthesis:** Controlled plastic deformation, Polyol method, Centrifugation")
        st.markdown("- **Characterization:** Aberration-corrected TEM, GPA, XRD, FFT")
        st.subheader("🧠 NLP Reasoning Options")
        st.session_state['use_ontology'] = st.checkbox(
            "Use ontology-based resolution", value=True,
            help="Maps synonyms like 'ISF', 'PFM' to canonical concepts",
        )
        ...
        st.session_state['use_relationship_extraction'] = st.checkbox(
            ...
            help="Identifies causal links between defects and properties",
        )
...
# ============================================================================
# 1. QUERY ANALYSIS DATA STRUCTURES (Defect-Engineered Ag NPs)
# ============================================================================
class DefectAgCoreProblem(Enum):
    SINTERING_TEMPERATURE_REDUCTION = "sintering_temperature_reduction"
    DEFECT_ENGINEERING_MECHANISM = "defect_engineering_mechanism"
    STRESS_DIFFUSION_COUPLING = "stress_diffusion_coupling"
    MULTISCALE_MODELING = "multiscale_modeling"
    GENERAL = "general"
    MULTI_PROBLEM = "multi_problem"

@dataclass
class DefectAgProblemDefinition:
    problem_id: DefectAgCoreProblem
    title: str
...
# Pre-defined Problem Definitions
AG_DEFECT_PROBLEM_DEFINITIONS: Dict[DefectAgCoreProblem, DefectAgProblemDefinition] = {
    DefectAgCoreProblem.SINTERING_TEMPERATURE_REDUCTION: DefectAgProblemDefinition(
        problem_id=DefectAgCoreProblem.SINTERING_TEMPERATURE_REDUCTION, title="Ultra-Low-Temperature Pressureless Sintering",
        scientific_description="Achieving dense sintering of Ag NPs at ultra-low temperatures without external pressure for power electronics packaging.",
        root_cause="High activation energy for atomic diffusion limits sintering kinetics at low temperatures.",
        key_concepts=["pressureless_sintering", "sintering_neck", "stress_mediated_diffusion", "sic_gan_electronics"],
        key_relationships=[("stress_mediated_diffusion", "CAUSES", "sintering_neck"), ("sintering_neck", "ENABLES", "pressureless_sintering")],
        solution_directions=["Introduce high densities of defects (stacking faults/twins) to lower diffusion barrier", "Utilize stress-mediated diffusion", "Reduce particle size to increase surface area"],
        relevant_materials=["silver_nanoparticles", "sic_gan_electronics"],
        relevant_phenomena=["pressureless_sintering", "sintering_neck"],
        relevant_properties=["lattice_strain", "vacancy_formation_energy"],
        example_queries=["How to achieve ultra-low-temperature sintering for Ag nanoparticles?", "What enables pressureless sintering in nano-silver?"],
        visualization_focus=["sintering_temperature_plot", "defect_density_correlation"]
    ),
    DefectAgCoreProblem.DEFECT_ENGINEERING_MECHANISM: DefectAgProblemDefinition(
        problem_id=DefectAgCoreProblem.DEFECT_ENGINEERING_MECHANISM, title="Engineering Stacking Faults and Twin Boundaries",
        scientific_description="Controlling the introduction of planar defects (ISF/ESF) and deformation twins to enhance properties.",
        root_cause="Standard synthesis yields low defect densities; controlled plastic deformation is required.",
        key_concepts=["stacking_faults", "deformation_twins", "partial_dislocations", "controlled_plastic_deformation"],
        key_relationships=[("controlled_plastic_deformation", "CAUSES", "deformation_twins"), ("partial_dislocations", "CAUSES", "stacking_faults")],
        solution_directions=["Apply high-speed centrifugation or uniaxial compression", "Optimize polyol synthesis parameters", "Introduce stress gradients"],
        relevant_materials=["silver_nanoparticles", "fcc_silver"],
        relevant_phenomena=["controlled_plastic_deformation"],
        relevant_properties=["stacking_faults", "deformation_twins", "lattice_strain"],
        example_queries=["How are deformation twins introduced in Ag nanoparticles?", "What is the role of Shockley partial dislocations in forming stacking faults?"],
        visualization_focus=["tem_defect_visualization", "strain_mapping"]
    ),
    DefectAgCoreProblem.STRESS_DIFFUSION_COUPLING: DefectAgProblemDefinition(
        problem_id=DefectAgCoreProblem.STRESS_DIFFUSION_COUPLING, title="Stress-Mediated Diffusion Mechanism",
        scientific_description="Local hydrostatic stresses and microstress gradients accelerate atomic mass transport.",
        root_cause="Lattice distortion and eigenstrains from defects create configurational forces driving diffusion.",
        key_concepts=["stress_mediated_diffusion", "hydrostatic_stress", "pipe_diffusion", "vacancy_formation_energy"],
        key_relationships=[("hydrostatic_stress", "DRIVES", "stress_mediated_diffusion"), ("partial_dislocations", "ENABLES", "pipe_diffusion")],
        solution_directions=["Engineer defect distributions to optimize stress fields", "Lower vacancy migration barriers via alloying", "Utilize pipe diffusion along dislocation cores"],
        relevant_materials=["silver_nanoparticles"],
        relevant_phenomena=["stress_mediated_diffusion", "pipe_diffusion"],
        relevant_properties=["hydrostatic_stress", "lattice_strain", "vacancy_formation_energy"],
        example_queries=["How does hydrostatic stress drive atomic diffusion?", "Explain the role of pipe diffusion in lowering sintering temperature."],
        visualization_focus=["stress_field_heatmap", "diffusion_pathway_visualization"]
    ),
    DefectAgCoreProblem.MULTISCALE_MODELING: DefectAgProblemDefinition(
        problem_id=DefectAgCoreProblem.MULTISCALE_MODELING, title="Multiscale Modeling and AI Interpolation",
        scientific_description="Bridging DFT, MD, and Phase-Field models using gated attention AI to predict defect evolution.",
        root_cause="Single-scale models fail to capture the full atomistic-to-mesoscale linkage of defect dynamics.",
        key_concepts=["phase_field_modelling", "molecular_dynamics", "density_functional_theory", "gated_attention_ai"],
        key_relationships=[("density_functional_theory", "COMPUTES", "vacancy_formation_energy"), ("phase_field_modelling", "MODELS", "sintering_neck")],
        solution_directions=["Use DFT to parameterize MD potentials", "Feed MD outputs into Phase-Field models", "Apply G-ADW transformers for cross-scale interpolation"],
        relevant_materials=["fcc_silver"],
        relevant_phenomena=["stress_mediated_diffusion"],
        relevant_properties=["vacancy_formation_energy"],
        example_queries=["How is phase-field modelling used to predict sintering?", "What is the role of gated attention AI in multiscale material modeling?"],
        visualization_focus=["model_architecture_diagram", "simulation_vs_experiment_plot"]
    ),
    DefectAgCoreProblem.GENERAL: DefectAgProblemDefinition(
        problem_id=DefectAgCoreProblem.GENERAL, title="General Defect Engineering Inquiry", scientific_description="General inquiry about defect-engineered Ag nanoparticles.",
        root_cause="N/A", key_concepts=["silver_nanoparticles"], key_relationships=[], solution_directions=[],
        relevant_materials=[], relevant_phenomena=[], relevant_properties=[], example_queries=["What are defect-engineered Ag nanoparticles?"], visualization_focus=["general_overview"]
    ),
    DefectAgCoreProblem.MULTI_PROBLEM: DefectAgProblemDefinition(
        problem_id=DefectAgCoreProblem.MULTI_PROBLEM, title="Multi-Problem Defect Ag Inquiry", scientific_description="Inquiry spanning multiple core problems.",
...
class QueryAnalysisResult:
    ...
    primary_problem: DefectAgCoreProblem
    secondary_problems: List[DefectAgCoreProblem]
...
class FallbackAnalyzer(LLMQueryAnalyzer):
    PROBLEM_KEYWORDS = {
        DefectAgCoreProblem.SINTERING_TEMPERATURE_REDUCTION: {"sintering", "temperature", "pressureless", "ultra-low", "packaging", "die-attach"},
        DefectAgCoreProblem.DEFECT_ENGINEERING_MECHANISM: {"stacking fault", "twin", "partial dislocation", "deformation", "planar defect"},
        DefectAgCoreProblem.STRESS_DIFFUSION_COUPLING: {"stress", "diffusion", "hydrostatic", "pipe diffusion", "vacancy", "migration"},
        DefectAgCoreProblem.MULTISCALE_MODELING: {"phase-field", "dft", "md", "molecular dynamics", "gated attention", "multiscale", "ai"},
    }
    ...
    def analyze_query(self, query: str, ontology: Any) -> QueryAnalysisResult:
        ...
        primary = max(problem_scores, key=problem_scores.get) if sum(problem_scores.values()) > 0 else DefectAgCoreProblem.GENERAL
        ...
        inferred = []
        if primary != DefectAgCoreProblem.GENERAL:
            pdef = AG_DEFECT_PROBLEM_DEFINITIONS[primary]
        ...
        priorities = {}
        pdef = AG_DEFECT_PROBLEM_DEFINITIONS.get(primary, AG_DEFECT_PROBLEM_DEFINITIONS[DefectAgCoreProblem.GENERAL])
...
class OpenAIQueryAnalyzer(LLMQueryAnalyzer):
    ...
    def analyze_query(self, query: str, ontology: Any) -> QueryAnalysisResult:
        ...
        system_prompt = """You are an expert in defect-engineered silver nanoparticles. Analyze the user's query and return ONLY valid JSON with:
        1. "primary_problem": One of: sintering_temperature_reduction, defect_engineering_mechanism, stress_diffusion_coupling, multiscale_modeling, general, multi_problem
        ...
        try:
            ...
            problem_map = {p.value: p for p in DefectAgCoreProblem}
            primary = problem_map.get(parsed.get("primary_problem", "general"), DefectAgCoreProblem.GENERAL)
            ...
            return QueryAnalysisResult(
                ...
                visualization_focus=AG_DEFECT_PROBLEM_DEFINITIONS[primary].visualization_focus, reasoning_chain=parsed.get("reasoning_chain", ["LLM analysis completed"]), confidence=0.85
            )
        except Exception as e:
...
class LocalLLMQueryAnalyzer(LLMQueryAnalyzer):
    ...
    def analyze_query(self, query: str, ontology: Any) -> QueryAnalysisResult:
        if not self.is_available():
            return FallbackAnalyzer().analyze_query(query, ontology)
        prompt = (
            f"[INST] You are an expert in defect-engineered Ag nanoparticles. Analyze: '{query}'. "
...
class DynamicOntologyExpander:
    ...
    def _create_bridge_concept(self, missing_concept: str, source_query: str, problem: DefectAgCoreProblem) -> Optional[Dict]:
        bridge_name = f"query_bridge_{missing_concept.replace(' ', '_').lower()}"
        if bridge_name in self.ontology.concepts: return None
        pdef = AG_DEFECT_PROBLEM_DEFINITIONS.get(problem, AG_DEFECT_PROBLEM_DEFINITIONS[DefectAgCoreProblem.GENERAL])
...
class GraphRAGAnswerGenerator:
    ...
    def generate_ground_response(self, query: str, analysis: QueryAnalysisResult, subgraph: nx.Graph, concept_abstract_map: Dict[str, List[int]], all_texts: Union[List[str], Dict[int, str]], max_docs_per_concept: int = 2) -> str:
        ...
        nl = chr(10)
        prompt = "You are an expert in defect-engineered silver nanoparticles. Answer the user's query based *strictly* on the provided graph context and evidence snippets." + nl
        ...
        if evidence_snippets:
            ...
            prompt += "No direct text snippets found. Rely on your general knowledge of defect engineering in Ag nanoparticles but note the lack of specific retrieved context." + nl
...
class QuerySessionManager:
    SESSION_KEY = "ag_defect_query_session"
    @classmethod
...
def render_llm_query_panel(ontology: Any, expander: DynamicOntologyExpander, full_graph: nx.Graph) -> Optional[QueryAnalysisResult]:
    ...
    example_queries = [q for pdef in AG_DEFECT_PROBLEM_DEFINITIONS.values() for q in pdef.example_queries[:1]]
    selected_example = st.sidebar.selectbox("Or select an example:", [""] + example_queries, key="example_query_select")
    query = st.sidebar.text_area("Your question:", value=selected_example, height=100, key="llm_query_input", placeholder="e.g., How to achieve pressureless sintering of Ag nanoparticles?")
...
def render_llm_qa_tab(analysis_data: Dict, ontology: Any):
    ...
    with col1: query = st.text_input("Enter your research question:", placeholder="e.g., What enables pressureless sintering of Ag nanoparticles?")
...
def main() -> None:
    ...
    st.caption(
        "Multi-level reasoning concept graph for defect-engineered silver nanoparticles (Ag NPs) | "
        "Focus: Defect engineering, stress-mediated diffusion, pressureless sintering, multiscale modeling, and advanced characterization | "
        ...
        with tabs[tab_idx]:
            ...
            if top_scores.empty:
                ...
                st.download_button(
                    ...
                    file_name="ag_defect_research_directions.csv", mime="text/csv",
                )
        ...
        with tabs[tab_idx]:
            ...
            if st.button("Generate Publication Figure"):
                ...
                if pub_bytes:
                    st.download_button(
                        ...
                        data=pub_bytes,
                        file_name="ag_defect_graph_publication.png",
                        mime="image/png",
                    )
            st.markdown("---")
            ...
            if st.button("Generate Markdown Report"):
                ...
                st.download_button(
                    ...
                    file_name="ag_defect_analysis_report.md",
                    mime="text/markdown",
                )
            ...
            st.download_button(
                ...
                data=csv_concepts,
                file_name="ag_defect_concepts_enhanced.csv", mime="text/csv",
            )
